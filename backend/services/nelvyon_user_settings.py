...
class Nelvyon_user_settingsService(WorkspaceAwareMixin):
    """Legacy nelvyon_user_settings — workspace-scoped (Fase 6)."""

    model = Nelvyon_user_settings
    ...
    ) -> Optional[Nelvyon_user_settings]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a nelvyon_user_settings row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for nelvyon_user_settings {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update nelvyon_user_settings")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Nelvyon_user_settings")
            if not hasattr(Nelvyon_user_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_user_settings")
            result = await self.db.execute(
                select(Nelvyon_user_settings).where(
                    getattr(Nelvyon_user_settings, field_name) == field_value,
                    Nelvyon_user_settings.workspace_id == workspace_id,
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_user_settings by {field_name}: {str(e)}")
            raise
    ...
    async def list_by_field(
        ...
        limit: int = 20,
        workspace_id: Optional[int] = None,
    ) -> List[Nelvyon_user_settings]:
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Nelvyon_user_settings")
            if not hasattr(Nelvyon_user_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_user_settings")
            result = await self.db.execute(
                select(Nelvyon_user_settings)
                .where(
                    getattr(Nelvyon_user_settings, field_name) == field_value,
                    Nelvyon_user_settings.workspace_id == workspace_id,
                )
                .offset(skip)
                .limit(limit)
                .order_by(Nelvyon_user_settings.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_user_settingss by {field_name}: {str(e)}")
            raise
