...
from models.os_deals import Os_deals
from services.workspace_mixin import WorkspaceAwareMixin

logger = logging.getLogger(__name__)


class Os_dealsService(WorkspaceAwareMixin):
    """NELVYON OS internal pipeline deals — workspace-scoped (Fase 2C)."""
    ...
    async def create(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create an os_deals row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for os_deals {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update os_deals")
