...
from models.partner_records import Partner_records
from services.workspace_mixin import WorkspaceAwareMixin

logger = logging.getLogger(__name__)


class Partner_recordsService(WorkspaceAwareMixin):
    """Partner records — workspace-scoped (PII)."""

    model = Partner_records
    ...
    ) -> Optional[Partner_records]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a partner_records row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for partner_records {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update partner_records")
    ...
    async def get_by_field(
        ...
        if workspace_id is None:
            raise ValueError("workspace_id is required for get_by_field on Partner_records")
        if not hasattr(Partner_records, field_name):
            raise ValueError(f"Field {field_name} does not exist on Partner_records")
        result = await self.db.execute(
            select(Partner_records).where(
                getattr(Partner_records, field_name) == field_value,
                Partner_records.workspace_id == workspace_id,
            )
        )
    ...
    async def list_by_field(
        ...
        limit: int = 20,
        workspace_id: Optional[int] = None,
    ) -> List[Partner_records]:
        if workspace_id is None:
            raise ValueError("workspace_id is required for list_by_field on Partner_records")
        if not hasattr(Partner_records, field_name):
            raise ValueError(f"Field {field_name} does not exist on Partner_records")
        result = await self.db.execute(
            select(Partner_records)
            .where(
                getattr(Partner_records, field_name) == field_value,
                Partner_records.workspace_id == workspace_id,
            )
            .offset(skip)
            .limit(limit)
            .order_by(Partner_records.id.desc())
        )
