...
class MessagesService:
    ...
    async def create(
        ...
        try:
            ...
            # Source of truth: message workspace_id must match conversation.workspace_id.
            data["workspace_id"] = conv.workspace_id
    ...
    async def get_by_id(
        ...
        """Por ID; si `workspace_id` se pasa, aislamiento tenant por fila (preferido SaaS)."""
        try:
            query = select(Messages).where(Messages.id == obj_id)
            if workspace_id is not None:
                query = query.where(Messages.workspace_id == workspace_id)
            elif user_id:
    ...
    async def get_list(
        ...
        """Listado: si `workspace_id` está definido, aislamiento tenant por fila (preferido SaaS)."""
        try:
            ...
            if workspace_id is not None:
                query = query.where(Messages.workspace_id == workspace_id)
                count_query = count_query.where(Messages.workspace_id == workspace_id)
            elif user_id:
                query = query.where(Messages.user_id == user_id)
    ...
    async def update(
        ...
        """Update messages; scope por workspace cuando se proporciona."""
        try:
    ...
    async def delete(
        ...
        """Delete messages; scope por workspace cuando se proporciona."""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id, workspace_id=workspace_id)
            if not obj:
