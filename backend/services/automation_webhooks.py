...
class Automation_webhooksService(WorkspaceAwareMixin):
    """Automation webhooks — workspace-scoped CRUD."""

    model = Automation_webhooks
    ...
    ) -> Optional[Automation_webhooks]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create an automation_webhook")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for automation_webhooks {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update automation_webhooks")
    ...
    async def get_by_field(
        ...
        if workspace_id is None:
            raise ValueError("workspace_id is required for get_by_field on Automation_webhooks")
        if not hasattr(Automation_webhooks, field_name):
            raise ValueError(f"Field {field_name} does not exist on Automation_webhooks")
        result = await self.db.execute(
            select(Automation_webhooks).where(
                getattr(Automation_webhooks, field_name) == field_value,
                Automation_webhooks.workspace_id == workspace_id,
            )
        )
    ...
    async def list_by_field(
        ...
        limit: int = 20,
        workspace_id: Optional[int] = None,
    ) -> List[Automation_webhooks]:
        if workspace_id is None:
            raise ValueError("workspace_id is required for list_by_field on Automation_webhooks")
        if not hasattr(Automation_webhooks, field_name):
            raise ValueError(f"Field {field_name} does not exist on Automation_webhooks")
        result = await self.db.execute(
            ...
            .offset(skip)
            .limit(limit)
            .order_by(Automation_webhooks.id.desc())
        )
