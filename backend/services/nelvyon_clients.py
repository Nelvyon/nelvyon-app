...
class Nelvyon_clientsService(WorkspaceAwareMixin):
    """Legacy nelvyon_clients — workspace-scoped (Fase 6)."""

    model = Nelvyon_clients
    ...
    async def create(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a nelvyon_clients row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for nelvyon_clients {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update nelvyon_clients")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Nelvyon_clients")
            if not hasattr(Nelvyon_clients, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_clients")
            result = await self.db.execute(
                select(Nelvyon_clients).where(
                    getattr(Nelvyon_clients, field_name) == field_value,
                    Nelvyon_clients.workspace_id == workspace_id,
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_clients by {field_name}: {str(e)}")
            raise
    ...
    async def list_by_field(
        ...
        limit: int = 20,
        workspace_id: Optional[int] = None,
    ) -> List[Nelvyon_clients]:
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Nelvyon_clients")
            if not hasattr(Nelvyon_clients, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_clients")
            result = await self.db.execute(
                select(Nelvyon_clients)
                .where(
                    getattr(Nelvyon_clients, field_name) == field_value,
                    Nelvyon_clients.workspace_id == workspace_id,
                )
                .offset(skip)
                .limit(limit)
                .order_by(Nelvyon_clients.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_clientss by {field_name}: {str(e)}")
            raise
