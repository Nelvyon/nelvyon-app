...
class Nelvyon_projectsService(WorkspaceAwareMixin):
    """Legacy nelvyon_projects — workspace-scoped (Fase 6)."""

    model = Nelvyon_projects
    ...
    async def create(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a nelvyon_projects row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for nelvyon_projects {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update nelvyon_projects")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Nelvyon_projects")
            if not hasattr(Nelvyon_projects, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_projects")
            result = await self.db.execute(
                select(Nelvyon_projects).where(
                    getattr(Nelvyon_projects, field_name) == field_value,
                    Nelvyon_projects.workspace_id == workspace_id,
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_projects by {field_name}: {str(e)}")
            raise
    ...
    async def list_by_field(
        ...
        limit: int = 20,
        workspace_id: Optional[int] = None,
    ) -> List[Nelvyon_projects]:
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Nelvyon_projects")
            if not hasattr(Nelvyon_projects, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_projects")
            result = await self.db.execute(
                select(Nelvyon_projects)
                .where(
                    getattr(Nelvyon_projects, field_name) == field_value,
                    Nelvyon_projects.workspace_id == workspace_id,
                )
                .offset(skip)
                .limit(limit)
                .order_by(Nelvyon_projects.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_projectss by {field_name}: {str(e)}")
            raise
