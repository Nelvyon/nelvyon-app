...
from models.nelvyon_assets import Nelvyon_assets
from services.workspace_mixin import WorkspaceAwareMixin

logger = logging.getLogger(__name__)


class Nelvyon_assetsService(WorkspaceAwareMixin):
    """Legacy nelvyon_assets — workspace-scoped (Fase 6)."""

    model = Nelvyon_assets
    ...
    ) -> Optional[Nelvyon_assets]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a nelvyon_assets row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for nelvyon_assets {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update nelvyon_assets")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Nelvyon_assets")
            if not hasattr(Nelvyon_assets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_assets")
            result = await self.db.execute(
                select(Nelvyon_assets).where(
                    getattr(Nelvyon_assets, field_name) == field_value,
                    Nelvyon_assets.workspace_id == workspace_id,
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_assets by {field_name}: {str(e)}")
            raise
    ...
    async def list_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Nelvyon_assets")
            if not hasattr(Nelvyon_assets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_assets")
            result = await self.db.execute(
                select(Nelvyon_assets)
                .where(
                    getattr(Nelvyon_assets, field_name) == field_value,
                    Nelvyon_assets.workspace_id == workspace_id,
                )
                .offset(skip)
                .limit(limit)
                .order_by(Nelvyon_assets.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_assetss by {field_name}: {str(e)}")
            raise
