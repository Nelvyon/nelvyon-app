...
class Nelvyon_agentsService(WorkspaceAwareMixin):
    """Legacy nelvyon_agents — workspace-scoped (Fase 6)."""

    model = Nelvyon_agents
    ...
    ) -> Optional[Nelvyon_agents]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a nelvyon_agents row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for nelvyon_agents {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update nelvyon_agents")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Nelvyon_agents")
            if not hasattr(Nelvyon_agents, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_agents")
            result = await self.db.execute(
                select(Nelvyon_agents).where(
                    getattr(Nelvyon_agents, field_name) == field_value,
                    Nelvyon_agents.workspace_id == workspace_id,
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_agents by {field_name}: {str(e)}")
            raise
    ...
    async def list_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Nelvyon_agents")
            if not hasattr(Nelvyon_agents, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_agents")
            result = await self.db.execute(
                select(Nelvyon_agents)
                .where(
                    getattr(Nelvyon_agents, field_name) == field_value,
                    Nelvyon_agents.workspace_id == workspace_id,
                )
                .offset(skip)
                .limit(limit)
                .order_by(Nelvyon_agents.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_agentss by {field_name}: {str(e)}")
            raise
