...
class Sales_recordsService(WorkspaceAwareMixin):
    """Sales records — workspace-scoped."""
    ...
    ) -> Optional[Sales_records]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create a sales_records row")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for sales_records {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update sales_records")
    ...
    async def get_by_field(
        ...
        if workspace_id is None:
            raise ValueError("workspace_id is required for get_by_field on Sales_records")
        if not hasattr(Sales_records, field_name):
            raise ValueError(f"Field {field_name} does not exist on Sales_records")
        result = await self.db.execute(
            select(Sales_records).where(
                getattr(Sales_records, field_name) == field_value,
                Sales_records.workspace_id == workspace_id,
            )
        )
    ...
    async def list_by_field(
        ...
        if workspace_id is None:
            raise ValueError("workspace_id is required for list_by_field on Sales_records")
        if not hasattr(Sales_records, field_name):
            raise ValueError(f"Field {field_name} does not exist on Sales_records")
        result = await self.db.execute(
            select(Sales_records)
            .where(
                getattr(Sales_records, field_name) == field_value,
                Sales_records.workspace_id == workspace_id,
            )
            .offset(skip)
            .limit(limit)
            .order_by(Sales_records.id.desc())
        )
