...
def should_initialize_mock_data() -> bool:
    """
    Control whether JSON fixtures under mock_data/ are loaded at startup.

    - production / staging: only if ALLOW_MOCK_SEED is explicitly true (never by default).
    - ENVIRONMENT unset: allowed (typical local dev) unless MGX_IGNORE_INIT_DATA is set.
    - development / test / dev: allowed unless MGX_IGNORE_INIT_DATA is set.
    """
    if "MGX_IGNORE_INIT_DATA" in os.environ:
        logger.info("Mock data: skipped (MGX_IGNORE_INIT_DATA)")
        return False
    raw = os.getenv("ENVIRONMENT")
    if raw is None:
        # Local runs often omit ENVIRONMENT; allow fixtures unless explicitly gated below.
        return True
    env = raw.lower()
    if env in ("production", "prod", "staging"):
        allowed = os.getenv("ALLOW_MOCK_SEED", "").strip().lower() in ("true", "1", "yes")
        if not allowed:
            logger.info(
                "Mock data: skipped in %s (set ALLOW_MOCK_SEED=true to load mock_data/*.json)",
                env,
            )
        return allowed
    return True
...
async def initialize_mock_data():
    ...
    if not should_initialize_mock_data():
        return
