...
class AppointmentsService(WorkspaceAwareMixin):
    """Service layer for Appointments — workspace-aware (Fase 6)."""
    ...
    async def create(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create an appointment")
        payload = dict(data)
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update an appointment")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Appointments")
            ...
            result = await self.db.execute(
                select(Appointments).where(
                    getattr(Appointments, field_name) == field_value,
                    Appointments.workspace_id == workspace_id,
                )
            )
    ...
    async def list_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Appointments")
            if not hasattr(Appointments, field_name):
                raise ValueError(f"Field {field_name} does not exist on Appointments")
            result = await self.db.execute(
                select(Appointments)
