...
class Nelvyon_outputsService(WorkspaceAwareMixin):
    """Legacy nelvyon_outputs — workspace-scoped (Fase 6)."""

    model = Nelvyon_outputs
    ...
    ) -> Optional[Nelvyon_outputs]:
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to create nelvyon_outputs")
        payload = dict(data)
    ...
    async def check_ownership(
        ...
        try:
            ...
            logger.error(f"Error checking ownership for nelvyon_outputs {obj_id}: {str(e)}")
            return False
    ...
    async def update(
        ...
        if user_id is None or workspace_id is None:
            raise ValueError("user_id and workspace_id are required to update nelvyon_outputs")
    ...
    async def get_by_field(
        ...
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for get_by_field on Nelvyon_outputs")
            if not hasattr(Nelvyon_outputs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_outputs")
            result = await self.db.execute(
                select(Nelvyon_outputs).where(
                    getattr(Nelvyon_outputs, field_name) == field_value,
                    Nelvyon_outputs.workspace_id == workspace_id,
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_outputs by {field_name}: {str(e)}")
            raise
    ...
    async def list_by_field(
        ...
        limit: int = 20,
        workspace_id: Optional[int] = None,
    ) -> List[Nelvyon_outputs]:
        try:
            if workspace_id is None:
                raise ValueError("workspace_id is required for list_by_field on Nelvyon_outputs")
            if not hasattr(Nelvyon_outputs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Nelvyon_outputs")
            result = await self.db.execute(
                select(Nelvyon_outputs)
                .where(
                    getattr(Nelvyon_outputs, field_name) == field_value,
                    Nelvyon_outputs.workspace_id == workspace_id,
                )
                .offset(skip)
                .limit(limit)
                .order_by(Nelvyon_outputs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching nelvyon_outputss by {field_name}: {str(e)}")
            raise
