...
const FULL_VERIFIED_NOTE =
  "UI message catalog (apps/web/messages/{locale}.json) has 100% key parity with es for ALL namespaces — verified by automated disk tests. SaaS namespaces (nav/common/errors/settings/sso/audit) are natively localized for fr/de/it/pt where Spanish differs from English; brand tokens (CRM, SEO, WhatsApp, …) may match EN by design. Email is PARTIAL (Resend welcome/passwordReset/invoice/jobCompleted/onboardingComplete + SES payment_failed/cancellation localized; other SES catalog + billing dunning/offboarding templates Spanish-only — human/legal review before claiming FULL). PDF is PARTIAL (quote/invoice label maps for es/en/fr/de/it/pt; not full document/legal localization). Do not claim email/PDF FULL_VERIFIED.";
