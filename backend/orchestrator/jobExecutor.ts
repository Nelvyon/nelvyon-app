...
function acceptanceForAgent(agentId: string): string[] {
  ...
  const byAgent: Record<string, string[]> = {
    ...
    cto: [...common, "mentions_technical"],
    marketing: [...common, "has_next_steps"],
    google_ads: [...common],
    meta_ads: [...common],
    tiktok_ads: [...common],
    email_marketing: [...common],
    qa: [...common],
    workflows: [...common],
    portal_client: [...common],
    operations: [...common, "mentions_summary"],
    devops: [...common, "mentions_technical"],
    social_media: [...common, "mentions_content"],
    product: [...common, "has_next_steps"],
...
function buildSandboxDeliverable(agentId: string, input: string, correlationId: string): string {
  ...
  return [
    ...
    brief,
    `## Analysis`,
    `Sandbox deliverable for ${agentId}. Opportunity/CRM/SEO/support/content/report/finance/risk/technical/summary/ads/marketing as applicable.`,
    `## Recommendations`,
    `- Validate assumptions with tenant data`,
    `- Escalate sensitive actions for human approval`,
    `- No paid spend or production deploy without approval`,
    `## Next steps`,
