...
from core.session_cookie import attach_session_cookie, clear_session_cookie
...
def get_spa_callback_base(request: Request) -> str:
    """Where the browser loads the SPA; OIDC redirect lands here /auth/callback (no token in query)."""
    try:
        explicit = (getattr(settings, "frontend_app_url", None) or "").strip()
    except AttributeError:
        explicit = ""
    if explicit:
        return explicit.rstrip("/")
    return get_dynamic_backend_url(request).rstrip("/")


def _session_max_age_seconds() -> int:
    try:
        jwt_min = int(getattr(settings, "jwt_expire_minutes", 60))
    except (TypeError, ValueError):
        jwt_min = 60
    return max(60, jwt_min * 60)
...
async def callback(
    ...
    try:
        ...
        attach_session_cookie(redirect_response, request, app_token, _session_max_age_seconds())
...
async def exchange_platform_token(
    ...
    user_email = payload_data.get("email", "") or ""
    if not user_email and str(platform_user_id) == str(getattr(settings, "admin_user_id", "")):
        user_email = getattr(settings, "admin_user_email", "") or ""
    user_name = payload_data.get("name") or payload_data.get("username")
    if not user_name:
        user_name = derive_name_from_email(user_email)

    user = await auth_service.get_or_create_user(platform_sub=platform_user_id, email=user_email, name=user_name)
    logger.info("[token/exchange] User verified: platform_user_id=%s, role=%s", platform_user_id, user.role)

    app_token, _expires_at, _ = await auth_service.issue_app_token(user=user)
    logger.info("[token/exchange] Token issued successfully for user_id=%s", user.id)

    body = TokenExchangeResponse(token=app_token)
    response = JSONResponse(content=body.model_dump())
    attach_session_cookie(response, request, app_token, _session_max_age_seconds())
    return response
...
async def logout(request: Request):
    """Logout: clear HttpOnly session cookie and return IdP logout URL if configured."""
