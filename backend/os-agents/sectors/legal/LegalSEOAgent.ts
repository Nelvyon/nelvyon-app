...
export type LegalSEOAgentDeps = { llm?: ILlmClient };
export class LegalSEOAgent {
  constructor(private readonly deps: LegalSEOAgentDeps = {}) {}
...
### BRIEF OPERATIVO
SEO local y de nicho para despacho "${enriched.firmName}" (${enriched.practiceArea}, público ${enriched.targetClient}, ${enriched.location ?? "principal ciudad España"}):
- Lista de keywords long-tail por especialidad y ciudad (ej. abogado laboralista Madrid, gestoría fiscal Barcelona).
- Ficha Google My Business: categorías sugeridas, descripción, atributos, publicaciones tipo.
- Meta titles y meta descriptions para 5 páginas de servicio típicas.
- Ejemplo de JSON-LD para LegalService/Attorney (schema.org) con placeholders.
- Estrategia ética para reseñas en Google (solicitud, timing, cumplimiento LOPD).
- Link building: directorios jurídicos y medios especializados con tácticas seguras (guest post, estudios citables).
Incluye recordatorio de cumplimiento normativa publicidad abogacía.`;
    const __crmOut = { agentId: "legal-seo", result: await this.llm.complete(prompt, legalMarketingLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let legalSEOAgentCached: LegalSEOAgent | undefined;
export function getLegalSEOAgent(): LegalSEOAgent { if (!legalSEOAgentCached) legalSEOAgentCached = new LegalSEOAgent(); return legalSEOAgentCached; }
export function resetLegalSEOAgentForTests(): void { legalSEOAgentCached = undefined; }
