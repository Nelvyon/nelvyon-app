...
export type LegalThoughtLeadershipAgentDeps = { llm?: ILlmClient };
export class LegalThoughtLeadershipAgent {
  constructor(private readonly deps: LegalThoughtLeadershipAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Thought leadership jurídico para "${enriched.firmName}" (${enriched.practiceArea}, tono ${enriched.tone}, audiencia ${enriched.targetClient}):
- Artículo opinión estilo medio especializado (tesis, 3 argumentos, conclusión prudente).
- Guión podcast/vídeo 8-10 min sobre tema legal de actualidad vinculado al área.
- Hilo educativo LinkedIn/Twitter (5-7 tuits) sobre cambio legislativo reciente (genérico si no se da norma).
- Plantilla ponencia congreso (título, objetivos, bloques, Q&A).
- Bio ejecutiva para medios y conferencias (150 y 300 palabras).
- Estrategia LinkedIn para socios: frecuencia, formatos, temas, compliance publicitario.
Disclaimer: contenido divulgativo, no asesoramiento.`;
    const __crmOut = { agentId: "legal-thought-leadership", result: await this.llm.complete(prompt, legalMarketingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let legalThoughtLeadershipCached: LegalThoughtLeadershipAgent | undefined;
export function getLegalThoughtLeadershipAgent(): LegalThoughtLeadershipAgent { if (!legalThoughtLeadershipCached) legalThoughtLeadershipCached = new LegalThoughtLeadershipAgent(); return legalThoughtLeadershipCached; }
export function resetLegalThoughtLeadershipAgentForTests(): void { legalThoughtLeadershipCached = undefined; }
