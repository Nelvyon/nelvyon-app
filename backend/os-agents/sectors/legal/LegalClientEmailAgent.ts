...
export type LegalClientEmailAgentDeps = { llm?: ILlmClient };
export class LegalClientEmailAgent {
  constructor(private readonly deps: LegalClientEmailAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Secuencia de emails para despacho "${enriched.firmName}" (${enriched.practiceArea}, tono ${enriched.tone}, clientes ${enriched.targetClient}):
1) Email bienvenida nuevo cliente: qué esperar del proceso, plazos orientativos, documentación habitual (sin prometer resultados).
2) Plantilla actualización estado del caso semanal/quincenal adaptable.
3) Email cierre de caso con resumen y próximos pasos recomendados.
4) Solicitud de reseña post-caso (Google) respetando LOPD y secreto profesional.
5) Newsletter trimestral con novedades legales filtradas al perfil del cliente (estructura + 3 bullets de ejemplo).
Usa placeholders [Nombre], [Referencia caso]. Mantén cumplimiento deontológico.`;
    const __crmOut = { agentId: "legal-client-email", result: await this.llm.complete(prompt, legalMarketingLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let legalClientEmailCached: LegalClientEmailAgent | undefined;
export function getLegalClientEmailAgent(): LegalClientEmailAgent { if (!legalClientEmailCached) legalClientEmailCached = new LegalClientEmailAgent(); return legalClientEmailCached; }
export function resetLegalClientEmailAgentForTests(): void { legalClientEmailCached = undefined; }
