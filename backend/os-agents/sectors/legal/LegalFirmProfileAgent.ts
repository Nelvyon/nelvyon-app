...
export type LegalFirmProfileAgentDeps = { llm?: ILlmClient };
export class LegalFirmProfileAgent {
  constructor(private readonly deps: LegalFirmProfileAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Eres estratega de marca para despachos en España. Genera perfil profesional completo para "${enriched.firmName}" (área: ${enriched.practiceArea}, tono: ${enriched.tone}, cliente objetivo: ${enriched.targetClient}, ubicación: ${enriched.location ?? "España"}):
- Presentación institucional con años de experiencia y especialización (sin inflar cifras inventadas; indica marco genérico si faltan datos).
- Bio de socios principales con estructura de credenciales (ICAM, doctorado, master) dejando placeholders [NOMBRE], [AÑOS] donde aplique.
- Áreas de práctica en lenguaje ciudadano accesible, sin jerga innecesaria.
- Diferenciadores creíbles frente a otros despachos.
- Texto optimizado para Google My Business.
- Página web "Quiénes somos" (HTML/Markdown breve).
- Descripciones para directorios jurídicos tipo Economist & Jurist, Abogados.es.
Aviso: no es asesoramiento legal; contenido comunicacional.`;
    const __crmOut = { agentId: "legal-firm-profile", result: await this.llm.complete(prompt, legalMarketingLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let legalFirmProfileCached: LegalFirmProfileAgent | undefined;
export function getLegalFirmProfileAgent(): LegalFirmProfileAgent { if (!legalFirmProfileCached) legalFirmProfileCached = new LegalFirmProfileAgent(); return legalFirmProfileCached; }
export function resetLegalFirmProfileAgentForTests(): void { legalFirmProfileCached = undefined; }
