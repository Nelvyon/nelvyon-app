...
export type LegalReputationAgentDeps = { llm?: ILlmClient };
export class LegalReputationAgent {
  constructor(private readonly deps: LegalReputationAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Gestión reputación online para "${enriched.firmName}" (${enriched.practiceArea}, ${enriched.targetClient}):
- Sistema captación reseñas post-caso (email + plantilla WhatsApp corta) con opt-in LOPD.
- Plantillas respuesta reseñas positivas y negativas sin vulnerar secreto profesional.
- Estrategia directorios premium (Martindale, Chambers, Legal 500): pasos, materiales, diferenciación.
- Plan para menciones en medios: pitch, notas de prensa temáticas.
- Protocolo crisis reputacional (roles, tiempos, canales, línea con colegio).
No incluyas datos personales reales.`;
    const __crmOut = { agentId: "legal-reputation", result: await this.llm.complete(prompt, legalMarketingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let legalReputationCached: LegalReputationAgent | undefined;
export function getLegalReputationAgent(): LegalReputationAgent { if (!legalReputationCached) legalReputationCached = new LegalReputationAgent(); return legalReputationCached; }
export function resetLegalReputationAgentForTests(): void { legalReputationCached = undefined; }
