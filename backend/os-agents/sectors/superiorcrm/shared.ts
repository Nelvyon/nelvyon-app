...
export interface SuperiorCrmInput {
  userId: string;
  sector: string;
  brand: string;
  crmBrief?: string;
...
export interface SuperiorCrmOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseSuperiorCrmLlmJson(raw: string, label: string): Omit<SuperiorCrmOutput, "agentId"> {
...
export function buildSuperiorCrmPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: SuperiorCrmInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const crm = params.input.crmBrief?.trim() ? params.input.crmBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

SUPERIORIDAD CRM NELVYON OS (v1):
- **Prefijo agentes**: **SuperiorCrm**; pipeline, scoring y retención predictiva.
- **Win rate >35%** en pipeline cualificado; **lead scoring accuracy >90%**.
- **Cycle time** reducción **>25%** vs baseline; **forecast accuracy >92%**.
- **Enriquecimiento automático >85%** de contactos.
- **Detección churn >30 días** de antelación.
...
export async function runSuperiorCrmAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: SuperiorCrmInput,
  temperature: number,
): Promise<SuperiorCrmOutput> {
  const prompt = buildSuperiorCrmPrompt({
  ...
  const parsed = parseSuperiorCrmLlmJson(raw, agentId);
  const out: SuperiorCrmOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSuperiorCrmLlm(): ILlmClient {
