...
export type BuildContentMarketingAgentDeps = { llm?: ILlmClient };
export class BuildContentMarketingAgent {
  constructor(private readonly deps: BuildContentMarketingAgentDeps = {}) {}
  ...
  async run(userId: string, input: BuildInput): Promise<BuildOutput> {
    ...
    const loc = enriched.location ? ` Área: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Contenido construcción/arquitectura "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, tono ${enriched.tone}).${loc}
- 20 ideas IG/LinkedIn/Pinterest obra materiales errores antes-después autorización obra.
- Artículo blog guía reforma elegir profesional materiales sostenibles outline 1200 palabras.
- Guión vídeo timelapse obra short long form hooks.
- Newsletter mensual tendencias sector ejemplo titulares.
- Pinterest tableros SEO pins tráfico portfolio.`;
    const __crmOut = { agentId: "build-content-marketing", result: await this.llm.complete(prompt, buildLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let buildContentMarketingCached: BuildContentMarketingAgent | undefined;
export function getBuildContentMarketingAgent(): BuildContentMarketingAgent { if (!buildContentMarketingCached) buildContentMarketingCached = new BuildContentMarketingAgent(); return buildContentMarketingCached; }
export function resetBuildContentMarketingAgentForTests(): void { buildContentMarketingCached = undefined; }
