...
export type BuildReferralNetworkAgentDeps = { llm?: ILlmClient };
export class BuildReferralNetworkAgent {
  constructor(private readonly deps: BuildReferralNetworkAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Red referidos obra "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}, ${enriched.location ?? "región flexible"}).
- Mensaje cliente satisfecho pide recomendación post acta obra con ética profesional sin comisiones opacas prohibidas revise normativa mercantil.
- Colaboración arquitecto-constructor reciprocalidad servicios delineados briefing mutuo.
- Acuerdos inmobiliaria agente promotor outline valor compartido.
- Prescriptores interioristas programa comisión transparente declarable plantilla alto nivel.
Convenios proveedor referencia cruzada condiciones revisión jurídica.`;
    const __crmOut = { agentId: "build-referral-network", result: await this.llm.complete(prompt, buildLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let buildReferralNetworkCached: BuildReferralNetworkAgent | undefined;
export function getBuildReferralNetworkAgent(): BuildReferralNetworkAgent { if (!buildReferralNetworkCached) buildReferralNetworkCached = new BuildReferralNetworkAgent(); return buildReferralNetworkCached; }
export function resetBuildReferralNetworkAgentForTests(): void { buildReferralNetworkCached = undefined; }
