...
export type BuildLeadGenerationAgentDeps = { llm?: ILlmClient };
export class BuildLeadGenerationAgent {
  constructor(private readonly deps: BuildLeadGenerationAgentDeps = {}) {}
  ...
  async run(userId: string, input: BuildInput): Promise<BuildOutput> {
    ...
    const loc = enriched.location ?? "ciudad ejemplo";
...
### BRIEF OPERATIVO
Generación leads construcción "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}, foco geo ${loc}):
- Landing presupuesto formulario campos calificación GDPR.
- Google Search keywords alta intención reformas integrales unifamiliar naves ejemplo.
- Meta antes/después con permiso uso imagen proyecto.
- Habitissimo Certicalia perfil rápido respuesta lead scoring sugerido.
- Colaboración inmobiliarias reforma post compra pitch breve.`;
    const __crmOut = { agentId: "build-lead-generation", result: await this.llm.complete(prompt, buildLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let buildLeadGenerationCached: BuildLeadGenerationAgent | undefined;
export function getBuildLeadGenerationAgent(): BuildLeadGenerationAgent { if (!buildLeadGenerationCached) buildLeadGenerationCached = new BuildLeadGenerationAgent(); return buildLeadGenerationCached; }
export function resetBuildLeadGenerationAgentForTests(): void { buildLeadGenerationCached = undefined; }
