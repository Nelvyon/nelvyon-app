...
export type BuildBudgetConversionAgentDeps = { llm?: ILlmClient };
export class BuildBudgetConversionAgent {
  constructor(private readonly deps: BuildBudgetConversionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Conversión presupuestos obra "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}, ${enriched.location ?? "zona proyecto"}).
- Email envío presupuesto valor plazos calidad garantías licencias proyectos ejecutivos placeholders.
- Diferenciación vs oferta económica agresiva enfoque riesgos calidad soporte obra.
- Secuencias 3/7/14 días asuntos bullets breves sin presión abusiva.
- Objeciones precio demora confianza con respuestas modelo transparencia.
- Invitación visita obra oficina para proyectos complejos checklist temas técnico.
- Esquema contrato obra sencillo cláusulas revisión jurídica obligada no automatizar firma.`;
    const __crmOut = { agentId: "build-budget-conversion", result: await this.llm.complete(prompt, buildLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let buildBudgetConversionCached: BuildBudgetConversionAgent | undefined;
export function getBuildBudgetConversionAgent(): BuildBudgetConversionAgent { if (!buildBudgetConversionCached) buildBudgetConversionCached = new BuildBudgetConversionAgent(); return buildBudgetConversionCached; }
export function resetBuildBudgetConversionAgentForTests(): void { buildBudgetConversionCached = undefined; }
