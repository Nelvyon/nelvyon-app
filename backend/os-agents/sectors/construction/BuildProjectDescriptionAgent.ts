...
export type BuildProjectDescriptionAgentDeps = { llm?: ILlmClient };
export class BuildProjectDescriptionAgent {
  constructor(private readonly deps: BuildProjectDescriptionAgentDeps = {}) {}
  ...
  async run(userId: string, input: BuildInput): Promise<BuildOutput> {
    ...
    const loc = enriched.location ?? "mercado local";
...
### BRIEF OPERATIVO
Ficha proyecto portfolio "${enriched.businessName}" (${enriched.serviceType}, cliente tipo ${enriched.targetClient}, tono ${enriched.tone}, ${loc}):
- Nombre proyecto evocador placeholder.
- Historia cliente reto solución resultado sin datos privados identifying.
- Especificaciones técnicas beneficios uso materiales elegidos justificación diseño función.
- m² cronograma adherencia presupuesto solo si pueden documentarse.
- Adaptaciones texto web LinkedIn Houzz mismo núcleo.
- CTA presupuesto similar sin compromiso legal sustitutivo.`;
    const __crmOut = { agentId: "build-project-description", result: await this.llm.complete(prompt, buildLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let buildProjectDescriptionCached: BuildProjectDescriptionAgent | undefined;
export function getBuildProjectDescriptionAgent(): BuildProjectDescriptionAgent { if (!buildProjectDescriptionCached) buildProjectDescriptionCached = new BuildProjectDescriptionAgent(); return buildProjectDescriptionCached; }
export function resetBuildProjectDescriptionAgentForTests(): void { buildProjectDescriptionCached = undefined; }
