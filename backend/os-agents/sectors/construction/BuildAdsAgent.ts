...
export type BuildAdsAgentDeps = { llm?: ILlmClient };
export class BuildAdsAgent {
  constructor(private readonly deps: BuildAdsAgentDeps = {}) {}
  ...
  async run(userId: string, input: BuildInput): Promise<BuildOutput> {
    ...
    const loc = enriched.location ? ` Segmentación geo: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Paid media obra "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, tono ${enriched.tone}).${loc}
- Google Search proyectos viviendas reformas industriales ejemplo grupos anuncios.
- Meta carousel proyectos terminados presupuesto consulta disclaimers obra.
- Retargeting visitantes portfolio límites frecuencia.
- Display branding zona acotada creatividades formatos típicos.
- LinkedIn B2B locales naves oficinas lead form mensaje ejecutivo.`;
    const __crmOut = { agentId: "build-ads", result: await this.llm.complete(prompt, buildLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let buildAdsCached: BuildAdsAgent | undefined;
export function getBuildAdsAgent(): BuildAdsAgent { if (!buildAdsCached) buildAdsCached = new BuildAdsAgent(); return buildAdsCached; }
export function resetBuildAdsAgentForTests(): void { buildAdsCached = undefined; }
