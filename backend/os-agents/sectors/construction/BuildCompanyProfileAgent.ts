...
export type BuildCompanyProfileAgentDeps = { llm?: ILlmClient };
export class BuildCompanyProfileAgent {
  constructor(private readonly deps: BuildCompanyProfileAgentDeps = {}) {}
  ...
  async run(userId: string, input: BuildInput): Promise<BuildOutput> {
    ...
    const loc = enriched.location ? ` Zona actuación: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Perfil empresa construcción/arquitectura "${enriched.businessName}" (tipo: ${enriched.serviceType}, cliente objetivo ${enriched.targetClient}, tono ${enriched.tone}).${loc}
- Presentación experiencia años especialización sin inflar.
- Portfolio proyectos narrativos con placeholders [OBRA].
- Certificaciones ISO colegios homologaciones [VERIFICABLES].
- Diferenciadores: garantía project management obra llave ingeniería integrada según aplique solo hechos ciertos.
- Google My Business, web Quiénes somos, fichas Habitissimo Certicalia Houzz.
No prometer plazos o precios concretos sin datos.`;
    const __crmOut = { agentId: "build-company-profile", result: await this.llm.complete(prompt, buildLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let buildCompanyProfileCached: BuildCompanyProfileAgent | undefined;
export function getBuildCompanyProfileAgent(): BuildCompanyProfileAgent { if (!buildCompanyProfileCached) buildCompanyProfileCached = new BuildCompanyProfileAgent(); return buildCompanyProfileCached; }
export function resetBuildCompanyProfileAgentForTests(): void { buildCompanyProfileCached = undefined; }
