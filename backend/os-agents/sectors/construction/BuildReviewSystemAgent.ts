...
export type BuildReviewSystemAgentDeps = { llm?: ILlmClient };
export class BuildReviewSystemAgent {
  constructor(private readonly deps: BuildReviewSystemAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Reseñas construcción "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}${enriched.location ? ", " + enriched.location : ""}):
- Mensaje post-entrega email WA Maps invitación reseña con foto resultado permiso obra.
- Guía páginas usuario pasos rápidos Maps.
- Plantillas positivo negativo incidencia resuelta explicación proceso sin datos contractuales sensibles públicos.
- Objetivo reseñas mensuales sugerido tamaño equipo.
- Puertos Google Habitissimo Houzz consistencia marca.
Tono ${enriched.tone}.`;
    const __crmOut = { agentId: "build-review-system", result: await this.llm.complete(prompt, buildLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let buildReviewSystemCached: BuildReviewSystemAgent | undefined;
export function getBuildReviewSystemAgent(): BuildReviewSystemAgent { if (!buildReviewSystemCached) buildReviewSystemCached = new BuildReviewSystemAgent(); return buildReviewSystemCached; }
export function resetBuildReviewSystemAgentForTests(): void { buildReviewSystemCached = undefined; }
