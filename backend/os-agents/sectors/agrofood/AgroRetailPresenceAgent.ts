...
export type AgroRetailPresenceAgentDeps = { llm?: ILlmClient };
export class AgroRetailPresenceAgent {
  constructor(private readonly deps: AgroRetailPresenceAgentDeps = {}) {}
  ...
  async run(userId: string, input: AgroInput): Promise<AgroOutput> {
    ...
    const ori = enriched.origin ? ` Procedencia shelf talker: ${enriched.origin}.` : "";
...
### BRIEF OPERATIVO
Presencia retail agro "${enriched.businessName}" (${enriched.productType}, ${enriched.targetMarket}, tono ${enriched.tone}).${ori}
- Argumentario lineal supermercado rotación margen deal marketing trade spend placeholder.
- Ficha central compras formato precio PVP sugerido márgenes sin datos inventados.
- Merchandising posición góndola PLV degustaciones compliance higiene.
- Programa sell-out promos bundles temporales acuerdo legal tienda.
- Estrategia delicatessen independiente storytelling muestra demostración chef canal HORECA cruce.`;
    const __crmOut = { agentId: "agro-retail-presence", result: await this.llm.complete(prompt, agroLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let agroRetailPresenceCached: AgroRetailPresenceAgent | undefined;
export function getAgroRetailPresenceAgent(): AgroRetailPresenceAgent {
  if (!agroRetailPresenceCached) agroRetailPresenceCached = new AgroRetailPresenceAgent();
  return agroRetailPresenceCached;
}
export function resetAgroRetailPresenceAgentForTests(): void { agroRetailPresenceCached = undefined; }
