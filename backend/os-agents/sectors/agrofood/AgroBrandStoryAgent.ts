...
export type AgroBrandStoryAgentDeps = { llm?: ILlmClient };
export class AgroBrandStoryAgent {
  constructor(private readonly deps: AgroBrandStoryAgentDeps = {}) {}
  ...
  async run(userId: string, input: AgroInput): Promise<AgroOutput> {
    ...
    const ori = enriched.origin ? ` Territorio: ${enriched.origin}.` : "";
...
### BRIEF OPERATIVO
Historia marca agroalimentaria "${enriched.businessName}" (producto ${enriched.productType}, mercado ${enriched.targetMarket}, voz ${enriched.tone}).${ori}
- Origen familia o empresa raíces territorio placeholders verificables.
- Conexión emocional producto lugar diferenciadores vs industrial local artesanía trazabilidad.
- Valores sostenibilidad medio ambiente sin greenwashing acciones auditable.
- Generaciones conocimiento manifiesto breve marca.
- Tagline principal + 3 variantes tono coherentes canal B2C B2B export.`;
    const __crmOut = { agentId: "agro-brand-story", result: await this.llm.complete(prompt, agroLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let agroBrandStoryCached: AgroBrandStoryAgent | undefined;
export function getAgroBrandStoryAgent(): AgroBrandStoryAgent {
  if (!agroBrandStoryCached) agroBrandStoryCached = new AgroBrandStoryAgent();
  return agroBrandStoryCached;
}
export function resetAgroBrandStoryAgentForTests(): void { agroBrandStoryCached = undefined; }
