...
export type AgroDigitalMarketingAgentDeps = { llm?: ILlmClient };
export class AgroDigitalMarketingAgent {
  constructor(private readonly deps: AgroDigitalMarketingAgentDeps = {}) {}
  ...
  async run(userId: string, input: AgroInput): Promise<AgroOutput> {
    ...
    const ori = enriched.origin ? ` Storytelling origen: ${enriched.origin}.` : "";
...
### BRIEF OPERATIVO
Marketing digital agro "${enriched.businessName}" (${enriched.productType}, mercado ${enriched.targetMarket}, ${enriched.tone}).${ori}
- D2C vs marketplaces Shopify WooCommerce ejemplo costes fulfillment sugeridos.
- 20 contenidos IG TikTok campo recetas tradición transparencia.
- Campaña email base propia segmentación KPI RGPD placeholders.
- Ficha PDP tienda bullets SEO FAQs alérgenos nota revisión etiquetado.
- Meta ads gourmet targeting intereses disclaimers país edad alcohol si vino.
- Marketplaces Gourmeto Delinat Amazon Fresh onboarding resumen ejemplo.`;
    const __crmOut = { agentId: "agro-digital-marketing", result: await this.llm.complete(prompt, agroLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let agroDigitalMarketingCached: AgroDigitalMarketingAgent | undefined;
export function getAgroDigitalMarketingAgent(): AgroDigitalMarketingAgent {
  if (!agroDigitalMarketingCached) agroDigitalMarketingCached = new AgroDigitalMarketingAgent();
  return agroDigitalMarketingCached;
}
export function resetAgroDigitalMarketingAgentForTests(): void { agroDigitalMarketingCached = undefined; }
