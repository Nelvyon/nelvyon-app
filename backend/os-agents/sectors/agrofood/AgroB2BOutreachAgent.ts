...
export type AgroB2BOutreachAgentDeps = { llm?: ILlmClient };
export class AgroB2BOutreachAgent {
  constructor(private readonly deps: AgroB2BOutreachAgentDeps = {}) {}
  ...
  async run(userId: string, input: AgroInput): Promise<AgroOutput> {
    ...
    const ori = enriched.origin ? ` Procedencia mencionada: ${enriched.origin}.` : "";
...
### BRIEF OPERATIVO
Outreach B2B agroalimentario "${enriched.businessName}" (${enriched.productType}, ${enriched.targetMarket}, tono ${enriched.tone}).${ori}
- Email compradores retail grandes superficies asunto bullets RGPD ejemplo.
- Propuesta distribuidores márgenes exclusividad apoyo PLV disclaimers financieros.
- Deck purchasing central compras formato rotación SKU placeholders.
- LinkedIn category managers compradores outline mensajes invitación.
- Argumentario ferias Alimentaria Fruit FITUR gastronomía objetivos reuniones leads.
- Propuesta restaurantes hostelería muestra degustaciones términos logística cadena frío si aplica.`;
    const __crmOut = { agentId: "agro-b2b-outreach", result: await this.llm.complete(prompt, agroLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let agroB2BOutreachCached: AgroB2BOutreachAgent | undefined;
export function getAgroB2BOutreachAgent(): AgroB2BOutreachAgent {
  if (!agroB2BOutreachCached) agroB2BOutreachCached = new AgroB2BOutreachAgent();
  return agroB2BOutreachCached;
}
export function resetAgroB2BOutreachAgentForTests(): void { agroB2BOutreachCached = undefined; }
