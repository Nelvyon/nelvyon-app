...
export interface PersonalizacionMasivaInput {
  userId: string;
  sector: string;
  brand: string;
...
export interface PersonalizacionMasivaOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parsePersonalizacionMasivaLlmJson(
  raw: string,
  label: string,
): Omit<PersonalizacionMasivaOutput, "agentId"> {
...
export function buildPersonalizacionMasivaPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: PersonalizacionMasivaInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const personalizationCtx = params.input.personalizationBrief?.trim()
    ? params.input.personalizationBrief.trim()
...
PERSONALIZACIÓN MASIVA NELVYON OS (v1):
- **Prefijo agentes**: **PersonalizacionMasiva**; personalización 1-a-1 a escala de millones.
- **Web <100ms**; email **>20 variables**; uplift conversión **+35%** vs genérico.
- **Segmentación dinámica** en tiempo real; **0%** contenido idéntico entre contactos distintos.
...
- Personalización / contexto: ${personalizationCtx}
...
export async function runPersonalizacionMasivaAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: PersonalizacionMasivaInput,
  temperature: number,
): Promise<PersonalizacionMasivaOutput> {
  const prompt = buildPersonalizacionMasivaPrompt({
  ...
  const parsed = parsePersonalizacionMasivaLlmJson(raw, agentId);
  const out: PersonalizacionMasivaOutput = { agentId, ...parsed };
  try {
...
export function getDefaultPersonalizacionMasivaLlm(): ILlmClient {
