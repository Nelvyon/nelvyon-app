...
export function parseNewsletterLlmJson(raw: string, label: string): Omit<NewsletterOutput, "agentId"> {
...
export function buildNewsletterPrompt(params: {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const newsletterCtx = params.input.newsletterBrief?.trim() ? params.input.newsletterBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

NEWSLETTER + MONETIZACIÓN AUDIENCIA NELVYON OS (v1):
- **Prefijo agentes**: **Newsletter**; newsletter y monetización de audiencia.
- **Open rate >45%**; **CTR >8%**; **revenue/email >0.50€**; personalización **100%**.
- **Monetización automática**; crecimiento lista **+20%** mensual.
...
- Newsletter / contexto: ${newsletterCtx}
...
export async function runNewsletterAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  const parsed = parseNewsletterLlmJson(raw, agentId);
  const out: NewsletterOutput = { agentId, ...parsed };
  try {
...
export function getDefaultNewsletterLlm(): ILlmClient {
