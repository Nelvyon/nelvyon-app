...
export type HealthAppointmentNurturingAgentDeps = { llm?: ILlmClient };
export class HealthAppointmentNurturingAgent {
  constructor(private readonly deps: HealthAppointmentNurturingAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea nurturing para convertir consultas en citas en ${enriched.clinicName}: respuesta inmediata web/WhatsApp, seguimiento 24/48/72h, email informativo del procedimiento con testimonios y precio aproximado, oferta de primera visita y script de recepción telefónica.`;
    const __crmOut = { agentId: "health-appointment-nurturing", result: await this.llm.complete(prompt, healthLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let healthAppointmentNurturingCached: HealthAppointmentNurturingAgent | undefined;
export function getHealthAppointmentNurturingAgent(): HealthAppointmentNurturingAgent { if (!healthAppointmentNurturingCached) healthAppointmentNurturingCached = new HealthAppointmentNurturingAgent(); return healthAppointmentNurturingCached; }
export function resetHealthAppointmentNurturingAgentForTests(): void { healthAppointmentNurturingCached = undefined; }
