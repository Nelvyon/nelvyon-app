...
export type HealthCrisisCommsAgentDeps = { llm?: ILlmClient };
export class HealthCrisisCommsAgent {
  constructor(private readonly deps: HealthCrisisCommsAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera protocolo de comunicación de crisis para ${enriched.clinicName}: respuesta a reseña negativa sin vulnerar LOPD, comunicado interno, protocolo ante reclamación, disculpa pública si procede, recuperación reputacional y FAQ interna.`;
    const __crmOut = { agentId: "health-crisis-comms", result: await this.llm.complete(prompt, healthLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let healthCrisisCommsCached: HealthCrisisCommsAgent | undefined;
export function getHealthCrisisCommsAgent(): HealthCrisisCommsAgent { if (!healthCrisisCommsCached) healthCrisisCommsCached = new HealthCrisisCommsAgent(); return healthCrisisCommsCached; }
export function resetHealthCrisisCommsAgentForTests(): void { healthCrisisCommsCached = undefined; }
