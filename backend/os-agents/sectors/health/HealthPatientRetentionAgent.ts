...
export type HealthPatientRetentionAgentDeps = { llm?: ILlmClient };
export class HealthPatientRetentionAgent {
  constructor(private readonly deps: HealthPatientRetentionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Diseña estrategia de retención/fidelización para ${enriched.clinicName}: calendario revisiones por especialidad, email cumpleaños, programa VIP, encuesta satisfacción, upsell ético y comunicación de nuevos servicios.`;
    const __crmOut = { agentId: "health-patient-retention", result: await this.llm.complete(prompt, healthLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let healthPatientRetentionCached: HealthPatientRetentionAgent | undefined;
export function getHealthPatientRetentionAgent(): HealthPatientRetentionAgent { if (!healthPatientRetentionCached) healthPatientRetentionCached = new HealthPatientRetentionAgent(); return healthPatientRetentionCached; }
export function resetHealthPatientRetentionAgentForTests(): void { healthPatientRetentionCached = undefined; }
