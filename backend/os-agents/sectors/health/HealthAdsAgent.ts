...
export type HealthAdsAgentDeps = { llm?: ILlmClient };
export class HealthAdsAgent {
  constructor(private readonly deps: HealthAdsAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea campañas paid media para ${enriched.clinicName}: Google Search alta intención, Meta para servicios preventivos/estéticos, copy conforme normativa sanitaria española y retargeting para presupuestos no convertidos.`;
    const __crmOut = { agentId: "health-ads", result: await this.llm.complete(prompt, healthLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let healthAdsCached: HealthAdsAgent | undefined;
export function getHealthAdsAgent(): HealthAdsAgent { if (!healthAdsCached) healthAdsCached = new HealthAdsAgent(); return healthAdsCached; }
export function resetHealthAdsAgentForTests(): void { healthAdsCached = undefined; }
