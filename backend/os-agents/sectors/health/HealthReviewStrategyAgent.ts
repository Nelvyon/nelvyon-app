...
export type HealthReviewStrategyAgentDeps = { llm?: ILlmClient };
export class HealthReviewStrategyAgent {
  constructor(private readonly deps: HealthReviewStrategyAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Diseña sistema de reseñas para ${enriched.clinicName}: solicitud post-consulta por SMS/WhatsApp/email, guía para reseñar, respuestas a reseñas positivas/negativas respetando LOPD, manejo de crisis de reputación y objetivos mensuales de reviews.`;
    const __crmOut = { agentId: "health-review-strategy", result: await this.llm.complete(prompt, healthLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let healthReviewStrategyCached: HealthReviewStrategyAgent | undefined;
export function getHealthReviewStrategyAgent(): HealthReviewStrategyAgent { if (!healthReviewStrategyCached) healthReviewStrategyCached = new HealthReviewStrategyAgent(); return healthReviewStrategyCached; }
export function resetHealthReviewStrategyAgentForTests(): void { healthReviewStrategyCached = undefined; }
