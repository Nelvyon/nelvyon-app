...
export type HealthClinicProfileAgentDeps = { llm?: ILlmClient };
export class HealthClinicProfileAgent {
  constructor(private readonly deps: HealthClinicProfileAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera perfil profesional completo para ${enriched.clinicName} (${enriched.specialty}) en ${enriched.location ?? "España"}: misión, valores, bio profesional, servicios, diferenciadores reales, texto GMB, Doctoralia/Top Doctors y página Quiénes somos.`;
    const __crmOut = { agentId: "health-clinic-profile", result: await this.llm.complete(prompt, healthLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let healthClinicProfileCached: HealthClinicProfileAgent | undefined;
export function getHealthClinicProfileAgent(): HealthClinicProfileAgent { if (!healthClinicProfileCached) healthClinicProfileCached = new HealthClinicProfileAgent(); return healthClinicProfileCached; }
export function resetHealthClinicProfileAgentForTests(): void { healthClinicProfileCached = undefined; }
