...
export type HealthReferralAgentDeps = { llm?: ILlmClient };
export class HealthReferralAgent {
  constructor(private readonly deps: HealthReferralAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea programa de referidos para ${enriched.clinicName}: mensajes para pacientes satisfechos, incentivos, seguimiento post-tratamiento, colaboración entre especialistas y convenios con empresas.`;
    const __crmOut = { agentId: "health-referral", result: await this.llm.complete(prompt, healthLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let healthReferralCached: HealthReferralAgent | undefined;
export function getHealthReferralAgent(): HealthReferralAgent { if (!healthReferralCached) healthReferralCached = new HealthReferralAgent(); return healthReferralCached; }
export function resetHealthReferralAgentForTests(): void { healthReferralCached = undefined; }
