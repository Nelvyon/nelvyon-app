...
export type HealthContentMarketingAgentDeps = { llm?: ILlmClient };
export class HealthContentMarketingAgent {
  constructor(private readonly deps: HealthContentMarketingAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Diseña estrategia de contenido médico para ${enriched.clinicName}: 20 posts RRSS, artículo 1200 palabras, guion vídeo explicativo, FAQ 10 preguntas, infografía textual y política de contenido cumpliendo normativa sanitaria española.`;
    const __crmOut = { agentId: "health-content-marketing", result: await this.llm.complete(prompt, healthLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let healthContentMarketingCached: HealthContentMarketingAgent | undefined;
export function getHealthContentMarketingAgent(): HealthContentMarketingAgent { if (!healthContentMarketingCached) healthContentMarketingCached = new HealthContentMarketingAgent(); return healthContentMarketingCached; }
export function resetHealthContentMarketingAgentForTests(): void { healthContentMarketingCached = undefined; }
