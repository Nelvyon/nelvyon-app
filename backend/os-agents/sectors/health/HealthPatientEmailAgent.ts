...
export type HealthPatientEmailAgentDeps = { llm?: ILlmClient };
export class HealthPatientEmailAgent {
  constructor(private readonly deps: HealthPatientEmailAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea secuencia email para pacientes de ${enriched.clinicName}: bienvenida, recordatorio cita, post-consulta, revisión periódica, reactivación y newsletter mensual de salud (${enriched.specialty}).`;
    const __crmOut = { agentId: "health-patient-email", result: await this.llm.complete(prompt, healthLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let healthPatientEmailCached: HealthPatientEmailAgent | undefined;
export function getHealthPatientEmailAgent(): HealthPatientEmailAgent { if (!healthPatientEmailCached) healthPatientEmailCached = new HealthPatientEmailAgent(); return healthPatientEmailCached; }
export function resetHealthPatientEmailAgentForTests(): void { healthPatientEmailCached = undefined; }
