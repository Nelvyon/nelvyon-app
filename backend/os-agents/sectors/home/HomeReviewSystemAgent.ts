...
export type HomeReviewSystemAgentDeps = { llm?: ILlmClient };
export class HomeReviewSystemAgent {
  constructor(private readonly deps: HomeReviewSystemAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Gestión de reseñas para "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetArea}):
- Mensaje post-servicio Google vía WhatsApp + email (corto, amable).
- Mini guía cliente: pasos para dejar reseña en Maps.
- Plantillas respuesta reseñas positivas y negativas (profesional, empático).
- Estrategia ante reseñas injustas o falsas (canal privado primero).
- KPI: objetivo reseñas mensuales sugerido por servicio/zona.
- Presencia Trustpilot y Habitissimo como capa opcional de reputación.
Cumplimiento RGPD en solicitudes.`;
    const __crmOut = { agentId: "home-review-system", result: await this.llm.complete(prompt, homeLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let homeReviewSystemCached: HomeReviewSystemAgent | undefined;
export function getHomeReviewSystemAgent(): HomeReviewSystemAgent { if (!homeReviewSystemCached) homeReviewSystemCached = new HomeReviewSystemAgent(); return homeReviewSystemCached; }
export function resetHomeReviewSystemAgentForTests(): void { homeReviewSystemCached = undefined; }
