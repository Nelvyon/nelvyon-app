...
export type HomeClientRetentionAgentDeps = { llm?: ILlmClient };
export class HomeClientRetentionAgent {
  constructor(private readonly deps: HomeClientRetentionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Retención y fidelización para "${enriched.businessName}" (${enriched.serviceType}, clientes ${enriched.targetArea}, tono ${enriched.tone}):
- Follow-up WhatsApp/email post-servicio (satisfacción NPS corto).
- Recordatorios mantenimiento preventivo por tipo: caldera anual, desatascos, revisión eléctrica periódica (marcos legales genéricos).
- Programa cliente habitual: escalones descuento cumplimiento condiciones claras.
- Newsletter trimestral: 4 bloques consejos hogar adaptable al servicio.
- Referido vecinos: mensaje dual descuento (ético, sin spam).
Plantillas cortas listas para enviar.`;
    const __crmOut = { agentId: "home-client-retention", result: await this.llm.complete(prompt, homeLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let homeClientRetentionCached: HomeClientRetentionAgent | undefined;
export function getHomeClientRetentionAgent(): HomeClientRetentionAgent { if (!homeClientRetentionCached) homeClientRetentionCached = new HomeClientRetentionAgent(); return homeClientRetentionCached; }
export function resetHomeClientRetentionAgentForTests(): void { homeClientRetentionCached = undefined; }
