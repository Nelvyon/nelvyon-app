...
export type HomeBeforeAfterContentAgentDeps = { llm?: ILlmClient };
export class HomeBeforeAfterContentAgent {
  constructor(private readonly deps: HomeBeforeAfterContentAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Contenido antes/después RRSS para "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetArea}, tono ${enriched.tone}):
- 20 ideas de posts con encuadre foto antes/después.
- Captions Instagram/Facebook: problema → solución → resultado + CTA.
- Ideas Reels/TikTok: gancho primeros 3s, proceso trabajo, música/texto overlay sugerido.
- Hashtags por servicio y ciudad (mix local + oficio).
- WhatsApp Business: mensaje modelo para enviar portfolio a lead frío (consentimiento datos).
Incluye nota uso de obra real y permisos imagen cliente.`;
    const __crmOut = { agentId: "home-before-after-content", result: await this.llm.complete(prompt, homeLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let homeBeforeAfterContentCached: HomeBeforeAfterContentAgent | undefined;
export function getHomeBeforeAfterContentAgent(): HomeBeforeAfterContentAgent { if (!homeBeforeAfterContentCached) homeBeforeAfterContentCached = new HomeBeforeAfterContentAgent(); return homeBeforeAfterContentCached; }
export function resetHomeBeforeAfterContentAgentForTests(): void { homeBeforeAfterContentCached = undefined; }
