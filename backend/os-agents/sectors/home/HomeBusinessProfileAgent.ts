...
export type HomeBusinessProfileAgentDeps = { llm?: ILlmClient };
export class HomeBusinessProfileAgent {
  constructor(private readonly deps: HomeBusinessProfileAgentDeps = {}) {}
  ...
  async run(userId: string, input: HomeInput): Promise<HomeOutput> {
    ...
    const u = enriched.urgency ? ` Urgencia/disponibilidad: ${enriched.urgency}.` : "";
...
### BRIEF OPERATIVO
Perfil profesional para empresa de servicios del hogar "${enriched.businessName}" (servicio: ${enriched.serviceType}, zona: ${enriched.targetArea}, tono: ${enriched.tone}).${u}
- Presentación con experiencia y zona de cobertura (placeholders si faltan datos).
- Servicios detallados con precios orientativos o "presupuesto sin compromiso".
- Credenciales: instalador autorizado, homologaciones, seguros (estructura genérica).
- Diferenciadores: 24h, garantía trabajos, RC, tiempos de respuesta.
- Texto Google My Business y página "Quiénes somos" web.
No inventes certificaciones concretas sin dato; usa [COMPLETAR].`;
    const __crmOut = { agentId: "home-business-profile", result: await this.llm.complete(prompt, homeLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let homeBusinessProfileCached: HomeBusinessProfileAgent | undefined;
export function getHomeBusinessProfileAgent(): HomeBusinessProfileAgent { if (!homeBusinessProfileCached) homeBusinessProfileCached = new HomeBusinessProfileAgent(); return homeBusinessProfileCached; }
export function resetHomeBusinessProfileAgentForTests(): void { homeBusinessProfileCached = undefined; }
