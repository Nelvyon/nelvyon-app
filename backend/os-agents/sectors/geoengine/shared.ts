...
export function parseGeoEngineLlmJson(raw: string, label: string): Omit<GeoEngineOutput, "agentId"> {
...
export function buildGeoEnginePrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const geo = params.input.geoBrief?.trim() ? params.input.geoBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

GEO ENGINE NELVYON OS (v1):
- **Prefijo agentes**: **GeoEngine**; geolocalización y targeting geográfico.
- **Mercados prioritarios**: **España, México, Colombia, Argentina, Chile, Perú, EE.UU. (hispano), Brasil**.
- **SEO local**: ficha **Google My Business** optimizada, **schema LocalBusiness**, **NAP** consistente.
- **Pricing por poder adquisitivo**: España **100%** / México **60%** / Colombia **50%** / Argentina **40%** / Brasil **55%**.
- **Contenido localizado**: no solo traducción — **referencias culturales**, **festividades**, **términos locales** por país.
- **Targeting**: segmentos geográficos por zona; heatmaps de actividad; competidores locales por región.
...
export async function runGeoEngineAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  const prompt = buildGeoEnginePrompt({
  ...
  const parsed = parseGeoEngineLlmJson(raw, agentId);
  const out: GeoEngineOutput = { agentId, ...parsed };
  try {
...
export function getDefaultGeoEngineLlm(): ILlmClient {
