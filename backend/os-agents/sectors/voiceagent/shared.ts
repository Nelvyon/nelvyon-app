...
export function parseVoiceAgentLlmJson(raw: string, label: string): Omit<VoiceAgentOutput, "agentId"> {
...
export function buildVoiceAgentPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const voice = params.input.voiceBrief?.trim() ? params.input.voiceBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

VOICE AGENT NELVYON OS (v1):
- **Prefijo agentes**: **VoiceAgent**; llamadas y voz automáticas B2B/B2C.
- **Duración óptima**: **3-7 minutos**; **>10 min** → baja conversión.
- **Sentiment en vivo**: **positivo / neutral / negativo** durante la llamada.
- **Objeciones auto**: **precio, tiempo, competidor, no necesito** → respuesta automática por tipo.
- **GDPR**: **consentimiento grabación** obligatorio al inicio; **opt-out inmediato** si lo solicita el contacto.
- **Horario**: solo **09:00-20:00** hora local del contacto.
- **Post-llamada**: transcripción + diarization, resumen, action items, CRM y follow-up email.
...
export async function runVoiceAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  const parsed = parseVoiceAgentLlmJson(raw, agentId);
  const out: VoiceAgentOutput = { agentId, ...parsed };
  try {
...
export function getDefaultVoiceAgentLlm(): ILlmClient {
