...
export type PharmacyReviewSystemAgentDeps = { llm?: ILlmClient };
export class PharmacyReviewSystemAgent {
  constructor(private readonly deps: PharmacyReviewSystemAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Reseñas farmacia "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}):
- Solicitud post-visita WhatsApp corto ticket QR Maps sin mencionar tratamientos ni medicación.
- Plantillas positivo / negativo neutro profesional sin revelar datos salud RGPD especial categoría datos salud.
- Respuesta críticas roturas stock referencias rupturas nacional explicación prudente.
- Objetivo reseñas mensuales sugerencia por ubicación ${enriched.location ?? "local"}.
- Segmentar feedback consejo rapidez precio comunicación.`;
    const __crmOut = { agentId: "pharmacy-review-system", result: await this.llm.complete(prompt, pharmacyLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let pharmacyReviewSystemCached: PharmacyReviewSystemAgent | undefined;
export function getPharmacyReviewSystemAgent(): PharmacyReviewSystemAgent { if (!pharmacyReviewSystemCached) pharmacyReviewSystemCached = new PharmacyReviewSystemAgent(); return pharmacyReviewSystemCached; }
export function resetPharmacyReviewSystemAgentForTests(): void { pharmacyReviewSystemCached = undefined; }
