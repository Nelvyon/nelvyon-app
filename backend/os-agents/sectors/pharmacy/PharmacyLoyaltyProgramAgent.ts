...
export type PharmacyLoyaltyProgramAgentDeps = { llm?: ILlmClient };
export class PharmacyLoyaltyProgramAgent {
  constructor(private readonly deps: PharmacyLoyaltyProgramAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Programa fidelización farmacia "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}, ${enriched.tone}):
- Reglas puntos € compra → puntos → canje descuentos parafarmacia revisar normativa promociones.
- Niveles ocasional/habitual/premium beneficios ejemplos compliance publicitario.
- Comunicaciones email WhatsApp alta programa y estado puntos RGPD minimización.
- Tarjeta digital vs física flujo adhesión kiosk app partner placeholder.
- KPI redención frecuencia ticket medio dashboards sugeridos.
- Integraciones software gestión farmacia campos técnicos genéricos [ERP].`;
    const __crmOut = { agentId: "pharmacy-loyalty-program", result: await this.llm.complete(prompt, pharmacyLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let pharmacyLoyaltyProgramCached: PharmacyLoyaltyProgramAgent | undefined;
export function getPharmacyLoyaltyProgramAgent(): PharmacyLoyaltyProgramAgent { if (!pharmacyLoyaltyProgramCached) pharmacyLoyaltyProgramCached = new PharmacyLoyaltyProgramAgent(); return pharmacyLoyaltyProgramCached; }
export function resetPharmacyLoyaltyProgramAgentForTests(): void { pharmacyLoyaltyProgramCached = undefined; }
