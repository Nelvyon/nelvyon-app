...
export type PharmacyEmailCampaignAgentDeps = { llm?: ILlmClient };
export class PharmacyEmailCampaignAgent {
  constructor(private readonly deps: PharmacyEmailCampaignAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Email fidelización "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}, ${enriched.tone}):
- Bienvenida nuevos clientes servicios farmacia sin prometer resultados clínicos.
- Newsletter mensual bloques salud novedades oferta genérica legal.
- Recordatorio adherencia medicación crónica servicio modelo (consentimiento explícito datos salud RGPD).
- Vacunación recordatorio plantilla neutra CTA cita presencial placeholder.
- Puntos programa saldo recompensas sin condicionar tratamiento médico.
- Win-back +3 meses tono cercano voucher revisable.`;
    const __crmOut = { agentId: "pharmacy-email-campaign", result: await this.llm.complete(prompt, pharmacyLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let pharmacyEmailCampaignCached: PharmacyEmailCampaignAgent | undefined;
export function getPharmacyEmailCampaignAgent(): PharmacyEmailCampaignAgent { if (!pharmacyEmailCampaignCached) pharmacyEmailCampaignCached = new PharmacyEmailCampaignAgent(); return pharmacyEmailCampaignCached; }
export function resetPharmacyEmailCampaignAgentForTests(): void { pharmacyEmailCampaignCached = undefined; }
