...
export type PharmacyLocalSEOAgentDeps = { llm?: ILlmClient };
export class PharmacyLocalSEOAgent {
  constructor(private readonly deps: PharmacyLocalSEOAgentDeps = {}) {}
...
### BRIEF OPERATIVO
SEO local farmacia "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}, ${enriched.tone}, área ${loc}):
- Keywords servicio ciudad farmacia guardia parafarmacia online dermo ejemplos.
- GMB: horarios guardia atributos fotos interiores sin pacientes reconocibles.
- Meta titles/descriptions páginas categoría parafarmacia OTC cosmética.
- JSON-LD Pharmacy / DrugStore placeholder.
- Estrategia reseñas Google ética LOPDGDD datos salud indirectos.
- Portales Doctoralia Sanitas mapas farmacias presencia.`;
    const __crmOut = { agentId: "pharmacy-local-seo", result: await this.llm.complete(prompt, pharmacyLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let pharmacyLocalSEOCached: PharmacyLocalSEOAgent | undefined;
export function getPharmacyLocalSEOAgent(): PharmacyLocalSEOAgent { if (!pharmacyLocalSEOCached) pharmacyLocalSEOCached = new PharmacyLocalSEOAgent(); return pharmacyLocalSEOCached; }
export function resetPharmacyLocalSEOAgentForTests(): void { pharmacyLocalSEOCached = undefined; }
