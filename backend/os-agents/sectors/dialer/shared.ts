...
export function parseDialerLlmJson(raw: string, label: string): Omit<DialerOutput, "agentId"> {
...
export function buildDialerPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const dialerCtx = params.input.dialerBrief?.trim() ? params.input.dialerBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

DIALER NATIVO SALES NELVYON OS (v1):
- **Prefijo agentes**: **Dialer**; marcación y ventas por teléfono.
- **Connect rate >35%** (industria 18%); **script personalizado <3 s** antes de llamada.
- **Transcripción RT <1 s** latencia; **CRM update post-llamada <30 s** automático.
- **Compliance grabación 195 países** automático; **0 tareas manuales** post-llamada.
...
export async function runDialerAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: DialerInput,
  temperature: number,
): Promise<DialerOutput> {
  const prompt = buildDialerPrompt({
  ...
  const parsed = parseDialerLlmJson(raw, agentId);
  const out: DialerOutput = { agentId, ...parsed };
  try {
...
export function getDefaultDialerLlm(): ILlmClient {
