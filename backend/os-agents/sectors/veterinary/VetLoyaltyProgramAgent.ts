...
export type VetLoyaltyProgramAgentDeps = { llm?: ILlmClient };
export class VetLoyaltyProgramAgent {
  constructor(private readonly deps: VetLoyaltyProgramAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Fidelización clínica "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetPet}, tono ${enriched.tone}${enriched.location ? ", " + enriched.location : ""}):
- Diseño plan salud anual cuota revisión vacunas desparasitación ejemplo tablas placeholders precios revisables.
- Comparativa ahorro visitas sueltas método cálculo transparente.
- Email captación contrato consentimientos RGPD tratamiento datos titular animal.
- Recordatorios servicios incluidos automatización segura.
- Puntos tienda y clínica cruzados beneficio ejemplo legal publicidad profesional veterinaria.
- Referidos mutuos mensaje recomendación sin sobornos indebidos revisar ética colegio.`;
    const __crmOut = { agentId: "vet-loyalty-program", result: await this.llm.complete(prompt, vetLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let vetLoyaltyProgramCached: VetLoyaltyProgramAgent | undefined;
export function getVetLoyaltyProgramAgent(): VetLoyaltyProgramAgent { if (!vetLoyaltyProgramCached) vetLoyaltyProgramCached = new VetLoyaltyProgramAgent(); return vetLoyaltyProgramCached; }
export function resetVetLoyaltyProgramAgentForTests(): void { vetLoyaltyProgramCached = undefined; }
