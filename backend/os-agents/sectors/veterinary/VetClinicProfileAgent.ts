...
export type VetClinicProfileAgentDeps = { llm?: ILlmClient };
export class VetClinicProfileAgent {
  constructor(private readonly deps: VetClinicProfileAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Perfil profesional veterinaria/mascotas "${enriched.businessName}" (tipo: ${enriched.serviceType}, mascotas: ${enriched.targetPet}, tono ${enriched.tone}).${loc}
- Presentación especialización equipo veterinario o negocio con credenciales [COLEGIO].
- Servicios beneficios mascota y familia sin diagnósticos ni promesas curativas.
- Diferenciadores: urgencias 24h, diagnóstico por imagen, hospitalización [COMPLETAR según verdades].
- Google My Business, web "Quiénes somos", directorios sector veterinario.
Disclaimer: comunicación institucional, no consejo clínico remoto.`;
    const __crmOut = { agentId: "vet-clinic-profile", result: await this.llm.complete(prompt, vetLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let vetClinicProfileCached: VetClinicProfileAgent | undefined;
export function getVetClinicProfileAgent(): VetClinicProfileAgent { if (!vetClinicProfileCached) vetClinicProfileCached = new VetClinicProfileAgent(); return vetClinicProfileCached; }
export function resetVetClinicProfileAgentForTests(): void { vetClinicProfileCached = undefined; }
