...
export type VetAppointmentNurturingAgentDeps = { llm?: ILlmClient };
export class VetAppointmentNurturingAgent {
  constructor(private readonly deps: VetAppointmentNurturingAgentDeps = {}) {}
  ...
  async run(userId: string, input: VetInput): Promise<VetOutput> {
    ...
    const loc = enriched.location ? ` Región: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Nurturing citas "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetPet}, tono ${enriched.tone}).${loc}
- Respuesta inmediata web/WA info general + CTA reserva sin triaje clínico.
- Follow-up 24/48h si no reserva.
- Email preparación visita qué llevar transporte ayunas si aplica genérico.
- Recordatorio cita instrucciones llegada parking.
- Post-visita seguimiento cuidados generales sin detalles clínicos identificables en email masivo.
- Serie recordatorios vacunas desparasitación calendario orientativo cumplir RGPD titular mascota.`;
    const __crmOut = { agentId: "vet-appointment-nurturing", result: await this.llm.complete(prompt, vetLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let vetAppointmentNurturingCached: VetAppointmentNurturingAgent | undefined;
export function getVetAppointmentNurturingAgent(): VetAppointmentNurturingAgent { if (!vetAppointmentNurturingCached) vetAppointmentNurturingCached = new VetAppointmentNurturingAgent(); return vetAppointmentNurturingCached; }
export function resetVetAppointmentNurturingAgentForTests(): void { vetAppointmentNurturingCached = undefined; }
