...
export type VetLocalSEOAgentDeps = { llm?: ILlmClient };
export class VetLocalSEOAgent {
  constructor(private readonly deps: VetLocalSEOAgentDeps = {}) {}
  ...
  async run(userId: string, input: VetInput): Promise<VetOutput> {
    ...
    const loc = enriched.location ?? "ciudad principal";
...
### BRIEF OPERATIVO
SEO local veterinario "${enriched.businessName}" (${enriched.serviceType}, público ${enriched.targetPet}, ${enriched.tone}, ${loc}):
- Keywords servicio ciudad urgencias grooming tienda ejemplo.
- Ficha GMB fotos equipo instalaciones mascotas modelo con autorización uso imagen.
- Meta titles/descriptions servicios típicos.
- JSON-LD VeterinaryCare AnimalShelter PetStore según encaje negocio.
- Estrategia reseñas Google timings post-visita.
- Directorios especializados Doctoralia veterinaria y locales.`;
    const __crmOut = { agentId: "vet-local-seo", result: await this.llm.complete(prompt, vetLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let vetLocalSEOCached: VetLocalSEOAgent | undefined;
export function getVetLocalSEOAgent(): VetLocalSEOAgent { if (!vetLocalSEOCached) vetLocalSEOCached = new VetLocalSEOAgent(); return vetLocalSEOCached; }
export function resetVetLocalSEOAgentForTests(): void { vetLocalSEOCached = undefined; }
