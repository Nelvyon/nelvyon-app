...
export interface MembresiaCursosInput {
  userId: string;
  sector: string;
  brand: string;
  courseBrief?: string;
...
export interface MembresiaCursosOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseMembresiaCursosLlmJson(raw: string, label: string): Omit<MembresiaCursosOutput, "agentId"> {
...
export function buildMembresiaCursosPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: MembresiaCursosInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const courseCtx = params.input.courseBrief?.trim() ? params.input.courseBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

PÁGINAS MEMBRESÍA + CURSOS NELVYON OS (v1):
- **Prefijo agentes**: **MembresiaCursos**; membresía, cursos y comunidad.
- **Curso completo <10 min**; completion **>70%** vs 15% industria; certificados **<5s**.
- **Paddle** integrado; comunidad con moderación IA; analytics RT por alumno.
...
export async function runMembresiaCursosAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: MembresiaCursosInput,
  temperature: number,
): Promise<MembresiaCursosOutput> {
  const prompt = buildMembresiaCursosPrompt({
  ...
  const parsed = parseMembresiaCursosLlmJson(raw, agentId);
  const out: MembresiaCursosOutput = { agentId, ...parsed };
  try {
...
export function getDefaultMembresiaCursosLlm(): ILlmClient {
