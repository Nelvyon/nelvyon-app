...
export function parseHrTechLlmJson(raw: string, label: string): Omit<HrTechOutput, "agentId"> {
...
export function buildHrTechPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const hrCtx = params.input.hrTechBrief?.trim() ? params.input.hrTechBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

HR TECH NELVYON OS (v1):
- **Prefijo agentes**: **HrTech**; RR.HH. y talento.
- **Screening CV <2 min** por candidato; **time-to-hire - >50%** vs manual.
- **eNPS mensual automático** sin encuestador; **onboarding <1 día**.
- **Compliance laboral 195 países** automático; **turnover - >30%** con detección temprana fuga.
...
export async function runHrTechAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: HrTechInput,
  temperature: number,
): Promise<HrTechOutput> {
  const prompt = buildHrTechPrompt({
  ...
  const parsed = parseHrTechLlmJson(raw, agentId);
  const out: HrTechOutput = { agentId, ...parsed };
  try {
...
export function getDefaultHrTechLlm(): ILlmClient {
