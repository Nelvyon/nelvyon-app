...
export interface IntegracionesNativasInput {
  userId: string;
  sector: string;
  brand: string;
...
export interface IntegracionesNativasOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseIntegracionesNativasLlmJson(
  raw: string,
  label: string,
): Omit<IntegracionesNativasOutput, "agentId"> {
...
export function buildIntegracionesNativasPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: IntegracionesNativasInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const integrationCtx = params.input.integrationBrief?.trim()
    ? params.input.integrationBrief.trim()
...
INTEGRACIONES NATIVAS GA4+META+ADS+SHOPIFY NELVYON OS (v1):
- **Prefijo agentes**: **IntegracionesNativas**; sync nativo cross-plataforma.
- **Sync RT <30s**; **0%** pérdida eventos conversión; dedupe cross-plataforma automático.
- **Setup completo <5 min**; discrepancias **<1h**; **ROAS unificado** en tiempo real.
...
- Integraciones / contexto: ${integrationCtx}
...
export async function runIntegracionesNativasAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: IntegracionesNativasInput,
  temperature: number,
): Promise<IntegracionesNativasOutput> {
  const prompt = buildIntegracionesNativasPrompt({
  ...
  const parsed = parseIntegracionesNativasLlmJson(raw, agentId);
  const out: IntegracionesNativasOutput = { agentId, ...parsed };
  try {
...
export function getDefaultIntegracionesNativasLlm(): ILlmClient {
