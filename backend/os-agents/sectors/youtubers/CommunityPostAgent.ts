...
export type CommunityPostInput = { topic: string; objective: "engagement" | "awareness" | "conversion" };
export type CommunityPostResult = { posts: Array<{ text: string; cta: string; poll?: string[] }> };
export type CommunityPostAgentDeps = { llm?: ILlmClient };

export class CommunityPostAgent {
  constructor(private readonly deps: CommunityPostAgentDeps = {}) {}
  private get llm(): ILlmClient { return this.deps.llm ?? LlmClient.getInstance(); }
  async generatePosts(userId: string, input: CommunityPostInput): Promise<CommunityPostResult> {
...
### BRIEF OPERATIVO
Crea posts de Comunidad YouTube para máximo engagement.
Tema: ${enriched.topic}. Objetivo: ${enriched.objective}. Calidad top 1% creator economy.
Devuelve SOLO JSON: {"posts":[{"text":"","cta":"","poll":["",""]}]}
Genera 5 posts.`;
    return parseJson<CommunityPostResult>(await this.llm.complete(prompt, llmOpts(0.4)), "CommunityPostAgent");
  }
}
let cached: CommunityPostAgent | undefined;
export function getCommunityPostAgent(): CommunityPostAgent { if (!cached) cached = new CommunityPostAgent(); return cached; }
export function resetCommunityPostAgentForTests(): void { cached = undefined; }
