...
export type VideoSEOAuditInput = { title: string; description: string; tags: string[]; transcriptSummary: string };
export type VideoSEOAuditResult = {
  score: number;
  prioritizedImprovements: Array<{ area: string; issue: string; recommendation: string; priority: "high" | "medium" | "low" }>;
};
export type VideoSEOAuditAgentDeps = { llm?: ILlmClient };

export class VideoSEOAuditAgent {
  constructor(private readonly deps: VideoSEOAuditAgentDeps = {}) {}
  private get llm(): ILlmClient { return this.deps.llm ?? LlmClient.getInstance(); }
  async auditVideoSeo(userId: string, input: VideoSEOAuditInput): Promise<VideoSEOAuditResult> {
...
### BRIEF OPERATIVO
Audita SEO de vídeo YouTube para aumentar views orgánicas.
Título: ${enriched.title}
Descripción: ${enriched.description}
Tags: ${(enriched.tags ?? []).join(", ")}
Resumen: ${enriched.transcriptSummary}
Devuelve SOLO JSON: {"score":0,"prioritizedImprovements":[{"area":"","issue":"","recommendation":"","priority":"high"}]}`;
    return parseJson<VideoSEOAuditResult>(await this.llm.complete(prompt, llmOpts(0.2)), "VideoSEOAuditAgent");
  }
}
let cached: VideoSEOAuditAgent | undefined;
export function getVideoSEOAuditAgent(): VideoSEOAuditAgent { if (!cached) cached = new VideoSEOAuditAgent(); return cached; }
export function resetVideoSEOAuditAgentForTests(): void { cached = undefined; }
