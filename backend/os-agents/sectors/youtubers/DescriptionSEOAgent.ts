...
export type DescriptionSEOInput = { title: string; transcriptSummary: string; links?: string[] };
export type DescriptionSEOResult = { description: string; keywords: string[]; timestamps: string[]; hashtags: string[] };
export type DescriptionSEOAgentDeps = { llm?: ILlmClient };

export class DescriptionSEOAgent {
  constructor(private readonly deps: DescriptionSEOAgentDeps = {}) {}
  private get llm(): ILlmClient { return this.deps.llm ?? LlmClient.getInstance(); }
  async generateDescription(userId: string, input: DescriptionSEOInput): Promise<DescriptionSEOResult> {
...
### BRIEF OPERATIVO
Genera descripción SEO de YouTube de alto rendimiento.
Título: ${enriched.title}. Resumen: ${enriched.transcriptSummary}. Links: ${(enriched.links ?? []).join(", ")}.
Incluye keywords, timestamps, links y hashtags.
Devuelve SOLO JSON: {"description":"","keywords":[],"timestamps":[],"hashtags":[]}`;
    return parseJson<DescriptionSEOResult>(await this.llm.complete(prompt, llmOpts(0.2)), "DescriptionSEOAgent");
  }
}
let cached: DescriptionSEOAgent | undefined;
export function getDescriptionSEOAgent(): DescriptionSEOAgent { if (!cached) cached = new DescriptionSEOAgent(); return cached; }
export function resetDescriptionSEOAgentForTests(): void { cached = undefined; }
