...
export type SponsorshipEmailInput = { brandName: string; niche: string; channelMetrics: string };
export type SponsorshipEmailResult = { subject: string; body: string; estimatedBudgetRange: string };
export type SponsorshipEmailAgentDeps = { llm?: ILlmClient };

export class SponsorshipEmailAgent {
  constructor(private readonly deps: SponsorshipEmailAgentDeps = {}) {}
  private get llm(): ILlmClient { return this.deps.llm ?? LlmClient.getInstance(); }
  async generateSponsorshipEmail(userId: string, input: SponsorshipEmailInput): Promise<SponsorshipEmailResult> {
...
### BRIEF OPERATIVO
Escribe email de outreach profesional para sponsorship a marca.
Marca ${enriched.brandName}, nicho ${enriched.niche}, métricas canal ${enriched.channelMetrics}.
Incluye propuesta de valor, ideas de integración y rango de presupuesto estimado.
Devuelve SOLO JSON: {"subject":"","body":"","estimatedBudgetRange":""}`;
    return parseJson<SponsorshipEmailResult>(await this.llm.complete(prompt, llmOpts(0.5)), "SponsorshipEmailAgent");
  }
}
let cached: SponsorshipEmailAgent | undefined;
export function getSponsorshipEmailAgent(): SponsorshipEmailAgent { if (!cached) cached = new SponsorshipEmailAgent(); return cached; }
export function resetSponsorshipEmailAgentForTests(): void { cached = undefined; }
