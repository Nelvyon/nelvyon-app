...
export type B2BLeadGenInput = { industry: string; icp: string; regions?: string[] };
export type B2BLeadGenResult = {
  targetCompanies: Array<{ company: string; fitReason: string; sourceIdeas: string[]; firstMessage: string }>;
};
export type B2BLeadGenAgentDeps = { llm?: ILlmClient };
export class B2BLeadGenAgent {
  constructor(private readonly deps: B2BLeadGenAgentDeps = {}) {}
  private get llm(): ILlmClient { return this.deps.llm ?? LlmClient.getInstance(); }
  async generateLeads(userId: string, input: B2BLeadGenInput): Promise<B2BLeadGenResult> {
...
OUTPUT FORMAT: Respuesta ÚNICAMENTE como JSON válido UTF-8, sin markdown ni texto antes/después. Esquema: {"targetCompanies":[{"company":"","fitReason":"","sourceIdeas":[],"firstMessage":""}]}
...
### BRIEF OPERATIVO
Genera lista de empresas objetivo B2B usando ICP ${enriched.icp}, industria ${enriched.industry}, regiones ${(enriched.regions ?? []).join(", ")}. Usa frameworks SPIN/MEDDIC. SOLO JSON {"targetCompanies":[{"company":"","fitReason":"","sourceIdeas":[],"firstMessage":""}]}`;
    return parseJson<B2BLeadGenResult>(await this.llm.complete(prompt, llmOpts(0.5)), "B2BLeadGenAgent");
  }
}
let cached: B2BLeadGenAgent | undefined;
export function getB2BLeadGenAgent(): B2BLeadGenAgent { if (!cached) cached = new B2BLeadGenAgent(); return cached; }
export function resetB2BLeadGenAgentForTests(): void { cached = undefined; }
