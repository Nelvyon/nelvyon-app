...
export type TourismReviewManagementAgentDeps = { llm?: ILlmClient };
export class TourismReviewManagementAgent {
  constructor(private readonly deps: TourismReviewManagementAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Experiencia en: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Gestión reseñas turismo "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetTraveler}, tono ${enriched.tone}).${loc}
- Solicitud post-estancia email WhatsApp plantillas RGPD opción salida.
- Respuestas positivas TripAdvisor Google Booking personalización por tipo estancia placeholders.
- Protocolo negativas empatía solución canal privado mención mejoras políticas plataforma.
- Estrategia ranking TripAdvisor factores honestos sin manipulación.
- Reseñas sospechosas reporte plataforma documentación interna nota legal.
- Objetivo mensual por fuente métricas internas ejemplo.`;
    const __crmOut = { agentId: "tourism-review-management", result: await this.llm.complete(prompt, tourismLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let tourismReviewManagementCached: TourismReviewManagementAgent | undefined;
export function getTourismReviewManagementAgent(): TourismReviewManagementAgent {
  if (!tourismReviewManagementCached) tourismReviewManagementCached = new TourismReviewManagementAgent();
  return tourismReviewManagementCached;
}
export function resetTourismReviewManagementAgentForTests(): void { tourismReviewManagementCached = undefined; }
