...
export type TourismListingOptimizationAgentDeps = { llm?: ILlmClient };
export class TourismListingOptimizationAgent {
  constructor(private readonly deps: TourismListingOptimizationAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Ubicación listing: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Optimización listings OTA "${enriched.businessName}" (${enriched.businessType}, público ${enriched.targetTraveler}, tono ${enriched.tone}).${loc}
- Títulos Booking Airbnb TripAdvisor keywords intención sin keyword stuffing.
- Descripción gancho emocional especificaciones diferenciadores formato plataforma.
- Amenities reframadas en beneficios bullets.
- FAQs plataforma respuestas modelo plantillas.
- Estrategia fotos orden captions highlights honestos.
- Gestión precios disponibilidad temporada sugerencias disclaimers legales país.`;
    const __crmOut = { agentId: "tourism-listing-optimization", result: await this.llm.complete(prompt, tourismLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let tourismListingOptimizationCached: TourismListingOptimizationAgent | undefined;
export function getTourismListingOptimizationAgent(): TourismListingOptimizationAgent {
  if (!tourismListingOptimizationCached) tourismListingOptimizationCached = new TourismListingOptimizationAgent();
  return tourismListingOptimizationCached;
}
export function resetTourismListingOptimizationAgentForTests(): void { tourismListingOptimizationCached = undefined; }
