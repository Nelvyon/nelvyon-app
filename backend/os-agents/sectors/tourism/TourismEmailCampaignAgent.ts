...
export type TourismEmailCampaignAgentDeps = { llm?: ILlmClient };
export class TourismEmailCampaignAgent {
  constructor(private readonly deps: TourismEmailCampaignAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Contexto destino: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Campañas email turismo "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetTraveler}, ${enriched.tone}).${loc}
- Confirmación reserva info práctica upsell experiencias placeholders horarios RGPD.
- Pre-llegada preparación consejos locales enlaces seguridad medio ambiente etiqueta.
- Post-estancia reseña oferta siguiente visita condiciones fecha límite suave.
- Temporada baja promoción subject lines segmentación ejemplo.
- Nurturing lead consultó sin reserva secuencia 3-5 emails valor sin spam.
- Programa huésped frecuente beneficios tiers claros cumplimiento promociones.`;
    const __crmOut = { agentId: "tourism-email-campaign", result: await this.llm.complete(prompt, tourismLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let tourismEmailCampaignCached: TourismEmailCampaignAgent | undefined;
export function getTourismEmailCampaignAgent(): TourismEmailCampaignAgent {
  if (!tourismEmailCampaignCached) tourismEmailCampaignCached = new TourismEmailCampaignAgent();
  return tourismEmailCampaignCached;
}
export function resetTourismEmailCampaignAgentForTests(): void { tourismEmailCampaignCached = undefined; }
