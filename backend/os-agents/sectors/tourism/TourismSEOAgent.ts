...
export type TourismSEOAgentDeps = { llm?: ILlmClient };
export class TourismSEOAgent {
  constructor(private readonly deps: TourismSEOAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Keywords geo: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
SEO turismo "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetTraveler}, tono ${enriched.tone}).${loc}
- Keywords alojamiento experiencia destino ejemplo hotel boutique apartamento tours.
- Meta titles/descriptions páginas producto plantillas CTR.
- Schema Hotel TouristAttraction LodgingBusiness TouristTrip según modelo validación dev.
- Reseñas TripAdvisor Google estrategia solicitud compliance plataformas.
- Link building blogs viajes medios turísticos propuestas colaboración.
- Comparadores Kayak Trivago GetYourGuide fichas y diferenciación propia web.`;
    const __crmOut = { agentId: "tourism-seo", result: await this.llm.complete(prompt, tourismLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let tourismSEOAgentCached: TourismSEOAgent | undefined;
export function getTourismSEOAgent(): TourismSEOAgent {
  if (!tourismSEOAgentCached) tourismSEOAgentCached = new TourismSEOAgent();
  return tourismSEOAgentCached;
}
export function resetTourismSEOAgentForTests(): void { tourismSEOAgentCached = undefined; }
