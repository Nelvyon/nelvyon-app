...
export type TourismPackageDescriptionAgentDeps = { llm?: ILlmClient };
export class TourismPackageDescriptionAgent {
  constructor(private readonly deps: TourismPackageDescriptionAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Ruta/destino paquete: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Descripción paquete experiencia "${enriched.businessName}" (${enriched.businessType}, viajero ${enriched.targetTraveler}, tono ${enriched.tone}).${loc}
- Nombre evocador paquete sin marcas registradas ajenas.
- Storytelling antes durante después recuerdo emocional sin prometer milagros.
- Itinerario narrativo experiencial no solo tablas horarias placeholders.
- Incluido no incluido transparencia tasas extras legales si aplica.
- Ideal para perfiles claros exclusiones responsabilidad.
- Precio valor percibido justificación sin precio falso CTA plazas temporada real.`;
    const __crmOut = { agentId: "tourism-package-description", result: await this.llm.complete(prompt, tourismLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let tourismPackageDescriptionCached: TourismPackageDescriptionAgent | undefined;
export function getTourismPackageDescriptionAgent(): TourismPackageDescriptionAgent {
  if (!tourismPackageDescriptionCached) tourismPackageDescriptionCached = new TourismPackageDescriptionAgent();
  return tourismPackageDescriptionCached;
}
export function resetTourismPackageDescriptionAgentForTests(): void { tourismPackageDescriptionCached = undefined; }
