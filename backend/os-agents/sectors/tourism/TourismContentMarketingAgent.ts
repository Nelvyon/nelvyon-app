...
export type TourismContentMarketingAgentDeps = { llm?: ILlmClient };
export class TourismContentMarketingAgent {
  constructor(private readonly deps: TourismContentMarketingAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Destino contenido: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Contenido turismo "${enriched.businessName}" (${enriched.businessType}, viajero ${enriched.targetTraveler}, ${enriched.tone}).${loc}
- 20 ideas posts por segmento familia pareja aventura lujo mochilero ejemplo ganchos.
- Artículo blog SEO guía destino qué hacer rutas outline 1500 palabras sugerido.
- Guión vídeo YouTube/Reels experiencia onsite B-roll CTA reserva.
- Newsletter inspiración oferta segment legal promociones condiciones.
- Pinterest tableros fase planificación keywords visuales hashtags.`;
    const __crmOut = { agentId: "tourism-content-marketing", result: await this.llm.complete(prompt, tourismLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let tourismContentMarketingCached: TourismContentMarketingAgent | undefined;
export function getTourismContentMarketingAgent(): TourismContentMarketingAgent {
  if (!tourismContentMarketingCached) tourismContentMarketingCached = new TourismContentMarketingAgent();
  return tourismContentMarketingCached;
}
export function resetTourismContentMarketingAgentForTests(): void { tourismContentMarketingCached = undefined; }
