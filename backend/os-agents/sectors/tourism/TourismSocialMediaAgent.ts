...
export type TourismSocialMediaAgentDeps = { llm?: ILlmClient };
export class TourismSocialMediaAgent {
  constructor(private readonly deps: TourismSocialMediaAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Hashtags geo: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Redes sociales turismo "${enriched.businessName}" (${enriched.businessType}, viajero ${enriched.targetTraveler}, tono ${enriched.tone}).${loc}
- 30 ideas contenido IG TikTok Pinterest ejemplos formatos trending responsables.
- Captions storytelling destino hashtags geográficos autorización modelo imagen huésped.
- UGC repost permiso marcación políticas community guidelines.
- Calendario editorial alta/media/baja temporada picos ejemplo.
- Colabs micro influencers bloggers brief deliverables FTC sugerido español UE.
- Stories disponibilidad ofertas CTA enlaces políticas plataforma.`;
    const __crmOut = { agentId: "tourism-social-media", result: await this.llm.complete(prompt, tourismLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let tourismSocialMediaCached: TourismSocialMediaAgent | undefined;
export function getTourismSocialMediaAgent(): TourismSocialMediaAgent {
  if (!tourismSocialMediaCached) tourismSocialMediaCached = new TourismSocialMediaAgent();
  return tourismSocialMediaCached;
}
export function resetTourismSocialMediaAgentForTests(): void { tourismSocialMediaCached = undefined; }
