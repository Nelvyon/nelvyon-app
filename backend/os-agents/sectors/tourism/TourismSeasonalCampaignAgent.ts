...
export type TourismSeasonalCampaignAgentDeps = { llm?: ILlmClient };
export class TourismSeasonalCampaignAgent {
  constructor(private readonly deps: TourismSeasonalCampaignAgentDeps = {}) {}
  ...
  async run(userId: string, input: TourismInput): Promise<TourismOutput> {
    ...
    const loc = enriched.location ? ` Destino campaña: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Campañas estacionales turismo "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetTraveler}, ${enriched.tone}).${loc}
- Verano reserva anticipada disponibilidad limitada copy email social ads disclaimers fecha.
- Navidad Nochevieja escapadas celebraciones bundles ejemplo condiciones cancelación.
- San Valentín escapadas románticas parejas upscale mid range variantes honestas.
- Semana Santa familias cultura aventura segmentación ejemplo.
- Temporada baja descuentos experiencias temporada inventario placeholders legales.
- Por canal meta Google email organic stories bullets CTA coherencia marca.`;
    const __crmOut = { agentId: "tourism-seasonal-campaign", result: await this.llm.complete(prompt, tourismLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let tourismSeasonalCampaignCached: TourismSeasonalCampaignAgent | undefined;
export function getTourismSeasonalCampaignAgent(): TourismSeasonalCampaignAgent {
  if (!tourismSeasonalCampaignCached) tourismSeasonalCampaignCached = new TourismSeasonalCampaignAgent();
  return tourismSeasonalCampaignCached;
}
export function resetTourismSeasonalCampaignAgentForTests(): void { tourismSeasonalCampaignCached = undefined; }
