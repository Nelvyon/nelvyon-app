...
export interface DisenoWebInput {
  userId: string;
  sector: string;
  brand: string;
  disenoWebBrief?: string;
  metricsBrief?: string;
  metadata?: Record<string, unknown>;
}

export interface DisenoWebOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseDisenoWebLlmJson(raw: string, label: string): Omit<DisenoWebOutput, "agentId"> {
...
export function buildDisenoWebPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: DisenoWebInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const webCtx = params.input.disenoWebBrief?.trim() ? params.input.disenoWebBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

DISEÑO WEB NELVYON OS (v1):
- **Prefijo agentes**: **DisenoWeb**; diseño y conversión web.
- **Auditoría web <3 min**; **diseño completo <10 min**.
- **PageSpeed >95** móvil y desktop; **CRO landing +>40%** vs diseño anterior.
- **WCAG AA 100% automático**; **mantenimiento 100% automático** sin intervención humana.
...
export async function runDisenoWebAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: DisenoWebInput,
  temperature: number,
): Promise<DisenoWebOutput> {
  const prompt = buildDisenoWebPrompt({
  ...
  const parsed = parseDisenoWebLlmJson(raw, agentId);
  const out: DisenoWebOutput = { agentId, ...parsed };
  try {
...
export function getDefaultDisenoWebLlm(): ILlmClient {
