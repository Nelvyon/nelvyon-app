...
export type FashionInfluencerBriefAgentDeps = { llm?: ILlmClient };
export class FashionInfluencerBriefAgent {
  constructor(private readonly deps: FashionInfluencerBriefAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera brief de influencers para ${enriched.brandName}: perfil ideal, briefing creativo, guion sugerido, hashtags campaña, etiquetado/disclosure y métricas de éxito esperadas.`;
    const __crmOut = { agentId: "fashion-influencer-brief", result: await this.llm.complete(prompt, fashionLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let fashionInfluencerBriefCached: FashionInfluencerBriefAgent | undefined;
export function getFashionInfluencerBriefAgent(): FashionInfluencerBriefAgent { if (!fashionInfluencerBriefCached) fashionInfluencerBriefCached = new FashionInfluencerBriefAgent(); return fashionInfluencerBriefCached; }
export function resetFashionInfluencerBriefAgentForTests(): void { fashionInfluencerBriefCached = undefined; }
