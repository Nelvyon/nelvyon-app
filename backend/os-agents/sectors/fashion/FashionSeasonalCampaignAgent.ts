...
export type FashionSeasonalCampaignAgentDeps = { llm?: ILlmClient };
export class FashionSeasonalCampaignAgent {
  constructor(private readonly deps: FashionSeasonalCampaignAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea campaña estacional para ${enriched.brandName}: concepto y naming, mensajes por canal, timeline semanal, copy touchpoints, countdown con urgencia y extensión post-campaña para ${enriched.season ?? "temporada actual"}.`;
    const __crmOut = { agentId: "fashion-seasonal-campaign", result: await this.llm.complete(prompt, fashionLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let fashionSeasonalCampaignCached: FashionSeasonalCampaignAgent | undefined;
export function getFashionSeasonalCampaignAgent(): FashionSeasonalCampaignAgent { if (!fashionSeasonalCampaignCached) fashionSeasonalCampaignCached = new FashionSeasonalCampaignAgent(); return fashionSeasonalCampaignCached; }
export function resetFashionSeasonalCampaignAgentForTests(): void { fashionSeasonalCampaignCached = undefined; }
