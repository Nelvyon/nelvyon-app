...
export type FashionLookbookCopyAgentDeps = { llm?: ILlmClient };
export class FashionLookbookCopyAgent {
  constructor(private readonly deps: FashionLookbookCopyAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera copy de lookbook para ${enriched.brandName}: titulo colección, intro editorial, descripciones de looks, textos de categorías web y copy para banners/creatividades.`;
    const __crmOut = { agentId: "fashion-lookbook-copy", result: await this.llm.complete(prompt, fashionLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let fashionLookbookCopyCached: FashionLookbookCopyAgent | undefined;
export function getFashionLookbookCopyAgent(): FashionLookbookCopyAgent { if (!fashionLookbookCopyCached) fashionLookbookCopyCached = new FashionLookbookCopyAgent(); return fashionLookbookCopyCached; }
export function resetFashionLookbookCopyAgentForTests(): void { fashionLookbookCopyCached = undefined; }
