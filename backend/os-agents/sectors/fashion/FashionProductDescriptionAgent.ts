...
export type FashionProductDescriptionAgentDeps = { llm?: ILlmClient };
export class FashionProductDescriptionAgent {
  constructor(private readonly deps: FashionProductDescriptionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera descripciones de producto para ${enriched.brandName} (${enriched.category}) con nombre evocador, meta <160, descripción larga con storytelling, beneficios emocionales/funcionales, guía tallas/aplicación, cuidados y keywords SEO naturales.`;
    const __crmOut = { agentId: "fashion-product-description", result: await this.llm.complete(prompt, fashionLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let fashionProductDescriptionCached: FashionProductDescriptionAgent | undefined;
export function getFashionProductDescriptionAgent(): FashionProductDescriptionAgent { if (!fashionProductDescriptionCached) fashionProductDescriptionCached = new FashionProductDescriptionAgent(); return fashionProductDescriptionCached; }
export function resetFashionProductDescriptionAgentForTests(): void { fashionProductDescriptionCached = undefined; }
