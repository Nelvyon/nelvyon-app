...
export type FashionBrandStoryAgentDeps = { llm?: ILlmClient };
export class FashionBrandStoryAgent {
  constructor(private readonly deps: FashionBrandStoryAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea historia de marca para ${enriched.brandName} (${enriched.category}) dirigida a ${enriched.targetAudience}, rango ${enriched.priceRange}, tono ${enriched.tone}, temporada ${enriched.season ?? "atemporal"}: origen, valores, mision/vision, voz, manifiesto y tagline principal + 3 alternativos.`;
    const __crmOut = { agentId: "fashion-brand-story", result: await this.llm.complete(prompt, fashionLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let fashionBrandStoryCached: FashionBrandStoryAgent | undefined;
export function getFashionBrandStoryAgent(): FashionBrandStoryAgent { if (!fashionBrandStoryCached) fashionBrandStoryCached = new FashionBrandStoryAgent(); return fashionBrandStoryCached; }
export function resetFashionBrandStoryAgentForTests(): void { fashionBrandStoryCached = undefined; }
