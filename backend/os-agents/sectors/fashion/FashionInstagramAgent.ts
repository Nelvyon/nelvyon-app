...
export type FashionInstagramAgentDeps = { llm?: ILlmClient };
export class FashionInstagramAgent {
  constructor(private readonly deps: FashionInstagramAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea estrategia Instagram para ${enriched.brandName}: 30 ideas, captions storytelling + hashtags, estrategia reels, guion stories con encuestas/CTA y calendario mensual por temporada ${enriched.season ?? "actual"}.`;
    const __crmOut = { agentId: "fashion-instagram", result: await this.llm.complete(prompt, fashionLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let fashionInstagramCached: FashionInstagramAgent | undefined;
export function getFashionInstagramAgent(): FashionInstagramAgent { if (!fashionInstagramCached) fashionInstagramCached = new FashionInstagramAgent(); return fashionInstagramCached; }
export function resetFashionInstagramAgentForTests(): void { fashionInstagramCached = undefined; }
