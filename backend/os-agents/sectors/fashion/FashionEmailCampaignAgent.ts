...
export type FashionEmailCampaignAgentDeps = { llm?: ILlmClient };
export class FashionEmailCampaignAgent {
  constructor(private readonly deps: FashionEmailCampaignAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea campañas de email para ${enriched.brandName}: lanzamiento colección, carrito abandonado, reactivación, bienvenida 3 emails, temporada y segmentación recomendada por comportamiento.`;
    const __crmOut = { agentId: "fashion-email-campaign", result: await this.llm.complete(prompt, fashionLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let fashionEmailCampaignCached: FashionEmailCampaignAgent | undefined;
export function getFashionEmailCampaignAgent(): FashionEmailCampaignAgent { if (!fashionEmailCampaignCached) fashionEmailCampaignCached = new FashionEmailCampaignAgent(); return fashionEmailCampaignCached; }
export function resetFashionEmailCampaignAgentForTests(): void { fashionEmailCampaignCached = undefined; }
