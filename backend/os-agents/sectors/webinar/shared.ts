...
export interface WebinarOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseWebinarLlmJson(raw: string, label: string): Omit<WebinarOutput, "agentId"> {
...
export function buildWebinarPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const webinarCtx = params.input.webinarBrief?.trim() ? params.input.webinarBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

WEBINAR + VIDEO HOSTING NELVYON OS (v1):
- **Prefijo agentes**: **Webinar**; webinars y hosting de vídeo.
- **Asistencia >60%** registrados (industria 40%); **engagement directo >70%** asistentes.
- **Conversión webinar→cliente >15%**; **replay views >3×** directo; **revenue/webinar >2.000€** auto.
- **Setup webinar completo <10 minutos**.
...
export async function runWebinarAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  const prompt = buildWebinarPrompt({
  ...
  const parsed = parseWebinarLlmJson(raw, agentId);
  const out: WebinarOutput = { agentId, ...parsed };
  try {
...
export function getDefaultWebinarLlm(): ILlmClient {
