...
export function parseQrCodeGeneratorLlmJson(raw: string, label: string): Omit<QrCodeGeneratorOutput, "agentId"> {
...
export function buildQrCodeGeneratorPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const qrCtx = params.input.qrBrief?.trim() ? params.input.qrBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

QR CODE GENERATOR NELVYON OS (v1):
- **Prefijo agentes**: **QrCodeGenerator**; generación y analytics de QR codes.
- **QR <1s**; dinámicos con tracking RT; bulk **10.000 únicos <60s**.
- **Branding con logo <3s**; analytics RT por código; formatos **SVG, PNG, PDF, EPS**.
...
export async function runQrCodeGeneratorAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: QrCodeGeneratorInput,
  temperature: number,
): Promise<QrCodeGeneratorOutput> {
  const prompt = buildQrCodeGeneratorPrompt({
  ...
  const parsed = parseQrCodeGeneratorLlmJson(raw, agentId);
  const out: QrCodeGeneratorOutput = { agentId, ...parsed };
  try {
...
export function getDefaultQrCodeGeneratorLlm(): ILlmClient {
