...
export type TechBlogContentAgentDeps = { llm?: ILlmClient };
export class TechBlogContentAgent {
  constructor(private readonly deps: TechBlogContentAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea contenido blog tecnico para ${enriched.startupName}: 5 ideas SEO + outline, articulo 1500 palabras, tutorial paso a paso, caso uso real con codigo si aplica, meta description y slug. Nicho: ${enriched.targetMarket}.`;
    const __crmOut = { agentId: "tech-blog-content", result: await this.llm.complete(prompt, llmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let cached: TechBlogContentAgent | undefined;
export function getTechBlogContentAgent(): TechBlogContentAgent { if (!cached) cached = new TechBlogContentAgent(); return cached; }
export function resetTechBlogContentAgentForTests(): void { cached = undefined; }
