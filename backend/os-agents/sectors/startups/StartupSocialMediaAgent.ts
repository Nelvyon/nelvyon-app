...
export type StartupSocialMediaAgentDeps = { llm?: ILlmClient };
export class StartupSocialMediaAgent {
  constructor(private readonly deps: StartupSocialMediaAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea estrategia social media para ${enriched.startupName}: 30 posts LinkedIn founder-led, 15 tweets/hilos X, estrategia TikTok/Instagram B2C si aplica y calendario mensual autentico no corporativo.`;
    const __crmOut = { agentId: "startup-social-media", result: await this.llm.complete(prompt, llmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let cached: StartupSocialMediaAgent | undefined;
export function getStartupSocialMediaAgent(): StartupSocialMediaAgent { if (!cached) cached = new StartupSocialMediaAgent(); return cached; }
export function resetStartupSocialMediaAgentForTests(): void { cached = undefined; }
