...
export type ElevatorPitchAgentDeps = { llm?: ILlmClient };
export class ElevatorPitchAgent {
  constructor(private readonly deps: ElevatorPitchAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea pitch para ${enriched.startupName}: versiones 30s, 1min, 3min con estructura problema-solucion-mercado-traccion-equipo-ask. Incluye version inversor, cliente y prensa. Producto: ${enriched.productDescription}. Mercado: ${enriched.targetMarket}. Etapa: ${enriched.stage}. Tono: ${enriched.tone}.`;
    const __crmOut = { agentId: "elevator-pitch", result: await this.llm.complete(prompt, llmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let cached: ElevatorPitchAgent | undefined;
export function getElevatorPitchAgent(): ElevatorPitchAgent { if (!cached) cached = new ElevatorPitchAgent(); return cached; }
export function resetElevatorPitchAgentForTests(): void { cached = undefined; }
