...
export type PRAndMediaAgentDeps = { llm?: ILlmClient };
export class PRAndMediaAgent {
  constructor(private readonly deps: PRAndMediaAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera estrategia PR para ${enriched.startupName}: press release (lanzamiento/funding/hito), 20 medios tech con angulo, cold email periodistas, byline tipo Forbes/TechCrunch, boilerplate y quote CEO.`;
    const __crmOut = { agentId: "pr-and-media", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: PRAndMediaAgent | undefined;
export function getPRAndMediaAgent(): PRAndMediaAgent { if (!cached) cached = new PRAndMediaAgent(); return cached; }
export function resetPRAndMediaAgentForTests(): void { cached = undefined; }
