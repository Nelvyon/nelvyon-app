...
export type InvestorDeckNarrativeAgentDeps = { llm?: ILlmClient };
export class InvestorDeckNarrativeAgent {
  constructor(private readonly deps: InvestorDeckNarrativeAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Escribe narrativa de deck de inversores en 12 slides para ${enriched.startupName} (problema, solucion, TAM/SAM/SOM, producto, negocio, traccion, competencia, equipo, roadmap, financials, uso fondos, CTA). Max 3 bullets por slide + dato clave.`;
    const __crmOut = { agentId: "investor-deck-narrative", result: await this.llm.complete(prompt, llmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let cached: InvestorDeckNarrativeAgent | undefined;
export function getInvestorDeckNarrativeAgent(): InvestorDeckNarrativeAgent { if (!cached) cached = new InvestorDeckNarrativeAgent(); return cached; }
export function resetInvestorDeckNarrativeAgentForTests(): void { cached = undefined; }
