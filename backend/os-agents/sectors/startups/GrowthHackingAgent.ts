...
export type GrowthHackingAgentDeps = { llm?: ILlmClient };
export class GrowthHackingAgent {
  constructor(private readonly deps: GrowthHackingAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera estrategia growth hacking para ${enriched.startupName}: 10 experimentos priorizados ICE, loops virales, referidos double-sided, canal principal y AARRR con targets por etapa ${enriched.stage}.`;
    const __crmOut = { agentId: "growth-hacking", result: await this.llm.complete(prompt, llmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let cached: GrowthHackingAgent | undefined;
export function getGrowthHackingAgent(): GrowthHackingAgent { if (!cached) cached = new GrowthHackingAgent(); return cached; }
export function resetGrowthHackingAgentForTests(): void { cached = undefined; }
