...
export type DeveloperCommunityAgentDeps = { llm?: ILlmClient };
export class DeveloperCommunityAgent {
  constructor(private readonly deps: DeveloperCommunityAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Disena estrategia comunidad dev para ${enriched.startupName}: onboarding Discord/Slack, 10 posts HN/Reddit/DEV, programa early adopters, feedback request beta y template changelog semanal.`;
    const __crmOut = { agentId: "developer-community", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: DeveloperCommunityAgent | undefined;
export function getDeveloperCommunityAgent(): DeveloperCommunityAgent { if (!cached) cached = new DeveloperCommunityAgent(); return cached; }
export function resetDeveloperCommunityAgentForTests(): void { cached = undefined; }
