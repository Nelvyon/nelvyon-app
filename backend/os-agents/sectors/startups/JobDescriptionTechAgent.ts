...
export type JobDescriptionTechAgentDeps = { llm?: ILlmClient };
export class JobDescriptionTechAgent {
  constructor(private readonly deps: JobDescriptionTechAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea job descriptions tech para ${enriched.startupName}: titulo optimizado, mision/cultura, responsabilidades de impacto, stack honesto, oferta (equity/remote/learning), proceso transparente y requisitos inclusivos.`;
    const __crmOut = { agentId: "job-description-tech", result: await this.llm.complete(prompt, llmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let cached: JobDescriptionTechAgent | undefined;
export function getJobDescriptionTechAgent(): JobDescriptionTechAgent { if (!cached) cached = new JobDescriptionTechAgent(); return cached; }
export function resetJobDescriptionTechAgentForTests(): void { cached = undefined; }
