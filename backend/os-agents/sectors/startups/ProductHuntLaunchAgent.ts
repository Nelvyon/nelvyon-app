...
export type ProductHuntLaunchAgentDeps = { llm?: ILlmClient };
export class ProductHuntLaunchAgent {
  constructor(private readonly deps: ProductHuntLaunchAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera lanzamiento Product Hunt para ${enriched.startupName}: tagline <60, descripcion <260, first comment fundador, FAQ, hunter outreach y mejor dia/hora. Producto: ${enriched.productDescription}.`;
    const __crmOut = { agentId: "product-hunt-launch", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: ProductHuntLaunchAgent | undefined;
export function getProductHuntLaunchAgent(): ProductHuntLaunchAgent { if (!cached) cached = new ProductHuntLaunchAgent(); return cached; }
export function resetProductHuntLaunchAgentForTests(): void { cached = undefined; }
