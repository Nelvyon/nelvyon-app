...
export type EducationEmailNurturingAgentDeps = { llm?: ILlmClient };
export class EducationEmailNurturingAgent {
  constructor(private readonly deps: EducationEmailNurturingAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Diseña secuencia de 6 emails nurturing para ${enriched.institutionName}: lead magnet, error común, caso de éxito, objeciones (tiempo/dinero/nivel), apertura y cierre de inscripción con urgencia genuina.`;
    const __crmOut = { agentId: "education-email-nurturing", result: await this.llm.complete(prompt, educationLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let educationEmailNurturingCached: EducationEmailNurturingAgent | undefined;
export function getEducationEmailNurturingAgent(): EducationEmailNurturingAgent { if (!educationEmailNurturingCached) educationEmailNurturingCached = new EducationEmailNurturingAgent(); return educationEmailNurturingCached; }
export function resetEducationEmailNurturingAgentForTests(): void { educationEmailNurturingCached = undefined; }
