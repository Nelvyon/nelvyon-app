...
export type EducationContentMarketingAgentDeps = { llm?: ILlmClient };
export class EducationContentMarketingAgent {
  constructor(private readonly deps: EducationContentMarketingAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea estrategia content marketing para ${enriched.institutionName}: 20 ideas LinkedIn/blog, artículo pilar 2000 palabras, guion podcast/YouTube educativo, newsletter semanal, hilo X y estrategia contenido gratuito que convierta a pago.`;
    const __crmOut = { agentId: "education-content-marketing", result: await this.llm.complete(prompt, educationLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let educationContentMarketingCached: EducationContentMarketingAgent | undefined;
export function getEducationContentMarketingAgent(): EducationContentMarketingAgent { if (!educationContentMarketingCached) educationContentMarketingCached = new EducationContentMarketingAgent(); return educationContentMarketingCached; }
export function resetEducationContentMarketingAgentForTests(): void { educationContentMarketingCached = undefined; }
