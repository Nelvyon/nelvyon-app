...
export type EducationCourseDescriptionAgentDeps = { llm?: ILlmClient };
export class EducationCourseDescriptionAgent {
  constructor(private readonly deps: EducationCourseDescriptionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea descripción completa de curso para ${enriched.institutionName} (${enriched.subjectArea}): título SEO, subtítulo de valor, descripción larga (~600 palabras), 8-10 resultados medibles, para quién es/no es, requisitos previos y transformación final. Público ${enriched.targetStudent}, tono ${enriched.tone}.`;
    const __crmOut = { agentId: "education-course-description", result: await this.llm.complete(prompt, educationLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let educationCourseDescriptionCached: EducationCourseDescriptionAgent | undefined;
export function getEducationCourseDescriptionAgent(): EducationCourseDescriptionAgent { if (!educationCourseDescriptionCached) educationCourseDescriptionCached = new EducationCourseDescriptionAgent(); return educationCourseDescriptionCached; }
export function resetEducationCourseDescriptionAgentForTests(): void { educationCourseDescriptionCached = undefined; }
