...
export type EducationAdsAgentDeps = { llm?: ILlmClient };
export class EducationAdsAgent {
  constructor(private readonly deps: EducationAdsAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera campañas paid media para ${enriched.institutionName}: Google Search ads, Meta ads cold/retargeting, retargeting 3 fases, copy YouTube pre-roll y presupuesto recomendado por fase de lanzamiento.`;
    const __crmOut = { agentId: "education-ads", result: await this.llm.complete(prompt, educationLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let educationAdsCached: EducationAdsAgent | undefined;
export function getEducationAdsAgent(): EducationAdsAgent { if (!educationAdsCached) educationAdsCached = new EducationAdsAgent(); return educationAdsCached; }
export function resetEducationAdsAgentForTests(): void { educationAdsCached = undefined; }
