...
export type EducationSalesPageAgentDeps = { llm?: ILlmClient };
export class EducationSalesPageAgent {
  constructor(private readonly deps: EducationSalesPageAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Escribe sales page para programa de ${enriched.institutionName}: headline, subheadline, pain points, credibilidad del formador, currículo módulo a módulo, testimonios template, garantía, FAQ objeciones, precio con justificación y CTA urgente real.`;
    const __crmOut = { agentId: "education-sales-page", result: await this.llm.complete(prompt, educationLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let educationSalesPageCached: EducationSalesPageAgent | undefined;
export function getEducationSalesPageAgent(): EducationSalesPageAgent { if (!educationSalesPageCached) educationSalesPageCached = new EducationSalesPageAgent(); return educationSalesPageCached; }
export function resetEducationSalesPageAgentForTests(): void { educationSalesPageCached = undefined; }
