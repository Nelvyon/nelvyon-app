...
export type EducationRetentionAgentDeps = { llm?: ILlmClient };
export class EducationRetentionAgent {
  constructor(private readonly deps: EducationRetentionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea estrategia de retención/finalización para ${enriched.institutionName}: activación inicial, check-in semanal, recuperación inactivos 7/14/21, logros/badges, celebraciones por progreso y plan para maximizar completion rate.`;
    const __crmOut = { agentId: "education-retention", result: await this.llm.complete(prompt, educationLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let educationRetentionCached: EducationRetentionAgent | undefined;
export function getEducationRetentionAgent(): EducationRetentionAgent { if (!educationRetentionCached) educationRetentionCached = new EducationRetentionAgent(); return educationRetentionCached; }
export function resetEducationRetentionAgentForTests(): void { educationRetentionCached = undefined; }
