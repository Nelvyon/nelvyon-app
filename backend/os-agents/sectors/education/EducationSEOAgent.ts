...
export type EducationSEOAgentDeps = { llm?: ILlmClient };
export class EducationSEOAgent {
  constructor(private readonly deps: EducationSEOAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Optimiza SEO educativo para ${enriched.institutionName}: keywords por temática/nivel, metas de curso/categoría, schema Course/EducationalOccupationalProgram, estrategia blog y link building sectorial.`;
    const __crmOut = { agentId: "education-seo", result: await this.llm.complete(prompt, educationLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let educationSEOCached: EducationSEOAgent | undefined;
export function getEducationSEOAgent(): EducationSEOAgent { if (!educationSEOCached) educationSEOCached = new EducationSEOAgent(); return educationSEOCached; }
export function resetEducationSEOAgentForTests(): void { educationSEOCached = undefined; }
