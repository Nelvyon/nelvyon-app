...
export type EducationCommunityAgentDeps = { llm?: ILlmClient };
export class EducationCommunityAgent {
  constructor(private readonly deps: EducationCommunityAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Diseña estrategia de comunidad educativa para ${enriched.institutionName}: estructura grupo privado, normas, eventos recurrentes, embajadores, engagement y métricas de salud de comunidad.`;
    const __crmOut = { agentId: "education-community", result: await this.llm.complete(prompt, educationLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let educationCommunityCached: EducationCommunityAgent | undefined;
export function getEducationCommunityAgent(): EducationCommunityAgent { if (!educationCommunityCached) educationCommunityCached = new EducationCommunityAgent(); return educationCommunityCached; }
export function resetEducationCommunityAgentForTests(): void { educationCommunityCached = undefined; }
