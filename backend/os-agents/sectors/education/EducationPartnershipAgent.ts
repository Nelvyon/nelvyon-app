...
export type EducationPartnershipAgentDeps = { llm?: ILlmClient };
export class EducationPartnershipAgent {
  constructor(private readonly deps: EducationPartnershipAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera estrategia de partnerships para ${enriched.institutionName}: socios ideales, propuesta de valor por socio, outreach email, estructura de acuerdo y programa de empresas colaboradoras.`;
    const __crmOut = { agentId: "education-partnership", result: await this.llm.complete(prompt, educationLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let educationPartnershipCached: EducationPartnershipAgent | undefined;
export function getEducationPartnershipAgent(): EducationPartnershipAgent { if (!educationPartnershipCached) educationPartnershipCached = new EducationPartnershipAgent(); return educationPartnershipCached; }
export function resetEducationPartnershipAgentForTests(): void { educationPartnershipCached = undefined; }
