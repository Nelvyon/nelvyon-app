...
export type EducationStudentTestimonialAgentDeps = { llm?: ILlmClient };
export class EducationStudentTestimonialAgent {
  constructor(private readonly deps: EducationStudentTestimonialAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea sistema de captación de testimonios para ${enriched.institutionName}: email post-curso, preguntas guía, template caso éxito web, solicitud video testimonio, formatos RRSS y mapeo objeciones que resuelven.`;
    const __crmOut = { agentId: "education-student-testimonial", result: await this.llm.complete(prompt, educationLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let educationStudentTestimonialCached: EducationStudentTestimonialAgent | undefined;
export function getEducationStudentTestimonialAgent(): EducationStudentTestimonialAgent { if (!educationStudentTestimonialCached) educationStudentTestimonialCached = new EducationStudentTestimonialAgent(); return educationStudentTestimonialCached; }
export function resetEducationStudentTestimonialAgentForTests(): void { educationStudentTestimonialCached = undefined; }
