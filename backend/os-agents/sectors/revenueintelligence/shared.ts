...
export interface RevenueIntelligenceInput {
  ...
  sector: string;
  brand: string;
  revenueIntelligenceBrief?: string;
  metricsBrief?: string;
...
export function parseRevenueIntelligenceLlmJson(
  raw: string,
  label: string,
): Omit<RevenueIntelligenceOutput, "agentId"> {
...
export function buildRevenueIntelligencePrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const riCtx = params.input.revenueIntelligenceBrief?.trim()
    ? params.input.revenueIntelligenceBrief.trim()
...
REVENUE INTELLIGENCE + CALL AI NELVYON OS (v1):
- **Prefijo agentes**: **RevenueIntelligence**; inteligencia de ingresos y llamadas.
- **Transcripción RT <1 s latencia**, **40+ idiomas**.
- **Deal risk durante llamada** en tiempo real; **win/loss <5 min** post-llamada.
- **Forecast accuracy >93%** a 30/60/90 días; **CRM auto <30 s** post-llamada.
- **0 tareas manuales** post-llamada para el vendedor.
...
- Revenue intelligence / contexto: ${riCtx}
...
export async function runRevenueIntelligenceAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: RevenueIntelligenceInput,
  temperature: number,
): Promise<RevenueIntelligenceOutput> {
  const prompt = buildRevenueIntelligencePrompt({
  ...
  const parsed = parseRevenueIntelligenceLlmJson(raw, agentId);
  const out: RevenueIntelligenceOutput = { agentId, ...parsed };
  try {
...
export function getDefaultRevenueIntelligenceLlm(): ILlmClient {
