...
export type NgoEmailCampaignAgentDeps = { llm?: ILlmClient };
export class NgoEmailCampaignAgent {
  constructor(private readonly deps: NgoEmailCampaignAgentDeps = {}) {}
  ...
  async run(userId: string, input: NgoInput): Promise<NgoOutput> {
    ...
    const geo = enriched.country ? ` Menciones fiscales región: ${enriched.country}.` : "";
...
### BRIEF OPERATIVO
Email marketing "${enriched.organizationName}" (${enriched.cause}, socios público ${enriched.targetAudience}, tono ${enriched.tone}).${geo}
- Bienvenida donante impacto primera contribución número agregados.
- Newsletter mensual historia métricas CTA donar voluntariado RGPD unsubscribe.
- Fin de año resumen rendición incentives fiscales reales año en curso.
- Renovación cuota socio anual beneficio continuación misión.
- Reactivación inactivos serie 3 correos espaciados sin guilt-tripping tóxico.
- Secuencia voluntario → micro-donativo opcional respetuosa opt-in.`;
    const __crmOut = { agentId: "ngo-email-campaign", result: await this.llm.complete(prompt, ngoLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let ngoEmailCampaignCached: NgoEmailCampaignAgent | undefined;
export function getNgoEmailCampaignAgent(): NgoEmailCampaignAgent { if (!ngoEmailCampaignCached) ngoEmailCampaignCached = new NgoEmailCampaignAgent(); return ngoEmailCampaignCached; }
export function resetNgoEmailCampaignAgentForTests(): void { ngoEmailCampaignCached = undefined; }
