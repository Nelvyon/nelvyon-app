...
export type NgoTransparencyReportAgentDeps = { llm?: ILlmClient };
export class NgoTransparencyReportAgent {
  constructor(private readonly deps: NgoTransparencyReportAgentDeps = {}) {}
  ...
  async run(userId: string, input: NgoInput): Promise<NgoOutput> {
    ...
    const geo = enriched.country ? ` Marco referencia ${enriched.country}.` : "";
...
### BRIEF OPERATIVO
Transparencia ONG "${enriched.organizationName}" (causa ${enriched.cause}, lectores ${enriched.targetAudience}, tono ${enriched.tone}).${geo}
- Resumen ejecutivo memoria anual lenguaje no técnico 1-2 páginas outline.
- Texto infografía distribución presupuesto % programa vs gastos gestión ejemplo comparables sector.
- Número impact año clave benchmarks honestos fuente [AUDITORÍA].
- FAQ fondos remuneración ratios evaluaciones externas.
- Comunicado crisis reputacional pasos contenido tiempo portavoz política evidencias.
Estrategia transparencia proactiva blog datos abiertos calendario publicaciones.`;
    const __crmOut = { agentId: "ngo-transparency-report", result: await this.llm.complete(prompt, ngoLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let ngoTransparencyReportCached: NgoTransparencyReportAgent | undefined;
export function getNgoTransparencyReportAgent(): NgoTransparencyReportAgent { if (!ngoTransparencyReportCached) ngoTransparencyReportCached = new NgoTransparencyReportAgent(); return ngoTransparencyReportCached; }
export function resetNgoTransparencyReportAgentForTests(): void { ngoTransparencyReportCached = undefined; }
