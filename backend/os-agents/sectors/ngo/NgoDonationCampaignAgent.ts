...
export type NgoDonationCampaignAgentDeps = { llm?: ILlmClient };
export class NgoDonationCampaignAgent {
  constructor(private readonly deps: NgoDonationCampaignAgentDeps = {}) {}
  ...
  async run(userId: string, input: NgoInput): Promise<NgoOutput> {
    ...
    const geo = enriched.country ? ` Marco fiscal/comunicación: ${enriched.country}.` : "";
...
### BRIEF OPERATIVO
Campaña donaciones "${enriched.organizationName}" (${enriched.cause}, ${enriched.targetAudience}, ${enriched.tone}).${geo}
- Carta storytelling caso anonimizado con consentimiento ético.
- Email urgencia con fecha/cupo real verificables.
- Landing donación tabla 10€/50€/100€ equivalencias impacto con asterisco metodología cálculo.
- Matching campaign mecánica transparente donante matching partner.
- Estrategia recurrente vs punctual comunicaciones y deducciones según país.
- Agradecimiento post-dono fidelización sin presión nueva donación inmediata.
Indicación revisión jurídico fiscal local.`;
    const __crmOut = { agentId: "ngo-donation-campaign", result: await this.llm.complete(prompt, ngoLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let ngoDonationCampaignCached: NgoDonationCampaignAgent | undefined;
export function getNgoDonationCampaignAgent(): NgoDonationCampaignAgent { if (!ngoDonationCampaignCached) ngoDonationCampaignCached = new NgoDonationCampaignAgent(); return ngoDonationCampaignCached; }
export function resetNgoDonationCampaignAgentForTests(): void { ngoDonationCampaignCached = undefined; }
