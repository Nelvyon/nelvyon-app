...
export type NgoGrantWritingAgentDeps = { llm?: ILlmClient };
export class NgoGrantWritingAgent {
  constructor(private readonly deps: NgoGrantWritingAgentDeps = {}) {}
  ...
  async run(userId: string, input: NgoInput): Promise<NgoOutput> {
    ...
    const geo = enriched.country ? ` Convocatorias referencia país: ${enriched.country}.` : "";
...
### BRIEF OPERATIVO
Redacción subvenciones ONG "${enriched.organizationName}" (causa ${enriched.cause}, beneficiarios objetivo comunicación ${enriched.targetAudience}).${geo}
Esquema propuesta: problema objetivos SMART metodología actividades cronograma presupuesto sostenibilidad igualdad género inclusión placeholders.
Resumen ejecutivo 1 página fundación privada.
Narrativa impacto evidencia indicadores resultado verificables.
Marco lógico simplificado entrada-salidas-resultados-impactos.
Checklist España/Europa AECID autonómico UE [rellenar convocatoria concreta].
Template informe final justificación beneficiarios agregados no datos personales.
Tono ${enriched.tone}. No sustituye revisión técnico-económico.`;
    const __crmOut = { agentId: "ngo-grant-writing", result: await this.llm.complete(prompt, ngoLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let ngoGrantWritingCached: NgoGrantWritingAgent | undefined;
export function getNgoGrantWritingAgent(): NgoGrantWritingAgent { if (!ngoGrantWritingCached) ngoGrantWritingCached = new NgoGrantWritingAgent(); return ngoGrantWritingCached; }
export function resetNgoGrantWritingAgentForTests(): void { ngoGrantWritingCached = undefined; }
