...
export function parseRetailLlmJson(raw: string, label: string): Omit<RetailOutput, "agentId"> {
...
export function buildRetailPrompt(params: {
  eliteRole: string;
  mission: string;
...
RETAIL NELVYON OS (v1):
- **Prefijo agentes**: **Retail**; comercio físico y local.
- **Google Maps top 3 local <90 días**; **demanda accuracy >88%**.
- **Pricing vs competencia cada 24 h** automático; **fidelización sin intervención**.
- **Reseñas respondidas <1 h** automático; **LTV +>35%** en 6 meses.
...
export async function runRetailAgentCore(
  ...
  input: RetailInput,
  temperature: number,
): Promise<RetailOutput> {
  const prompt = buildRetailPrompt({
  ...
  const parsed = parseRetailLlmJson(raw, agentId);
  const out: RetailOutput = { agentId, ...parsed };
  try {
...
export function getDefaultRetailLlm(): ILlmClient {
