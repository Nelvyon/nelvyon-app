...
export type FinanceTrustBuildingAgentDeps = { llm?: ILlmClient };
export class FinanceTrustBuildingAgent {
  constructor(private readonly deps: FinanceTrustBuildingAgentDeps = {}) {}
  ...
  async run(userId: string, input: FinanceInput): Promise<FinanceOutput> {
    ...
    const reg = enriched.regulation ? ` Supervisor/regulación: ${enriched.regulation}.` : "";
...
### BRIEF OPERATIVO
Construcción confianza "${enriched.companyName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}).${reg}
- Transparencia: mostrar regulación, licencias, tarifas claras (estructura página precios).
- Contenido educación financiera sin venta agresiva (calendario formatos).
- Testimonios con resultados cuantificados [X]% ahorro con aviso no garantiza futuro rendimiento.
- Presencia medios como fuente experta (pitch 3 líneas).
- Certificaciones sellos calidad (lista genérica placeholder).
Cumplimiento publicidad financiera y RGPD.`;
    const __crmOut = { agentId: "finance-trust-building", result: await this.llm.complete(prompt, financeLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let financeTrustBuildingCached: FinanceTrustBuildingAgent | undefined;
export function getFinanceTrustBuildingAgent(): FinanceTrustBuildingAgent { if (!financeTrustBuildingCached) financeTrustBuildingCached = new FinanceTrustBuildingAgent(); return financeTrustBuildingCached; }
export function resetFinanceTrustBuildingAgentForTests(): void { financeTrustBuildingCached = undefined; }
