...
export type FinanceReferralAgentDeps = { llm?: ILlmClient };
export class FinanceReferralAgent {
  constructor(private readonly deps: FinanceReferralAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Programa referidos "${enriched.companyName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}):
- Mensaje solicitud referidos con incentivos ejemplo cumpliendo normativa (evitar soborno al particulinar; usar marcos genéricos "beneficio reputacional/programa empresa").
- Colaboraciones abogados notarios gestores inmobiliarias (mensaje inicial + valor recíproco).
- Partnering planes pensiones empleadores (proposal one-pager bullets).
- Template acuerdo colaboración corredores/agentes vinculados (outline cláusulas revisables).
Incluye aviso legal: revisar distribución remuneraciones MiFID/DGS cuando aplique.`;
    const __crmOut = { agentId: "finance-referral", result: await this.llm.complete(prompt, financeLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let financeReferralCached: FinanceReferralAgent | undefined;
export function getFinanceReferralAgent(): FinanceReferralAgent { if (!financeReferralCached) financeReferralCached = new FinanceReferralAgent(); return financeReferralCached; }
export function resetFinanceReferralAgentForTests(): void { financeReferralCached = undefined; }
