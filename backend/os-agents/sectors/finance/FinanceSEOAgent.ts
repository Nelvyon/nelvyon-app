...
export type FinanceSEOAgentDeps = { llm?: ILlmClient };
export class FinanceSEOAgent {
  constructor(private readonly deps: FinanceSEOAgentDeps = {}) {}
...
### BRIEF OPERATIVO
SEO financiero para "${enriched.companyName}" (${enriched.serviceType}, audiencia ${enriched.targetClient}, ${enriched.tone}):
- Keywords por producto y ciudad (ej. asesor financiero Madrid, hipoteca fija tipo, comparativa seguros).
- Meta titles/descriptions páginas producto ejemplo.
- JSON-LD FinancialService / InsuranceAgency placeholders.
- Estrategia reseñas Google y Trustpilot (ética RGPD).
- Presencia HelpMyCash, Kelisto, iSeguros tácticas on-page/off-page.
- Link building medios económicos (guest post estudios datos).
Disclaimer SEO no sustituye compliance publicitario.`;
    const __crmOut = { agentId: "finance-seo", result: await this.llm.complete(prompt, financeLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let financeSEOAgentCached: FinanceSEOAgent | undefined;
export function getFinanceSEOAgent(): FinanceSEOAgent { if (!financeSEOAgentCached) financeSEOAgentCached = new FinanceSEOAgent(); return financeSEOAgentCached; }
export function resetFinanceSEOAgentForTests(): void { financeSEOAgentCached = undefined; }
