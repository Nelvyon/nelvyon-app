...
export type FinanceLeadGenerationAgentDeps = { llm?: ILlmClient };
export class FinanceLeadGenerationAgent {
  constructor(private readonly deps: FinanceLeadGenerationAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Captación leads financieros/seguros para "${enriched.companyName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}):
- Landing simulador/calculadora (hipoteca ahorro seguro): hero campos resultado CTA y consent RGPD.
- Google Search alta intención (ejemplos keywords adaptados al tipo servicio).
- Meta planificación financiera y seguros vida (mensajes prudentes sin promesas).
- Webinar educativo gratuito: tema título funnel emails inscripción.
- Convenios inmobiliarias leads hipotecarios (pitch y material co-branded placeholders).
Marcar necesidad revisión jurídica/compliance antes de publicar.`;
    const __crmOut = { agentId: "finance-lead-generation", result: await this.llm.complete(prompt, financeLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let financeLeadGenerationCached: FinanceLeadGenerationAgent | undefined;
export function getFinanceLeadGenerationAgent(): FinanceLeadGenerationAgent { if (!financeLeadGenerationCached) financeLeadGenerationCached = new FinanceLeadGenerationAgent(); return financeLeadGenerationCached; }
export function resetFinanceLeadGenerationAgentForTests(): void { financeLeadGenerationCached = undefined; }
