...
export type FinanceEmailNurturingAgentDeps = { llm?: ILlmClient };
export class FinanceEmailNurturingAgent {
  constructor(private readonly deps: FinanceEmailNurturingAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Secuencia email nurturing financiero "${enriched.companyName}" (${enriched.serviceType}, ${enriched.targetClient}, tono ${enriched.tone}):
Redacta 6 emails: (1) guía gratuita + presentación sin presión, (2) error común al contratar producto X genérico, (3) caso real con métricas placeholder [RENT]% ahorro, (4) objeción "ya tengo banco/seguro", (5) primera consulta gratuita con límites, (6) urgencia genuina tipo cambio fiscal/tipos/renovación póliza (sin alarmismo).
Asuntos sugeridos, cuerpo breve, enlaces placeholders, disclaimers inversiones.`;
    const __crmOut = { agentId: "finance-email-nurturing", result: await this.llm.complete(prompt, financeLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let financeEmailNurturingCached: FinanceEmailNurturingAgent | undefined;
export function getFinanceEmailNurturingAgent(): FinanceEmailNurturingAgent { if (!financeEmailNurturingCached) financeEmailNurturingCached = new FinanceEmailNurturingAgent(); return financeEmailNurturingCached; }
export function resetFinanceEmailNurturingAgentForTests(): void { financeEmailNurturingCached = undefined; }
