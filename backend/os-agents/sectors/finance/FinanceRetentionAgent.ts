...
export type FinanceRetentionAgentDeps = { llm?: ILlmClient };
export class FinanceRetentionAgent {
  constructor(private readonly deps: FinanceRetentionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Retención clientes financieros "${enriched.companyName}" (${enriched.serviceType}, ${enriched.targetClient}, tono ${enriched.tone}):
- Revisión anual cartera/pólizas: email convocatoria + agenda reuniones placeholders.
- Alertas renovación con valor educativo antes de fecha límite.
- Informe semestral rendimiento personalizado plantilla secciones (sin prometer rentabilidades futuras).
- Cross-sell ético cadencia hipoteca→hogar→vida→pensión solo si encaja perfil cliente.
- Comunicación proactiva cambios mercado/legislación afectando al cliente con CTA revisión.
Disclaimer idoneidad y datos personales pseudonimizados en ejemplos.`;
    const __crmOut = { agentId: "finance-retention", result: await this.llm.complete(prompt, financeLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let financeRetentionCached: FinanceRetentionAgent | undefined;
export function getFinanceRetentionAgent(): FinanceRetentionAgent { if (!financeRetentionCached) financeRetentionCached = new FinanceRetentionAgent(); return financeRetentionCached; }
export function resetFinanceRetentionAgentForTests(): void { financeRetentionCached = undefined; }
