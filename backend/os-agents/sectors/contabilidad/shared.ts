...
export interface ContabilidadInput {
  userId: string;
  sector: string;
  brand: string;
  contabilidadBrief?: string;
  metricsBrief?: string;
  metadata?: Record<string, unknown>;
}

export interface ContabilidadOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseContabilidadLlmJson(raw: string, label: string): Omit<ContabilidadOutput, "agentId"> {
...
export function buildContabilidadPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const contCtx = params.input.contabilidadBrief?.trim() ? params.input.contabilidadBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

CONTABILIDAD NELVYON OS (v1):
- **Prefijo agentes**: **Contabilidad**; finanzas y fiscalidad.
- **Registro facturas <30 s** por documento; **conciliación bancaria 100% automática**.
- **Declaraciones fiscales 195 países** automáticas; **cash flow RT <5 min**.
- **Cierre mensual <10 min** automático; **anomalías/fraude <1 h**.
...
export async function runContabilidadAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: ContabilidadInput,
  temperature: number,
): Promise<ContabilidadOutput> {
  const prompt = buildContabilidadPrompt({
  ...
  const parsed = parseContabilidadLlmJson(raw, agentId);
  const out: ContabilidadOutput = { agentId, ...parsed };
  try {
...
export function getDefaultContabilidadLlm(): ILlmClient {
