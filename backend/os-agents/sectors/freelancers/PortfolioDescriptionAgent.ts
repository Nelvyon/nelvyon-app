...
export type PortfolioDescriptionAgentDeps = { llm?: ILlmClient };
export class PortfolioDescriptionAgent {
  constructor(private readonly deps: PortfolioDescriptionAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera descripciones de portfolio para ${enriched.specialty} con formato STAR (Situation, Task, Action, Result): problema, solución aplicada, resultado medible (%, €, tiempo), aprendizaje clave. Público objetivo ${enriched.targetClient}.`;
    const __crmOut = { agentId: "portfolio-description", result: await this.llm.complete(prompt, llmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let cached: PortfolioDescriptionAgent | undefined;
export function getPortfolioDescriptionAgent(): PortfolioDescriptionAgent { if (!cached) cached = new PortfolioDescriptionAgent(); return cached; }
export function resetPortfolioDescriptionAgentForTests(): void { cached = undefined; }
