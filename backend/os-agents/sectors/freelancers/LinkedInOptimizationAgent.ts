...
export type LinkedInOptimizationAgentDeps = { llm?: ILlmClient };
export class LinkedInOptimizationAgent {
  constructor(private readonly deps: LinkedInOptimizationAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Optimiza perfil LinkedIn para ${enriched.professionalName} (${enriched.specialty}) para captar ${enriched.targetClient}. Entrega headline con keywords, about con hook+historia+CTA, featured recomendado, 5 posts de autoridad, hashtags y estrategia engagement.`;
    const __crmOut = { agentId: "linkedin-optimization", result: await this.llm.complete(prompt, llmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let cached: LinkedInOptimizationAgent | undefined;
export function getLinkedInOptimizationAgent(): LinkedInOptimizationAgent { if (!cached) cached = new LinkedInOptimizationAgent(); return cached; }
export function resetLinkedInOptimizationAgentForTests(): void { cached = undefined; }
