...
export type RateJustificationAgentDeps = { llm?: ILlmClient };
export class RateJustificationAgent {
  constructor(private readonly deps: RateJustificationAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea argumentario premium para justificar tarifas de ${enriched.professionalName} (${enriched.specialty}) frente a ${enriched.targetClient}: ROI vs coste, comparación con opciones baratas, objeciones de precio y reencuadre inversión.`;
    const __crmOut = { agentId: "rate-justification", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: RateJustificationAgent | undefined;
export function getRateJustificationAgent(): RateJustificationAgent { if (!cached) cached = new RateJustificationAgent(); return cached; }
export function resetRateJustificationAgentForTests(): void { cached = undefined; }
