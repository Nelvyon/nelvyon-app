...
export type ClientProposalAgentDeps = { llm?: ILlmClient };
export class ClientProposalAgent {
  constructor(private readonly deps: ClientProposalAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera propuesta comercial personalizada para ${enriched.specialty} de ${enriched.professionalName}: resumen ejecutivo, diagnóstico, solución por fases, entregables, timeline, inversión básica/estándar/premium, garantías y CTA final para ${enriched.targetClient}.`;
    const __crmOut = { agentId: "client-proposal", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: ClientProposalAgent | undefined;
export function getClientProposalAgent(): ClientProposalAgent { if (!cached) cached = new ClientProposalAgent(); return cached; }
export function resetClientProposalAgentForTests(): void { cached = undefined; }
