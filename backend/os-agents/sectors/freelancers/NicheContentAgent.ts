...
export type NicheContentAgentDeps = { llm?: ILlmClient };
export class NicheContentAgent {
  constructor(private readonly deps: NicheContentAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea contenido de autoridad para ${enriched.specialty}: 10 ideas de posts, estructura de artículo, lead magnet, template newsletter semanal, hilo X educativo y FAQ para posicionar experto ante ${enriched.targetClient}.`;
    const __crmOut = { agentId: "niche-content", result: await this.llm.complete(prompt, llmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let cached: NicheContentAgent | undefined;
export function getNicheContentAgent(): NicheContentAgent { if (!cached) cached = new NicheContentAgent(); return cached; }
export function resetNicheContentAgentForTests(): void { cached = undefined; }
