...
export type FreelancerEmailSequenceAgentDeps = { llm?: ILlmClient };
export class FreelancerEmailSequenceAgent {
  constructor(private readonly deps: FreelancerEmailSequenceAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Genera secuencia nurturing de 5 emails para leads de ${enriched.specialty}: bienvenida+valor, caso éxito, objeción común resuelta, prueba social, oferta con urgencia real. Tono ${enriched.tone}, cliente objetivo ${enriched.targetClient}.`;
    const __crmOut = { agentId: "freelancer-email-sequence", result: await this.llm.complete(prompt, llmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let cached: FreelancerEmailSequenceAgent | undefined;
export function getFreelancerEmailSequenceAgent(): FreelancerEmailSequenceAgent { if (!cached) cached = new FreelancerEmailSequenceAgent(); return cached; }
export function resetFreelancerEmailSequenceAgentForTests(): void { cached = undefined; }
