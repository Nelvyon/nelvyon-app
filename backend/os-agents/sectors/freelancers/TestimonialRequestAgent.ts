...
export type TestimonialRequestAgentDeps = { llm?: ILlmClient };
export class TestimonialRequestAgent {
  constructor(private readonly deps: TestimonialRequestAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Crea mensajes para solicitar testimonios a clientes de ${enriched.professionalName}: email post-proyecto, versión WhatsApp/LinkedIn, preguntas guía para testimonio potente y template de caso de éxito web. Especialidad ${enriched.specialty}.`;
    const __crmOut = { agentId: "testimonial-request", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: TestimonialRequestAgent | undefined;
export function getTestimonialRequestAgent(): TestimonialRequestAgent { if (!cached) cached = new TestimonialRequestAgent(); return cached; }
export function resetTestimonialRequestAgentForTests(): void { cached = undefined; }
