...
export type ReferralSystemAgentDeps = { llm?: ILlmClient };
export class ReferralSystemAgent {
  constructor(private readonly deps: ReferralSystemAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Diseña sistema de referidos para ${enriched.professionalName} (${enriched.specialty}): mensaje para pedir referidos, seguimiento post-proyecto, incentivo recomendado, script networking online/offline y tracking manual de referidos.`;
    const __crmOut = { agentId: "referral-system", result: await this.llm.complete(prompt, llmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let cached: ReferralSystemAgent | undefined;
export function getReferralSystemAgent(): ReferralSystemAgent { if (!cached) cached = new ReferralSystemAgent(); return cached; }
export function resetReferralSystemAgentForTests(): void { cached = undefined; }
