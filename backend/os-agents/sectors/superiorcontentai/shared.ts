...
export interface SuperiorContentAIInput {
  userId: string;
  sector: string;
  brand: string;
  contentBrief?: string;
...
export interface SuperiorContentAIOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseSuperiorContentAILlmJson(raw: string, label: string): Omit<SuperiorContentAIOutput, "agentId"> {
...
export function buildSuperiorContentAIPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: SuperiorContentAIInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const content = params.input.contentBrief?.trim() ? params.input.contentBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

SUPERIORIDAD CONTENIDO IA NELVYON OS (v1):
- **Prefijo agentes**: **SuperiorContentAI**; estrategia, redacción y repurposing con voz de marca.
- **Artículo 2000 palabras <30s**; **E-E-A-T >90/100**; repurposing **6 formatos <60s**.
- **Traducción 20+ idiomas <45s**; engagement **>5%**; **0% contenido genérico**.
...
export async function runSuperiorContentAIAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: SuperiorContentAIInput,
  temperature: number,
): Promise<SuperiorContentAIOutput> {
  const eliteRole = resolveSpecialtyElitePrompt(
    agentId,
    {
      sector: input.sector,
      brand: input.brand,
      businessContext: input.contentBrief,
      campaignGoal: "estrategia de contenido",
    },
    params.eliteRole,
  );
  const prompt = buildSuperiorContentAIPrompt({
    eliteRole,
  ...
  const parsed = parseSuperiorContentAILlmJson(raw, agentId);
  const out: SuperiorContentAIOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSuperiorContentAILlm(): ILlmClient {
