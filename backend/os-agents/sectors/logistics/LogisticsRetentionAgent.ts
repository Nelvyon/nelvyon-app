...
export type LogisticsRetentionAgentDeps = { llm?: ILlmClient };
export class LogisticsRetentionAgent {
  constructor(private readonly deps: LogisticsRetentionAgentDeps = {}) {}
  ...
  async run(userId: string, input: LogisticsInput): Promise<LogisticsOutput> {
    ...
    const cov = enriched.coverage ? ` Red operativa: ${enriched.coverage}.` : "";
...
### BRIEF OPERATIVO
Retención clientes logísticos "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}).${cov}
- Informe mensual rendimiento KPIs entregas tiempo incidencias coste envío placeholders.
- Email revisión trimestral contrato mejoras continuas SLA.
- Programa descuento volumen tramos ejemplo condiciones claras compliance.
- Playbook alerta proactiva incidencias antes reclamación canales comunicación.
- Newsletter novedades servicio mejoras producto roadmap suave.
- Upsell almacenaje seguro tracking premium argumentos sin presión engañosa.`;
    const __crmOut = { agentId: "logistics-retention", result: await this.llm.complete(prompt, logisticsLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let logisticsRetentionCached: LogisticsRetentionAgent | undefined;
export function getLogisticsRetentionAgent(): LogisticsRetentionAgent {
  if (!logisticsRetentionCached) logisticsRetentionCached = new LogisticsRetentionAgent();
  return logisticsRetentionCached;
}
export function resetLogisticsRetentionAgentForTests(): void { logisticsRetentionCached = undefined; }
