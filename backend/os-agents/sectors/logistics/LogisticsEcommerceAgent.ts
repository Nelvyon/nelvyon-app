...
export type LogisticsEcommerceAgentDeps = { llm?: ILlmClient };
export class LogisticsEcommerceAgent {
  constructor(private readonly deps: LogisticsEcommerceAgentDeps = {}) {}
  ...
  async run(userId: string, input: LogisticsInput): Promise<LogisticsOutput> {
    ...
    const cov = enriched.coverage ? ` Cobertura envíos: ${enriched.coverage}.` : "";
...
### BRIEF OPERATIVO
Estrategia fulfillment ecommerce "${enriched.businessName}" (${enriched.serviceType}, público ${enriched.targetClient}, ${enriched.tone}).${cov}
- Propuesta valor tiendas online: devoluciones plazos integraciones plataforma sin prometer mismo día universal.
- Comparativa orientativa vs logística propia tablas ejemplo descargo legal.
- Email captación Shopify WooCommerce PrestaShop tono partnership.
- Lead magnet guía externalizar logística checklist pasos.
- Contenidos posicionar partner logístico pilares SEO social.
- Casos éxito con métricas placeholder [DATOS_AUDITADOS].`;
    const __crmOut = { agentId: "logistics-ecommerce", result: await this.llm.complete(prompt, logisticsLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let logisticsEcommerceCached: LogisticsEcommerceAgent | undefined;
export function getLogisticsEcommerceAgent(): LogisticsEcommerceAgent {
  if (!logisticsEcommerceCached) logisticsEcommerceCached = new LogisticsEcommerceAgent();
  return logisticsEcommerceCached;
}
export function resetLogisticsEcommerceAgentForTests(): void { logisticsEcommerceCached = undefined; }
