...
export type LogisticsReviewSystemAgentDeps = { llm?: ILlmClient };
export class LogisticsReviewSystemAgent {
  constructor(private readonly deps: LogisticsReviewSystemAgentDeps = {}) {}
  ...
  async run(userId: string, input: LogisticsInput): Promise<LogisticsOutput> {
    ...
    const cov = enriched.coverage ? ` Cobertura mencionada: ${enriched.coverage}.` : "";
...
### BRIEF OPERATIVO
Reseñas reputación logística "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}).${cov}
- Mensaje post-servicio Google email WhatsApp B2C vs email formal B2B RGPD cortesía.
- Plantillas respuesta positiva negativa enfoque resolución incidentes mercancías retrasos.
- Estrategia casos delicados indemnización proceso transparente revisión jurídica sugerida.
- Objetivo reseñas mensual segmentado tipo cliente métricas internas ejemplo.
- Presencia Google Trustpilot directorios especializados consistencia datos NAP breve.`;
    const __crmOut = { agentId: "logistics-review-system", result: await this.llm.complete(prompt, logisticsLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let logisticsReviewSystemCached: LogisticsReviewSystemAgent | undefined;
export function getLogisticsReviewSystemAgent(): LogisticsReviewSystemAgent {
  if (!logisticsReviewSystemCached) logisticsReviewSystemCached = new LogisticsReviewSystemAgent();
  return logisticsReviewSystemCached;
}
export function resetLogisticsReviewSystemAgentForTests(): void { logisticsReviewSystemCached = undefined; }
