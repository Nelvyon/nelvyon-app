...
export type LogisticsAdsAgentDeps = { llm?: ILlmClient };
export class LogisticsAdsAgent {
  constructor(private readonly deps: LogisticsAdsAgentDeps = {}) {}
  ...
  async run(userId: string, input: LogisticsInput): Promise<LogisticsOutput> {
    ...
    const cov = enriched.coverage ? ` Geo/target: ${enriched.coverage}.` : "";
...
### BRIEF OPERATIVO
Paid media logística "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}).${cov}
- Google Search B2B alta intención grupos palabras ejemplo carga completa 3PL fulfillment.
- LinkedIn cargos Ops supply chain industrias ecommerce food ejemplos formularios.
- Retargeting landing tarifa abandonos límites frecuencia.
- Display branding verticales industria ecommerce alimentación creatividades ejemplo.
Disclaimer cumplimiento políticas anuncios y datos reales.`;
    const __crmOut = { agentId: "logistics-ads", result: await this.llm.complete(prompt, logisticsLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let logisticsAdsCached: LogisticsAdsAgent | undefined;
export function getLogisticsAdsAgent(): LogisticsAdsAgent {
  if (!logisticsAdsCached) logisticsAdsCached = new LogisticsAdsAgent();
  return logisticsAdsCached;
}
export function resetLogisticsAdsAgentForTests(): void { logisticsAdsCached = undefined; }
