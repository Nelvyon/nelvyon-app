...
export interface HelpDeskOmnichannelInput {
  userId: string;
  sector: string;
  brand: string;
  helpdeskBrief?: string;
  metricsBrief?: string;
  metadata?: Record<string, unknown>;
}

export interface HelpDeskOmnichannelOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseHelpDeskOmnichannelLlmJson(raw: string, label: string): Omit<HelpDeskOmnichannelOutput, "agentId"> {
...
export function buildHelpDeskOmnichannelPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: HelpDeskOmnichannelInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const helpdeskCtx = params.input.helpdeskBrief?.trim() ? params.input.helpdeskBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

HELPDESK OMNICHANNEL NELVYON OS (v1):
- **Prefijo agentes**: **HelpDeskOmnichannel**; soporte omnicanal unificado.
- **FRT <2 min**; resolución auto **>75%**; **CSAT >4.7/5**; **0% tickets perdidos**.
- **Omnicanal**: email, chat, WhatsApp, voz, RRSS; **SLA en tiempo real**.
...
export async function runHelpDeskOmnichannelAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: HelpDeskOmnichannelInput,
  temperature: number,
): Promise<HelpDeskOmnichannelOutput> {
  const prompt = buildHelpDeskOmnichannelPrompt({
  ...
  const parsed = parseHelpDeskOmnichannelLlmJson(raw, agentId);
  const out: HelpDeskOmnichannelOutput = { agentId, ...parsed };
  try {
...
export function getDefaultHelpDeskOmnichannelLlm(): ILlmClient {
