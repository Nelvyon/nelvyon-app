...
export function parseSalesIntelligenceLlmJson(
  raw: string,
  label: string,
): Omit<SalesIntelligenceOutput, "agentId"> {
...
export function buildSalesIntelligencePrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: SalesIntelligenceInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const intelCtx = params.input.salesIntelligenceBrief?.trim()
    ? params.input.salesIntelligenceBrief.trim()
...
SALES INTELLIGENCE + BUYER INTENT NELVYON OS (v1):
- **Prefijo agentes**: **SalesIntelligence**; inteligencia comercial y buyer intent.
- **Buyer intent <2 min** desde señal; **account scoring accuracy >88%**.
- **Prospecto 360° <60 s**; **deal risk detection >85%** precisión.
- **Sales cycle reduction >35%** vs sin inteligencia; **0 intervención humana** en research y scoring.
...
- Sales intelligence / contexto: ${intelCtx}
...
export async function runSalesIntelligenceAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  const prompt = buildSalesIntelligencePrompt({
  ...
  const parsed = parseSalesIntelligenceLlmJson(raw, agentId);
  const out: SalesIntelligenceOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSalesIntelligenceLlm(): ILlmClient {
