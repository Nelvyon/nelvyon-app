...
export interface SessionReplayInput {
  ...
  sector: string;
  brand: string;
  sessionReplayBrief?: string;
  metricsBrief?: string;
  metadata?: Record<string, unknown>;
}

export interface SessionReplayOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseSessionReplayLlmJson(raw: string, label: string): Omit<SessionReplayOutput, "agentId"> {
...
export function buildSessionReplayPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const replayCtx = params.input.sessionReplayBrief?.trim() ? params.input.sessionReplayBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

SESSION REPLAY + HEATMAPS NELVYON OS (v1):
- **Prefijo agentes**: **SessionReplay**; replay de sesión y heatmaps.
- **Grabación <1% CPU** impacto; **heatmap <60 s** tras visita.
- **Rage click detectado <30 s**; **GDPR anonimización 100%** automática.
- **Insights accionables <5 min**; **datos sensibles enmascarados 100%** antes de almacenar.
...
- Session replay / contexto: ${replayCtx}
...
export async function runSessionReplayAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: SessionReplayInput,
  temperature: number,
): Promise<SessionReplayOutput> {
  const prompt = buildSessionReplayPrompt({
  ...
  const parsed = parseSessionReplayLlmJson(raw, agentId);
  const out: SessionReplayOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSessionReplayLlm(): ILlmClient {
