...
export type HashtagStrategyInput = { niche: string; platform: string; postTopic: string };
export type HashtagStrategyResult = { strategy: Array<{ bucket: "nicho" | "medio" | "grande"; tags: string[]; estimatedReach: string }> };
export type HashtagStrategyAgentDeps = { llm?: ILlmClient };
export class HashtagStrategyAgent {
  constructor(private readonly deps: HashtagStrategyAgentDeps = {}) {}
  private get llm(): ILlmClient { return this.deps.llm ?? LlmClient.getInstance(); }
  async generateStrategy(userId: string, input: HashtagStrategyInput): Promise<HashtagStrategyResult> {
...
OUTPUT FORMAT: Respuesta ÚNICAMENTE como JSON válido UTF-8, sin markdown ni texto antes/después. Esquema: {"strategy":[{"bucket":"nicho","tags":[],"estimatedReach":""}]}
...
### BRIEF OPERATIVO
Crea estrategia de hashtags por bucket para ${enriched.platform}, nicho ${enriched.niche}, tema ${enriched.postTopic}. SOLO JSON {"strategy":[{"bucket":"nicho","tags":[],"estimatedReach":""}]}`;
    return parseJson<HashtagStrategyResult>(await this.llm.complete(prompt, llmOpts(0.2)), "HashtagStrategyAgent");
  }
}
let cached: HashtagStrategyAgent | undefined;
export function getHashtagStrategyAgent(): HashtagStrategyAgent { if (!cached) cached = new HashtagStrategyAgent(); return cached; }
export function resetHashtagStrategyAgentForTests(): void { cached = undefined; }
