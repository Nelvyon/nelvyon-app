...
export type CoachingAdsAgentDeps = { llm?: ILlmClient };
export class CoachingAdsAgent {
  constructor(private readonly deps: CoachingAdsAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Paid media coaching "${enriched.expertName}" (${enriched.niche}, ${enriched.targetAudience}, tono ${enriched.tone}${enriched.productType ? ", producto " + enriched.productType : ""}):
- Meta frío/tibio/caliente webinar gratuito copy + creatividad briefing.
- Lead magnet intereses nicho sin income claims.
- Retargeting visitantes sales page no compra frecuencia prudente.
- Lanzamiento fases preventa apertura urgencia cierre anuncios sugeridos.
- Presupuesto diario rango por fase placeholder.
Políticas Meta: sin ingresos garantizados ni testimonios engañosos; cumplir normativa local.`;
    const __crmOut = { agentId: "coaching-ads", result: await this.llm.complete(prompt, coachingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let coachingAdsCached: CoachingAdsAgent | undefined;
export function getCoachingAdsAgent(): CoachingAdsAgent { if (!coachingAdsCached) coachingAdsCached = new CoachingAdsAgent(); return coachingAdsCached; }
export function resetCoachingAdsAgentForTests(): void { coachingAdsCached = undefined; }
