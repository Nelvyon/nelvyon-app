...
export type CoachingTestimonialSystemAgentDeps = { llm?: ILlmClient };
export class CoachingTestimonialSystemAgent {
  constructor(private readonly deps: CoachingTestimonialSystemAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Sistema testimonios "${enriched.expertName}" (${enriched.niche}, ${enriched.targetAudience}):
- Preguntas entrevista antes/durante/después sin presionar frases inventadas.
- Email solicitud post-programa con permiso RGPD y uso imagen.
- Guión vídeo testimonio 60-90s estructura.
- Mapeo testimonio → objeción que desmonta (sin datos sensibles).
- Formatos sales page RRSS ads citas cortas.
- Release firmado plantilla bullets (no asesoramiento legal sustitutivo).
Tono comunicación coach: ${enriched.tone}.`;
    const __crmOut = { agentId: "coaching-testimonial-system", result: await this.llm.complete(prompt, coachingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let coachingTestimonialSystemCached: CoachingTestimonialSystemAgent | undefined;
export function getCoachingTestimonialSystemAgent(): CoachingTestimonialSystemAgent { if (!coachingTestimonialSystemCached) coachingTestimonialSystemCached = new CoachingTestimonialSystemAgent(); return coachingTestimonialSystemCached; }
export function resetCoachingTestimonialSystemAgentForTests(): void { coachingTestimonialSystemCached = undefined; }
