...
export type CoachingWebinarAgentDeps = { llm?: ILlmClient };
export class CoachingWebinarAgent {
  constructor(private readonly deps: CoachingWebinarAgentDeps = {}) {}
  ...
  async run(userId: string, input: CoachingInput): Promise<CoachingOutput> {
    ...
    const pt = enriched.productType ? ` Pitch producto tipo: ${enriched.productType}.` : "";
...
### BRIEF OPERATIVO
Guión webinar ventas "${enriched.expertName}" (${enriched.niche}, audiencia ${enriched.targetAudience}, tono ${enriched.tone}).${pt}
- Apertura promesa tiempo X min sin hype ilegal.
- Credenciales con humildad.
- Bloque valor 3 ideas accionables.
- Puente ético a oferta.
- Presentación programa precio bonos garantía ejemplos.
- Objeciones tiempo dinero resultado (sin garantizar resultado).
- Cierre CTA cupos fecha real.
- Email post-webinar no compradores recap + invitación dudas.
Marcar necesidad revisión jurídica según país.`;
    const __crmOut = { agentId: "coaching-webinar", result: await this.llm.complete(prompt, coachingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let coachingWebinarCached: CoachingWebinarAgent | undefined;
export function getCoachingWebinarAgent(): CoachingWebinarAgent { if (!coachingWebinarCached) coachingWebinarCached = new CoachingWebinarAgent(); return coachingWebinarCached; }
export function resetCoachingWebinarAgentForTests(): void { coachingWebinarCached = undefined; }
