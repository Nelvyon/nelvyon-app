...
export type CoachingUpsellFunnelAgentDeps = { llm?: ILlmClient };
export class CoachingUpsellFunnelAgent {
  constructor(private readonly deps: CoachingUpsellFunnelAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Funnel upsell LTV "${enriched.expertName}" (${enriched.niche}, ${enriched.targetAudience}, tono ${enriched.tone}):
- Escalera gratuito → tripwire → core → high ticket → mastermind con precios placeholder.
- Email upsell post-compra entrada timing 24-72h.
- Order bump copy checkout congruente.
- Downsell alternativa si rechaza upsell principal.
- Secuencia post-fin programa continuidad comunidad/mantenimiento.
- High ticket VSL outline + script aplicación calificación sin prometer ingresos.
${enriched.productType ? `Producto actual foco: ${enriched.productType}.` : ""}`;
    const __crmOut = { agentId: "coaching-upsell-funnel", result: await this.llm.complete(prompt, coachingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let coachingUpsellFunnelCached: CoachingUpsellFunnelAgent | undefined;
export function getCoachingUpsellFunnelAgent(): CoachingUpsellFunnelAgent { if (!coachingUpsellFunnelCached) coachingUpsellFunnelCached = new CoachingUpsellFunnelAgent(); return coachingUpsellFunnelCached; }
export function resetCoachingUpsellFunnelAgentForTests(): void { coachingUpsellFunnelCached = undefined; }
