...
export type CoachingLeadMagnetAgentDeps = { llm?: ILlmClient };
export class CoachingLeadMagnetAgent {
  constructor(private readonly deps: CoachingLeadMagnetAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Lead magnet "${enriched.expertName}" (${enriched.niche}, ${enriched.targetAudience}, ${enriched.tone}${enriched.productType ? ", alineado a " + enriched.productType : ""}):
- 5 ideas magnet (quiz guía masterclass plantilla calculadora) con ángulo nicho.
- Guión masterclass 60 min bloques tiempo interacción CTA final.
- Esquema PDF 10-15 páginas índice por capítulo.
- Secuencia post opt-in: entrega + 3 valor + oferta suave GDPR double opt-in sugerido.
- Landing opt-in headline bullets form campos mínimos.
- Distribución orgánica/paid por canal sin prometer resultados.`;
    const __crmOut = { agentId: "coaching-lead-magnet", result: await this.llm.complete(prompt, coachingLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let coachingLeadMagnetCached: CoachingLeadMagnetAgent | undefined;
export function getCoachingLeadMagnetAgent(): CoachingLeadMagnetAgent { if (!coachingLeadMagnetCached) coachingLeadMagnetCached = new CoachingLeadMagnetAgent(); return coachingLeadMagnetCached; }
export function resetCoachingLeadMagnetAgentForTests(): void { coachingLeadMagnetCached = undefined; }
