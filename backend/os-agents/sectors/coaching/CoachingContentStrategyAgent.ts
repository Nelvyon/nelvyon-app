...
export type CoachingContentStrategyAgentDeps = { llm?: ILlmClient };
export class CoachingContentStrategyAgent {
  constructor(private readonly deps: CoachingContentStrategyAgentDeps = {}) {}
  ...
  async run(userId: string, input: CoachingInput): Promise<CoachingOutput> {
    ...
    const pt = enriched.productType ? ` Oferta relacionada: ${enriched.productType}.` : "";
...
### BRIEF OPERATIVO
Estrategia contenido "${enriched.expertName}" (${enriched.niche}, ${enriched.targetAudience}, tono ${enriched.tone}).${pt}
- 30 ideas posts LinkedIn/IG/YouTube mezcla educación storytelling CTA suave.
- Estructura vídeo YouTube autoridad + captación.
- Hilo X/Twitter educativo con gancho y CTA lead magnet.
- Podcast: temas invitados frecuencia promoción cruzada.
- Calendario mensual mix gratuito/captación/venta.
- Fórmula educar → conectar → convertir sin promesas engañosas.`;
    const __crmOut = { agentId: "coaching-content-strategy", result: await this.llm.complete(prompt, coachingLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let coachingContentStrategyCached: CoachingContentStrategyAgent | undefined;
export function getCoachingContentStrategyAgent(): CoachingContentStrategyAgent { if (!coachingContentStrategyCached) coachingContentStrategyCached = new CoachingContentStrategyAgent(); return coachingContentStrategyCached; }
export function resetCoachingContentStrategyAgentForTests(): void { coachingContentStrategyCached = undefined; }
