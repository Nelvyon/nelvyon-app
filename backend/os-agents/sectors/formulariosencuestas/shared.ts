...
export interface FormulariosEncuestasInput {
  userId: string;
  sector: string;
  brand: string;
  formBrief?: string;
...
export interface FormulariosEncuestasOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseFormulariosEncuestasLlmJson(
  raw: string,
  label: string,
): Omit<FormulariosEncuestasOutput, "agentId"> {
...
export function buildFormulariosEncuestasPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: FormulariosEncuestasInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const formCtx = params.input.formBrief?.trim() ? params.input.formBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

FORMULARIOS + ENCUESTAS INTELIGENTES NELVYON OS (v1):
- **Prefijo agentes**: **FormulariosEncuestas**; formularios y encuestas inteligentes.
- **Formulario <2 min**; completion **>65%** vs 30% industria; NPS segmentado en **tiempo real**.
- **CRM <5s** por respuesta; sentimiento abierto **>90%**; **A/B testing** automático.
...
export async function runFormulariosEncuestasAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: FormulariosEncuestasInput,
  temperature: number,
): Promise<FormulariosEncuestasOutput> {
  const prompt = buildFormulariosEncuestasPrompt({
  ...
  const parsed = parseFormulariosEncuestasLlmJson(raw, agentId);
  const out: FormulariosEncuestasOutput = { agentId, ...parsed };
  try {
...
export function getDefaultFormulariosEncuestasLlm(): ILlmClient {
