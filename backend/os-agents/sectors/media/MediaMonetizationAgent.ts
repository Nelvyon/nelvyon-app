...
export type MediaMonetizationAgentDeps = { llm?: ILlmClient };
export class MediaMonetizationAgent {
  constructor(private readonly deps: MediaMonetizationAgentDeps = {}) {}
...
    const aud = enriched.audienceSize != null ? ` Audiencia estimada: ${enriched.audienceSize}.` : "";
    const fmt = enriched.format ? ` Soporte principal: ${enriched.format}.` : "";
    const tn = enriched.tone ? ` Tono comunicación: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
Modelos monetización medios/newsletters Nicho "${enriched.niche}". Ángulo: ${enriched.topic}.${aud}${fmt}${tn}
- Patrocinio afiliados producto premium paywall ejemplo ético.
- Paquetes anunciante rate card placeholder revisión jurídica impuestos.
- Secuencias venta programa miembros prueba gratuita disclaimers FTC/UE ejemplo.`;
    const __crmOut = { agentId: "media-monetization", result: await this.llm.complete(prompt, mediaLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let mediaMonetizationCached: MediaMonetizationAgent | undefined;
export function getMediaMonetizationAgent(): MediaMonetizationAgent {
  if (!mediaMonetizationCached) mediaMonetizationCached = new MediaMonetizationAgent();
  return mediaMonetizationCached;
}
export function resetMediaMonetizationAgentForTests(): void { mediaMonetizationCached = undefined; }
