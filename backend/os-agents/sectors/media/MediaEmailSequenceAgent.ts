...
export type MediaEmailSequenceAgentDeps = { llm?: ILlmClient };
export class MediaEmailSequenceAgent {
  constructor(private readonly deps: MediaEmailSequenceAgentDeps = {}) {}
...
    const aud = enriched.audienceSize != null ? ` Segmento tamaño ref: ${enriched.audienceSize}.` : "";
    const fmt = enriched.format ? ` Contexto producto: ${enriched.format}.` : "";
    const tn = enriched.tone ? ` Tono secuencia: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
Secuencia email 5-7 toques Nicho "${enriched.niche}". Objetivo embudo: ${enriched.topic}.${aud}${fmt}${tn}
- Día 0 bienvenida valor D2 historia D4 prueba social D6 oferta suave ejemplo.
- Asuntos variaciones A/B unsubscribe legal RGPD.
- P.S. y enlaces tracking placeholder.`;
    const __crmOut = { agentId: "media-email-sequence", result: await this.llm.complete(prompt, mediaLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let mediaEmailSequenceCached: MediaEmailSequenceAgent | undefined;
export function getMediaEmailSequenceAgent(): MediaEmailSequenceAgent {
  if (!mediaEmailSequenceCached) mediaEmailSequenceCached = new MediaEmailSequenceAgent();
  return mediaEmailSequenceCached;
}
export function resetMediaEmailSequenceAgentForTests(): void { mediaEmailSequenceCached = undefined; }
