...
export type MediaPodcastScriptAgentDeps = { llm?: ILlmClient };
export class MediaPodcastScriptAgent {
  constructor(private readonly deps: MediaPodcastScriptAgentDeps = {}) {}
...
    const aud = enriched.audienceSize != null ? ` Oyentes objetivo ref: ${enriched.audienceSize}.` : "";
    const tn = enriched.tone ? ` Tono host: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
Guión episodio podcast Nicho "${enriched.niche}". Episodio sobre: ${enriched.topic}.${aud}${tn}
- Cold open teaser intro música placeholder.
- Bloques tiempo invitado preguntas mid roll patrocinio ejemplo transparencia.
- Outro suscripción show notes timestamps CTA newsletter.`;
    const __crmOut = { agentId: "media-podcast-script", result: await this.llm.complete(prompt, mediaLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let mediaPodcastScriptCached: MediaPodcastScriptAgent | undefined;
export function getMediaPodcastScriptAgent(): MediaPodcastScriptAgent {
  if (!mediaPodcastScriptCached) mediaPodcastScriptCached = new MediaPodcastScriptAgent();
  return mediaPodcastScriptCached;
}
export function resetMediaPodcastScriptAgentForTests(): void { mediaPodcastScriptCached = undefined; }
