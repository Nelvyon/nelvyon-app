...
export type MediaAudienceGrowthAgentDeps = { llm?: ILlmClient };
export class MediaAudienceGrowthAgent {
  constructor(private readonly deps: MediaAudienceGrowthAgentDeps = {}) {}
...
    const aud = enriched.audienceSize != null ? ` Base actual: ${enriched.audienceSize}.` : "";
    const fmt = enriched.format ? ` Canal: ${enriched.format}.` : "";
    const tn = enriched.tone ? ` Tono marca: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
Estrategia crecimiento audiencia média/newsletter Nicho "${enriched.niche}". Foco tema: ${enriched.topic}.${aud}${fmt}${tn}
- Embudos landing lead magnet ejemplo.
- Colaboraciones cross promos placeholders.
- Consistencia cadencia sin prometer cifras inventadas.
- Métricas a seguir experimentos A/B.`;
    const __crmOut = { agentId: "media-audience-growth", result: await this.llm.complete(prompt, mediaLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let mediaAudienceGrowthCached: MediaAudienceGrowthAgent | undefined;
export function getMediaAudienceGrowthAgent(): MediaAudienceGrowthAgent {
  if (!mediaAudienceGrowthCached) mediaAudienceGrowthCached = new MediaAudienceGrowthAgent();
  return mediaAudienceGrowthCached;
}
export function resetMediaAudienceGrowthAgentForTests(): void { mediaAudienceGrowthCached = undefined; }
