...
export type MediaSeoArticlesAgentDeps = { llm?: ILlmClient };
export class MediaSeoArticlesAgent {
  constructor(private readonly deps: MediaSeoArticlesAgentDeps = {}) {}
...
    const aud = enriched.audienceSize != null ? ` Tráfico potencial ref: ${enriched.audienceSize}.` : "";
    const fmt = enriched.format ? ` Formato artículo: ${enriched.format}.` : "";
    const tn = enriched.tone ? ` Tono redacción: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
Artículo blog SEO blueprint Nicho "${enriched.niche}". Keyword/tema pillar: ${enriched.topic}.${aud}${fmt}${tn}
- H1 outline H2s meta title description internos FAQ schema sugerencia.
- Snippet destacado FAQs People Also Ask placeholders.
- CTA newsletter lead magnet relacionado ejemplo.`;
    const __crmOut = { agentId: "media-seo-articles", result: await this.llm.complete(prompt, mediaLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let mediaSeoArticlesCached: MediaSeoArticlesAgent | undefined;
export function getMediaSeoArticlesAgent(): MediaSeoArticlesAgent {
  if (!mediaSeoArticlesCached) mediaSeoArticlesCached = new MediaSeoArticlesAgent();
  return mediaSeoArticlesCached;
}
export function resetMediaSeoArticlesAgentForTests(): void { mediaSeoArticlesCached = undefined; }
