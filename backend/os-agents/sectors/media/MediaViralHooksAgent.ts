...
export type MediaViralHooksAgentDeps = { llm?: ILlmClient };
export class MediaViralHooksAgent {
  constructor(private readonly deps: MediaViralHooksAgentDeps = {}) {}
...
    const fmt = enriched.format ? ` Plataforma enfoque: ${enriched.format}.` : "";
    const tn = enriched.tone ? ` Estilo viral: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
20 hooks virales formato corto Nicho "${enriched.niche}". Tema contenido: ${enriched.topic}.${fmt}${tn}
- Primera línea retención 3 seg curiosidad sin clickbait engañoso.
- Variaciones pregunta dato historia contradicción ejemplo.
- Hashtags sugeridos nota comunidad etiquetas marca.`;
    const __crmOut = { agentId: "media-viral-hooks", result: await this.llm.complete(prompt, mediaLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let mediaViralHooksCached: MediaViralHooksAgent | undefined;
export function getMediaViralHooksAgent(): MediaViralHooksAgent {
  if (!mediaViralHooksCached) mediaViralHooksCached = new MediaViralHooksAgent();
  return mediaViralHooksCached;
}
export function resetMediaViralHooksAgentForTests(): void { mediaViralHooksCached = undefined; }
