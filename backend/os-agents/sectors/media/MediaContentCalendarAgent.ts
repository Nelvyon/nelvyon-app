...
export type MediaContentCalendarAgentDeps = { llm?: ILlmClient };
export class MediaContentCalendarAgent {
  constructor(private readonly deps: MediaContentCalendarAgentDeps = {}) {}
...
    const aud = enriched.audienceSize != null ? ` Escala audiencia ref: ${enriched.audienceSize}.` : "";
    const fmt = enriched.format ? ` Formato contenido: ${enriched.format}.` : "";
    const tn = enriched.tone ? ` Tono editorial: ${enriched.tone}.` : "";
...
### BRIEF OPERATIVO
Calendario contenido editorial 4 semanas Nicho "${enriched.niche}". Hilación tema: ${enriched.topic}.${aud}${fmt}${tn}
- Tabla semanal día canal pieza KPI sugerido.
- Bloques repurposing newsletter a blog social podcast.
- Hitos temporada campañas coordinadas placeholder.`;
    const __crmOut = { agentId: "media-content-calendar", result: await this.llm.complete(prompt, mediaLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let mediaContentCalendarCached: MediaContentCalendarAgent | undefined;
export function getMediaContentCalendarAgent(): MediaContentCalendarAgent {
  if (!mediaContentCalendarCached) mediaContentCalendarCached = new MediaContentCalendarAgent();
  return mediaContentCalendarCached;
}
export function resetMediaContentCalendarAgentForTests(): void { mediaContentCalendarCached = undefined; }
