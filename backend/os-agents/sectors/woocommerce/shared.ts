...
/** Segmentación WooCommerce NELVYON */
export type WooCommerceBuyerSegment = "one_time" | "recurring" | "vip";

export interface WooCommerceInput {
  userId: string;
  sector: string;
  brand: string;
  /** URL tienda (orientativo) */
  storeUrl?: string;
  segment?: WooCommerceBuyerSegment | string;
  metricsBrief?: string;
  metadata?: Record<string, unknown>;
}

export interface WooCommerceOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseWooCommerceLlmJson(raw: string, label: string): Omit<WooCommerceOutput, "agentId"> {
...
export function buildWooCommercePrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: WooCommerceInput;
}): string {
...
INTEGRACIÓN WOOCOMMERCE NELVYON (v1):
- **REST API WooCommerce**: Consumer Key / Consumer Secret (HTTPS); solo referencias seguras en prompts.
- **Carrito abandonado**: secuencia **3 emails** (**1h**, **24h**, **72h**) con descuento progresivo (**5%**, **10%**, **15%**).
- **Post-compra**: email **confirmación** + **upsell producto relacionado** + solicitud **review a los 7 días**.
- **SEO productos**: **title < 60 caracteres**, **description < 160 caracteres**, **schema JSON-LD** tipo **Product** + **Review** cuando aplique.
...
export async function runWooCommerceAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: WooCommerceInput,
  temperature: number,
): Promise<WooCommerceOutput> {
  const prompt = buildWooCommercePrompt({
  ...
  const parsed = parseWooCommerceLlmJson(raw, agentId);
  const out: WooCommerceOutput = { agentId, ...parsed };
  try {
...
export function getDefaultWooCommerceLlm(): ILlmClient {
