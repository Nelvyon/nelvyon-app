...
export interface SuperiorLeadEnrichmentInput {
  userId: string;
  sector: string;
  brand: string;
  leadBrief?: string;
...
export interface SuperiorLeadEnrichmentOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseSuperiorLeadEnrichmentLlmJson(
  raw: string,
  label: string,
): Omit<SuperiorLeadEnrichmentOutput, "agentId"> {
...
export function buildSuperiorLeadEnrichmentPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: SuperiorLeadEnrichmentInput;
}): string {
...
SUPERIORIDAD LEAD ENRICHMENT NELVYON OS (v1):
- **Prefijo agentes**: **SuperiorLeadEnrichment**; enriquecimiento, scoring y routing B2B.
- **Enriquecimiento completo <3s** por lead; cobertura **>85%** campos B2B.
- **Email verification >98%**; **ICP scoring >90%**; routing automático; datos **<24h** staleness.
...
export async function runSuperiorLeadEnrichmentAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: SuperiorLeadEnrichmentInput,
  temperature: number,
): Promise<SuperiorLeadEnrichmentOutput> {
  const prompt = buildSuperiorLeadEnrichmentPrompt({
  ...
  const parsed = parseSuperiorLeadEnrichmentLlmJson(raw, agentId);
  const out: SuperiorLeadEnrichmentOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSuperiorLeadEnrichmentLlm(): ILlmClient {
