...
export type AutoAdsAgentDeps = { llm?: ILlmClient };
export class AutoAdsAgent {
  constructor(private readonly deps: AutoAdsAgentDeps = {}) {}
  ...
  async run(userId: string, input: AutoInput): Promise<AutoOutput> {
    ...
    const loc = enriched.location ? ` Geo: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Paid media automoción "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}, tono ${enriched.tone}).${loc}
- Google Search: keywords alta intención compra usados, taller aceite urgente, alquiler weekend.
- Meta: ángulos copy + briefing creatividad por vehículo placeholder.
- Retargeting visitantes fichas sin conversión audiencias GA4 ejemplo.
- Display branding local conser/taller tamaños típicos.
- Presupuestos orientativos por estacionalidad (matrículaciones, vacaciones, invierno).
Cumple políticas meta/google vehículos y transparencia precio donde aplique.`;
    const __crmOut = { agentId: "auto-ads", result: await this.llm.complete(prompt, autoLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let autoAdsCached: AutoAdsAgent | undefined;
export function getAutoAdsAgent(): AutoAdsAgent { if (!autoAdsCached) autoAdsCached = new AutoAdsAgent(); return autoAdsCached; }
export function resetAutoAdsAgentForTests(): void { autoAdsCached = undefined; }
