...
export type AutoSeasonalCampaignAgentDeps = { llm?: ILlmClient };
export class AutoSeasonalCampaignAgent {
  constructor(private readonly deps: AutoSeasonalCampaignAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Campañas estacionales automoción "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}, ${enriched.tone}, zona ${enriched.location ?? "multi"}):
Por campaña incluye objetivo público canal copy CTA KPI sugerido:
1) Verano revisión prevacaciones neumático verano.
2) Invierno neumáticos frío batería anticongelante.
3) Picos matriculaciones marzo/septiembre promos nuevo/usado.
4) Fin año stock ocasión financiación (disclaimers Banco España).
5) Black Friday taller accesorios bundles.
Adapta bullets si negocio es solo rent-a-car u otro tipo.`;
    const __crmOut = { agentId: "auto-seasonal-campaign", result: await this.llm.complete(prompt, autoLlmOpts(0.9)), generatedAt: new Date().toISOString() };
...
let autoSeasonalCampaignCached: AutoSeasonalCampaignAgent | undefined;
export function getAutoSeasonalCampaignAgent(): AutoSeasonalCampaignAgent { if (!autoSeasonalCampaignCached) autoSeasonalCampaignCached = new AutoSeasonalCampaignAgent(); return autoSeasonalCampaignCached; }
export function resetAutoSeasonalCampaignAgentForTests(): void { autoSeasonalCampaignCached = undefined; }
