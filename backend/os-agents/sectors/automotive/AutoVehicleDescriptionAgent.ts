...
export type AutoVehicleDescriptionAgentDeps = { llm?: ILlmClient };
export class AutoVehicleDescriptionAgent {
  constructor(private readonly deps: AutoVehicleDescriptionAgentDeps = {}) {}
  ...
  async run(userId: string, input: AutoInput): Promise<AutoOutput> {
    ...
    const loc = enriched.location ? ` Mercado ref: ${enriched.location}.` : "";
...
### BRIEF OPERATIVO
Ficha venta vehículo para "${enriched.businessName}" (${enriched.businessType}, comprador ideal ${enriched.targetClient}, tono ${enriched.tone}).${loc}
Genera estructura con placeholders [MARCA] [MODELO] [AÑO] [KM] [COMBUSTIBLE]:
- Título SEO marca/modelo/año/versión.
- Descripción storytelling estado y propuesta valor.
- Especificaciones técnicas traducidas a beneficios.
- Mantenimiento destacado (ITV, revisiones) genérico verificable.
- Extras con valor estimado € placeholder.
- Precio justificado vs mercado (sin afirmar datos reales).
- CTA con disponibilidad prudente (sin presión engañosa).
Incluir aviso verificar documentación e historial oficial.`;
    const __crmOut = { agentId: "auto-vehicle-description", result: await this.llm.complete(prompt, autoLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let autoVehicleDescriptionCached: AutoVehicleDescriptionAgent | undefined;
export function getAutoVehicleDescriptionAgent(): AutoVehicleDescriptionAgent { if (!autoVehicleDescriptionCached) autoVehicleDescriptionCached = new AutoVehicleDescriptionAgent(); return autoVehicleDescriptionCached; }
export function resetAutoVehicleDescriptionAgentForTests(): void { autoVehicleDescriptionCached = undefined; }
