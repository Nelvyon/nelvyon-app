...
export type AutoLocalSEOAgentDeps = { llm?: ILlmClient };
export class AutoLocalSEOAgent {
  constructor(private readonly deps: AutoLocalSEOAgentDeps = {}) {}
  ...
  async run(userId: string, input: AutoInput): Promise<AutoOutput> {
    ...
    const loc = enriched.location ?? "ciudad principal España";
...
### BRIEF OPERATIVO
SEO local automoción "${enriched.businessName}" (${enriched.businessType}, ${enriched.targetClient}, zona ${loc}):
- Keywords por negocio y ciudad (concesionario, taller, compraventa, rent a car ejemplos).
- GMB: categorías, descripción, ideas fotos interior/exterior taller/stock.
- Meta titles/descriptions por servicio (5 páginas tipo).
- JSON-LD AutoDealer / AutoRepair / RentACarDealer placeholders.
- Estrategia reseñas y respuestas rápidas.
- Coches.net Autocasion directorios talleres tácticas listado.`;
    const __crmOut = { agentId: "auto-local-seo", result: await this.llm.complete(prompt, autoLlmOpts(0.2)), generatedAt: new Date().toISOString() };
...
let autoLocalSEOCached: AutoLocalSEOAgent | undefined;
export function getAutoLocalSEOAgent(): AutoLocalSEOAgent { if (!autoLocalSEOCached) autoLocalSEOCached = new AutoLocalSEOAgent(); return autoLocalSEOCached; }
export function resetAutoLocalSEOAgentForTests(): void { autoLocalSEOCached = undefined; }
