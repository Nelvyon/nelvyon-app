...
export function parseCustomerSuccessLlmJson(
  raw: string,
  label: string,
): Omit<CustomerSuccessOutput, "agentId"> {
...
export function buildCustomerSuccessPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const csCtx = params.input.customerSuccessBrief?.trim()
    ? params.input.customerSuccessBrief.trim()
...
CUSTOMER SUCCESS + HEALTH SCORES NELVYON OS (v1):
- **Prefijo agentes**: **CustomerSuccess**; éxito del cliente y health scores.
- **Health score RT <5 min**; **churn prediction accuracy >90%**.
- **Time to first value <24 h** tras signup; **expansión revenue >25%** base clientes/mes.
- **NPS >70** (industria SaaS 41); **0 intervención humana** en onboarding ni seguimiento.
...
- Customer success / contexto: ${csCtx}
...
export async function runCustomerSuccessAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: CustomerSuccessInput,
  temperature: number,
): Promise<CustomerSuccessOutput> {
  const prompt = buildCustomerSuccessPrompt({
  ...
  const parsed = parseCustomerSuccessLlmJson(raw, agentId);
  const out: CustomerSuccessOutput = { agentId, ...parsed };
  try {
...
export function getDefaultCustomerSuccessLlm(): ILlmClient {
