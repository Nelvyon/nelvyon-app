...
export interface ChatWidgetOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseChatWidgetLlmJson(raw: string, label: string): Omit<ChatWidgetOutput, "agentId"> {
...
export function buildChatWidgetPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const chatCtx = params.input.chatBrief?.trim() ? params.input.chatBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

CHAT WIDGET INTELIGENTE NELVYON OS (v1):
- **Prefijo agentes**: **ChatWidget**; widget de chat inteligente 24/7.
- **Respuesta <2s**; lead capture **>25%**; resolución auto **>80%** sin humano.
- **Personalización** desde primera visita; **CSAT >4.5/5** en conversaciones automáticas.
...
export async function runChatWidgetAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: ChatWidgetInput,
  temperature: number,
): Promise<ChatWidgetOutput> {
  const prompt = buildChatWidgetPrompt({
  ...
  const parsed = parseChatWidgetLlmJson(raw, agentId);
  const out: ChatWidgetOutput = { agentId, ...parsed };
  try {
...
export function getDefaultChatWidgetLlm(): ILlmClient {
