...
export interface FunnelMultipasoInput {
  userId: string;
  sector: string;
  brand: string;
  funnelBrief?: string;
...
export interface FunnelMultipasoOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseFunnelMultipasoLlmJson(raw: string, label: string): Omit<FunnelMultipasoOutput, "agentId"> {
...
export function buildFunnelMultipasoPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: FunnelMultipasoInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const funnelCtx = params.input.funnelBrief?.trim() ? params.input.funnelBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

FUNNEL MULTI-PASO COMPLETO NELVYON OS (v1):
- **Prefijo agentes**: **FunnelMultipaso**; funnels multi-paso de conversión.
- **Funnel completo <5 min**; conversión total **>3%** tráfico frío; email abandono **>15%** recuperación.
- **A/B automático** por paso; revenue por visitante **RT**; retargeting **<1h** tras abandono.
...
export async function runFunnelMultipasoAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: FunnelMultipasoInput,
  temperature: number,
): Promise<FunnelMultipasoOutput> {
  const prompt = buildFunnelMultipasoPrompt({
  ...
  const parsed = parseFunnelMultipasoLlmJson(raw, agentId);
  const out: FunnelMultipasoOutput = { agentId, ...parsed };
  try {
...
export function getDefaultFunnelMultipasoLlm(): ILlmClient {
