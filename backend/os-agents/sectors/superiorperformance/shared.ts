...
export interface SuperiorPerformanceInput {
  userId: string;
  sector: string;
  brand: string;
...
export interface SuperiorPerformanceOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseSuperiorPerformanceLlmJson(raw: string, label: string): Omit<SuperiorPerformanceOutput, "agentId"> {
...
export function buildSuperiorPerformancePrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: SuperiorPerformanceInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const performance = params.input.performanceBrief?.trim() ? params.input.performanceBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

SUPERIORIDAD PERFORMANCE NELVYON OS (v1):
- **Prefijo agentes**: **SuperiorPerformance**; Core Web Vitals, caché y optimización.
- **LCP <1s**; **CLS <0.05**, **INP <100ms**, **TTFB <200ms**; PageSpeed **>95** móvil y desktop.
- **Auditoría <30s**; mejoras automáticas; alertas degradación **<2 min**.
...
- Performance / contexto: ${performance}
...
export async function runSuperiorPerformanceAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: SuperiorPerformanceInput,
  temperature: number,
): Promise<SuperiorPerformanceOutput> {
  const prompt = buildSuperiorPerformancePrompt({
  ...
  const parsed = parseSuperiorPerformanceLlmJson(raw, agentId);
  const out: SuperiorPerformanceOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSuperiorPerformanceLlm(): ILlmClient {
