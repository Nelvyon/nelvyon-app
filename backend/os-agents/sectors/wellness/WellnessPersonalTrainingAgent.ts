...
export type WellnessPersonalTrainingAgentDeps = { llm?: ILlmClient };
export class WellnessPersonalTrainingAgent {
  constructor(private readonly deps: WellnessPersonalTrainingAgentDeps = {}) {}
  ...
  async run(userId: string, input: WellnessInput): Promise<WellnessOutput> {
    ...
    const spec = enriched.specialization ? ` Nicho: ${enriched.specialization}.` : "";
...
### BRIEF OPERATIVO
Marketing entrenador personal / boutique "${enriched.businessName}" (${enriched.serviceType}, cliente ${enriched.targetClient}, tono ${enriched.tone}).${spec}
- Propuesta de valor personal bio credenciales.
- Paquetes: sueltas bono10 mensual online con precios placeholder.
- Argumentario valor vs low-cost ético sin desprestigiar competencia.
- Secuencia captación IG/LinkedIn posts + DM plantilla inicial.
- Sistema reporting progreso para cliente formato PDF mensual ejemplo.
- Upsell nutrición + PT disclaimer no dietista si no cualificado.`;
    const __crmOut = { agentId: "wellness-personal-training", result: await this.llm.complete(prompt, wellnessLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let wellnessPersonalTrainingCached: WellnessPersonalTrainingAgent | undefined;
export function getWellnessPersonalTrainingAgent(): WellnessPersonalTrainingAgent { if (!wellnessPersonalTrainingCached) wellnessPersonalTrainingCached = new WellnessPersonalTrainingAgent(); return wellnessPersonalTrainingCached; }
export function resetWellnessPersonalTrainingAgentForTests(): void { wellnessPersonalTrainingCached = undefined; }
