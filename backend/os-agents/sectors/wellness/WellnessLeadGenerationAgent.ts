...
export type WellnessLeadGenerationAgentDeps = { llm?: ILlmClient };
export class WellnessLeadGenerationAgent {
  constructor(private readonly deps: WellnessLeadGenerationAgentDeps = {}) {}
  ...
  async run(userId: string, input: WellnessInput): Promise<WellnessOutput> {
    ...
    const spec = enriched.specialization ? ` Mensaje axe: ${enriched.specialization}.` : "";
...
### BRIEF OPERATIVO
Captación leads wellness "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}).${spec}
- Landing clase/membership prueba gratis formulario campos y copy.
- Google Search local gym entrenador yoga principiantes ejemplos keywords.
- Meta creatividades transformación solo con permiso y políticas.
- Reto 21/30 días lead magnet estructura semanal y CTA.
- B2B wellness empleados pitch corporativo breve.
- Campaña enero propósitos calendario anuncios + oferta.
Sin promesas médicas ni antes/después engañosos.`;
    const __crmOut = { agentId: "wellness-lead-generation", result: await this.llm.complete(prompt, wellnessLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let wellnessLeadGenerationCached: WellnessLeadGenerationAgent | undefined;
export function getWellnessLeadGenerationAgent(): WellnessLeadGenerationAgent { if (!wellnessLeadGenerationCached) wellnessLeadGenerationCached = new WellnessLeadGenerationAgent(); return wellnessLeadGenerationCached; }
export function resetWellnessLeadGenerationAgentForTests(): void { wellnessLeadGenerationCached = undefined; }
