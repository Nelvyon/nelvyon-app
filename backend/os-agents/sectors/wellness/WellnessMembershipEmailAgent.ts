...
export type WellnessMembershipEmailAgentDeps = { llm?: ILlmClient };
export class WellnessMembershipEmailAgent {
  constructor(private readonly deps: WellnessMembershipEmailAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Secuencia email socios "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}):
Redacta esquema con asunto + bullets cuerpo para:
1) Bienvenida nuevo socio y cómo sacar partido membresía.
2) Semana 1 check-in y recursos inicio.
3) Mes 1 celebración pequeña hito + siguiente objetivo saludable.
4) Renovación -30 días beneficios loyalty oferta placeholder.
5) Reactivación baja asistencia win-back prudente.
6) Referido socios activos incentivo revisable legalmente.
RGPD opt-out; sin prometer resultados corporales fijos.`;
    const __crmOut = { agentId: "wellness-membership-email", result: await this.llm.complete(prompt, wellnessLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let wellnessMembershipEmailCached: WellnessMembershipEmailAgent | undefined;
export function getWellnessMembershipEmailAgent(): WellnessMembershipEmailAgent { if (!wellnessMembershipEmailCached) wellnessMembershipEmailCached = new WellnessMembershipEmailAgent(); return wellnessMembershipEmailCached; }
export function resetWellnessMembershipEmailAgentForTests(): void { wellnessMembershipEmailCached = undefined; }
