...
export type WellnessRetentionAgentDeps = { llm?: ILlmClient };
export class WellnessRetentionAgent {
  constructor(private readonly deps: WellnessRetentionAgentDeps = {}) {}
  ...
  async run(userId: string, input: WellnessInput): Promise<WellnessOutput> {
    ...
    const extra = enriched.specialization ? ` Especialización foco: ${enriched.specialization}.` : "";
...
### BRIEF OPERATIVO
Retención socios largo plazo "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, ${enriched.tone}).${extra}
- Hitos 1m/3m/6m/1a insignias y comunicaciones push/email.
- Puntos por asistencia niveles premios ejemplo merchandising clase invitación.
- Recuperación 2-4 semanas sin venir email amable no culpabilizar.
- Programa buddy beneficio mutual consentido RGPD referral amigo.
- Encuesta satisfacción NPS corta acciones follow-up.
- Evento mensual exclusivo masterclass charla bienvenida invitados pago.`;
    const __crmOut = { agentId: "wellness-retention", result: await this.llm.complete(prompt, wellnessLlmOpts(0.5)), generatedAt: new Date().toISOString() };
...
let wellnessRetentionCached: WellnessRetentionAgent | undefined;
export function getWellnessRetentionAgent(): WellnessRetentionAgent { if (!wellnessRetentionCached) wellnessRetentionCached = new WellnessRetentionAgent(); return wellnessRetentionCached; }
export function resetWellnessRetentionAgentForTests(): void { wellnessRetentionCached = undefined; }
