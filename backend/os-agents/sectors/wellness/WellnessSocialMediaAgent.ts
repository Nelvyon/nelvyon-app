...
export type WellnessSocialMediaAgentDeps = { llm?: ILlmClient };
export class WellnessSocialMediaAgent {
  constructor(private readonly deps: WellnessSocialMediaAgentDeps = {}) {}
  ...
  async run(userId: string, input: WellnessInput): Promise<WellnessOutput> {
    ...
    const spec = enriched.specialization ? ` Enfoque: ${enriched.specialization}.` : "";
...
### BRIEF OPERATIVO
Estrategia RRSS fitness/bienestar "${enriched.businessName}" (${enriched.serviceType}, ${enriched.targetClient}, tono ${enriched.tone}).${spec}
- 30 ideas contenido: rutinas, nutrición general, BTS centro, motivación (transformaciones solo con consentimiento explícito).
- Captions + hashtags por especialidad.
- Plan Reels/TikTok hooks y frecuencia.
- Calendario editorial mensual tabla sugerida.
- Stories encuestas CTA prueba gratuita.
- Colaboraciones micro-influencers salud (brief ético).
Cumple políticas Meta sin afirmaciones médicas.`;
    const __crmOut = { agentId: "wellness-social-media", result: await this.llm.complete(prompt, wellnessLlmOpts(0.7)), generatedAt: new Date().toISOString() };
...
let wellnessSocialMediaCached: WellnessSocialMediaAgent | undefined;
export function getWellnessSocialMediaAgent(): WellnessSocialMediaAgent { if (!wellnessSocialMediaCached) wellnessSocialMediaCached = new WellnessSocialMediaAgent(); return wellnessSocialMediaCached; }
export function resetWellnessSocialMediaAgentForTests(): void { wellnessSocialMediaCached = undefined; }
