...
export type WellnessCorporateWellnessAgentDeps = { llm?: ILlmClient };
export class WellnessCorporateWellnessAgent {
  constructor(private readonly deps: WellnessCorporateWellnessAgentDeps = {}) {}
...
### BRIEF OPERATIVO
Bienestar corporativo B2B "${enriched.businessName}" (oferta tipo ${enriched.serviceType}, segmento cliente ${enriched.targetClient}, tono ${enriched.tone}):
- Propuesta valor empresas: absentismo foco bienestar evidencia cauta sin promesas legales laborales.
- Deck presentación bullets ROI ejemplificado placeholders horas pérdidas.
- Email outreach RRHH/directores asunto preview valor.
- Programa niveles básico/estándar/premium inclusión webinars meditación ergonomía.
- Convenio empresa template bullets confidencialidad RGPD talleres onsite.
- LinkedIn contenido decisores artículos cortos comentarios.
${enriched.specialization ? `Temática empresa: ${enriched.specialization}.` : ""}`;
    const __crmOut = { agentId: "wellness-corporate-wellness", result: await this.llm.complete(prompt, wellnessLlmOpts(0.4)), generatedAt: new Date().toISOString() };
...
let wellnessCorporateWellnessCached: WellnessCorporateWellnessAgent | undefined;
export function getWellnessCorporateWellnessAgent(): WellnessCorporateWellnessAgent { if (!wellnessCorporateWellnessCached) wellnessCorporateWellnessCached = new WellnessCorporateWellnessAgent(); return wellnessCorporateWellnessCached; }
export function resetWellnessCorporateWellnessAgentForTests(): void { wellnessCorporateWellnessCached = undefined; }
