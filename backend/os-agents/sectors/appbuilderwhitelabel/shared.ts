...
export interface AppBuilderWhiteLabelInput {
  userId: string;
  sector: string;
  brand: string;
  appBrief?: string;
...
export interface AppBuilderWhiteLabelOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseAppBuilderWhiteLabelLlmJson(
  raw: string,
  label: string,
): Omit<AppBuilderWhiteLabelOutput, "agentId"> {
...
export function buildAppBuilderWhiteLabelPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: AppBuilderWhiteLabelInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const appCtx = params.input.appBrief?.trim() ? params.input.appBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

APP BUILDER WHITE-LABEL NELVYON OS (v1):
- **Prefijo agentes**: **AppBuilderWhiteLabel**; apps white-label sin código.
- **App funcional <10 min**; publicación stores **<48h**; **PWA día 1**; branding **100%** personalizable.
- **OTA sin downtime**; **crash rate <0.1%**.
...
export async function runAppBuilderWhiteLabelAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: AppBuilderWhiteLabelInput,
  temperature: number,
): Promise<AppBuilderWhiteLabelOutput> {
  const prompt = buildAppBuilderWhiteLabelPrompt({
  ...
  const parsed = parseAppBuilderWhiteLabelLlmJson(raw, agentId);
  const out: AppBuilderWhiteLabelOutput = { agentId, ...parsed };
  try {
...
export function getDefaultAppBuilderWhiteLabelLlm(): ILlmClient {
