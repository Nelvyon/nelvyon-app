...
export interface SuperiorSeoInput {
  userId: string;
  sector: string;
  brand: string;
  seoBrief?: string;
...
export interface SuperiorSeoOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseSuperiorSeoLlmJson(raw: string, label: string): Omit<SuperiorSeoOutput, "agentId"> {
...
export function buildSuperiorSeoPrompt(params: {
  eliteRole: string;
  mission: string;
  fewShotExample: string;
  input: SuperiorSeoInput;
}): string {
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const seo = params.input.seoBrief?.trim() ? params.input.seoBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

SUPERIORIDAD SEO NELVYON OS (v1):
- **Prefijo agentes**: **SuperiorSeo**; SEO técnico, contenido y autoridad.
- **Top 3** en keywords objetivo en **<90 días**.
- **CTR orgánico >8%**; informes de posicionamiento **diarios** automatizados.
- **Core Web Vitals**: **LCP <1.5s**, **CLS <0.05**, **INP <100ms**.
- **Cobertura semántica >95%** de entidades del topic; **cannibalization** y **gaps** automáticos.
...
export async function runSuperiorSeoAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: SuperiorSeoInput,
  temperature: number,
): Promise<SuperiorSeoOutput> {
  const prompt = buildSuperiorSeoPrompt({
  ...
  const parsed = parseSuperiorSeoLlmJson(raw, agentId);
  const out: SuperiorSeoOutput = { agentId, ...parsed };
  try {
...
export function getDefaultSuperiorSeoLlm(): ILlmClient {
