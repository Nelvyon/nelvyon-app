...
export interface DatabaseImportOutput {
  agentId: string;
  content: string;
  score: number;
  highlights: string[];
...
export function parseDatabaseImportLlmJson(raw: string, label: string): Omit<DatabaseImportOutput, "agentId"> {
...
export function buildDatabaseImportPrompt(params: {
  eliteRole: string;
  mission: string;
  ...
  const meta =
    params.input.metadata && Object.keys(params.input.metadata).length > 0
      ...
      : "{}";
  const importCtx = params.input.importBrief?.trim() ? params.input.importBrief.trim() : "inferir desde sector";

  return `${params.eliteRole}

${ELITE_V300_STANDARDS}

DATABASE IMPORT + SEGMENTACIÓN NELVYON OS (v1):
- **Prefijo agentes**: **DatabaseImport**; importación masiva y segmentación.
- **1M registros <60s**; validación **100%**; deduplicación **>99%**.
- **Enriquecimiento >85%**; segmentación **<10s**; formatos CSV, Excel, JSON, XML, Google Sheets.
...
export async function runDatabaseImportAgentCore(
  agentId: string,
  llm: ILlmClient,
  params: {
    eliteRole: string;
  ...
  input: DatabaseImportInput,
  temperature: number,
): Promise<DatabaseImportOutput> {
  const prompt = buildDatabaseImportPrompt({
  ...
  const parsed = parseDatabaseImportLlmJson(raw, agentId);
  const out: DatabaseImportOutput = { agentId, ...parsed };
  try {
...
export function getDefaultDatabaseImportLlm(): ILlmClient {
