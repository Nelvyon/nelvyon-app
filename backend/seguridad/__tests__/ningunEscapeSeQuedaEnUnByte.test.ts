 ...
 *
 * EL ERROR, QUE HA MORDIDO CUATRO VECES. Cuando se escribe una expresión
 ...
 *
 * Tres de los cuatro casos los produje yo en esta sesión. Ninguno dio error.
 * Los tres se descubrieron por el comportamiento raro que causaban, no por
 * leerlos — porque leyéndolos no se ven.
 *
 * POR QUÉ UNA PRUEBA Y NO «TENER CUIDADO». Tener cuidado ya se intentó tres
 * veces. Un byte invisible es exactamente la clase de cosa que una revisión
 * humana no puede atrapar, y una máquina sí, en un segundo.
 *
...
const FUERA = /(^docs\/|__tests__|\.test\.|\.spec\.|^apps\/web\/e2e\/)/;
