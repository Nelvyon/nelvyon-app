...
async function waitForDeploy() {
  ...
  console.log("Waiting for staging deploy (C1 /publicidad + BFF)…");
  for (let i = 1; i <= 40; i += 1) {
    try {
      ...
      const page = await fetch(`${BASE}/publicidad`, { cache: "no-store", redirect: "manual" });
      const bffOk = bff.status !== 404 && bff.status !== 0;
      const pageExists = page.status !== 404;
      console.log(JSON.stringify({ attempt: i, health: health.status, adsBff: bff.status, publicidad: page.status }));
...
async function runSmoke(token, workspaceId) {
  console.log("\n=== C1 Ads — pages ===");
  await probePage("C1", "/publicidad hub", "/publicidad", token, workspaceId, {
    contains: ["__next"],
  });
  await probePage("C1", "/publicidad/google", "/publicidad/google", token, workspaceId);
  await probePage("C1", "/publicidad/meta", "/publicidad/meta", token, workspaceId);
  await probePage("C1", "/publicidad/briefing", "/publicidad/briefing", token, workspaceId, {
    contains: ["Briefing"],
  });
  await probePage("C1", "/analytics/publicidad", "/analytics/publicidad", token, workspaceId, {
    contains: ["Publicidad", "ROAS"],
  });
  await probePage("C1", "redirect /saas/dashboard/publicidad", "/saas/dashboard/publicidad", token, workspaceId, {
    redirectTo: "/publicidad",
  });
  ...
  const unified = await probeApi(
    ...
    "/api/platform/ads/reporting/unified",
    token,
    workspaceId,
    [200],
  );
  await probeApi("C1", "API google status", "/api/platform/ads/google/status", token, workspaceId, [200]);
  await probeApi("C1", "API meta status", "/api/platform/ads/meta/status", token, workspaceId, [200]);
  const alerts = await probeApi("C1", "API roas alerts", "/api/platform/ads/alerts/roas", token, workspaceId, [200]);

  if (unified && typeof unified.unified !== "object") {
    fail("C1", "unified shape", "missing unified block");
  } else if (unified) {
    pass("C1", "unified shape", `spend=${unified.unified?.total_spend ?? 0}`);
  }
  if (alerts && !Array.isArray(alerts.alerts)) {
    fail("C1", "alerts shape", "expected alerts array");
  }
}

async function main() {
  console.log(`Staging smoke C1 Ads → ${BASE}\n`);
  await waitForDeploy();
