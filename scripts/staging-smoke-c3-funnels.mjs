/**
 * Staging smoke test — FASE C élite (C3 Funnels).
 * Usage: node scripts/staging-smoke-c3-funnels.mjs [--skip-wait]
 */
...
async function waitForDeploy() {
  ...
  console.log("Waiting for staging deploy (C3 /funnels + BFF)…");
  for (let i = 1; i <= 40; i += 1) {
    try {
      const health = await fetch(`${BASE}/api/health/live`, { cache: "no-store" });
      const bff = await fetch(`${BASE}/api/platform/funnels/reporting/unified`, { cache: "no-store" });
      const page = await fetch(`${BASE}/funnels`, { cache: "no-store", redirect: "manual" });
      const bffOk = bff.status !== 404 && bff.status !== 0;
      const pageExists = page.status !== 404;
      console.log(
        JSON.stringify({ attempt: i, health: health.status, funnelsBff: bff.status, funnelsPage: page.status }),
      );
...
async function runSmoke(token, workspaceId) {
  console.log("\n=== C3 Funnels — pages ===");
  await probePage("C3", "/funnels hub", "/funnels", token, workspaceId, {
    contains: ["Embudos", "conversión"],
  });
  await probePage("C3", "/funnels/builder", "/funnels/builder", token, workspaceId, {
    contains: ["Builder", "Anuncio"],
  });
  await probePage("C3", "/analytics/funnels", "/analytics/funnels", token, workspaceId, {
    contains: ["Embudos", "conversión"],
  });
  await probePage("C3", "redirect /dashboard/funnels", "/dashboard/funnels", token, workspaceId, {
    redirectTo: "/funnels",
  });
  ...
  const unified = await probeApi(
    ...
    "/api/platform/funnels/reporting/unified",
    token,
    workspaceId,
    [200],
  );
  await probeApi("C3", "API list", "/api/platform/funnels", token, workspaceId, [200]);

  if (unified && typeof unified.unified !== "object") {
    fail("C3", "unified shape", "missing unified block");
  } else if (unified) {
    pass("C3", "unified shape", `funnels=${unified.unified?.total_funnels ?? 0}`);
  }
}

async function main() {
  console.log(`Staging smoke C3 Funnels → ${BASE}\n`);
  await waitForDeploy();
