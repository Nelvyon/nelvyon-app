/**
 * Staging E2E — automations-ops-pack + reputation-ops-pack (ADR-055).
 * Same mesh/portal/QA>=85/auditor pattern as `staging-smoke-beta-packs-e2e.mjs`.
 * Requires staging flags ON: NELVYON_AUTOMATIONS_OPS_PACK=1 / NELVYON_REPUTATION_OPS_PACK=1
 * (or staging/dev runtime, which enables them by default per `osPackFlags.ts`).
 * Usage: node scripts/staging-smoke-automations-reputation-e2e.mjs [--skip-wait] [--only=automations|reputation]
 */
...
async function main() {
  console.log(`=== Automations + Reputation packs E2E [${BASE}] ===`);
  if (!SKIP_WAIT) {
    await waitForStagingDeploy(BASE, { skipWait: false, label: "automations-reputation-e2e" });
