/**
 * Staging smoke test — FASE C élite (C5 Automations).
 * Usage: node scripts/staging-smoke-c5-automations.mjs [--skip-wait]
 */
...
async function waitForDeploy() {
  ...
  console.log("Waiting for staging deploy (C5 /automatizacion + BFF)…");
  for (let i = 1; i <= 40; i += 1) {
    try {
      const health = await fetch(`${BASE}/api/health/live`, { cache: "no-store" });
      const bff = await fetch(`${BASE}/api/platform/automations/reporting/unified`, { cache: "no-store" });
      const page = await fetch(`${BASE}/automatizacion`, { cache: "no-store", redirect: "manual" });
      ...
      console.log(
        JSON.stringify({
          ...
          automationsBff: bff.status,
          automatizacionPage: page.status,
        }),
      );
...
async function runSmoke(token, workspaceId) {
  console.log("\n=== C5 Automations — pages ===");
  await probePage("C5", "/automatizacion hub", "/automatizacion", token, workspaceId, {
    contains: ["Automatización", "workflow"],
  });
  await probePage("C5", "/automatizacion/flujos", "/automatizacion/flujos", token, workspaceId, {
    contains: ["Flujos", "automatización"],
  });
  await probePage("C5", "/automatizacion/recetas", "/automatizacion/recetas", token, workspaceId, {
    contains: ["Recetas", "onboarding"],
  });
  await probePage("C5", "/analytics/automatizacion", "/analytics/automatizacion", token, workspaceId, {
    contains: ["Automatización", "eventos"],
  });
  await probePage("C5", "redirect /dashboard/workflows", "/dashboard/workflows", token, workspaceId, {
    redirectTo: "/automatizacion/flujos",
  });
  ...
  await probeApi("C5", "API workflows", "/api/platform/automations/workflows", token, workspaceId, [200]);
  await probeApi("C5", "API rules", "/api/platform/automations/rules", token, workspaceId, [200]);

  if (unified && typeof unified.unified !== "object") {
    fail("C5", "unified shape", "missing unified block");
  } else if (unified) {
    pass("C5", "unified shape", `flows=${unified.unified?.total_flows ?? 0}`);
  }
}

async function main() {
  console.log(`Staging smoke C5 Automations → ${BASE}\n`);
  await waitForDeploy();
