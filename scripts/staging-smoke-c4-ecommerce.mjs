/**
 * Staging smoke test — FASE C élite (C4 Ecommerce).
 * Usage: node scripts/staging-smoke-c4-ecommerce.mjs [--skip-wait]
 */
...
async function waitForDeploy() {
  ...
  console.log("Waiting for staging deploy (C4 /ecommerce + BFF)…");
  for (let i = 1; i <= 40; i += 1) {
    try {
      const health = await fetch(`${BASE}/api/health/live`, { cache: "no-store" });
      const bff = await fetch(`${BASE}/api/platform/ecommerce/reporting/unified`, { cache: "no-store" });
      const page = await fetch(`${BASE}/ecommerce`, { cache: "no-store", redirect: "manual" });
      const bffOk = bff.status !== 404 && bff.status !== 0;
      const pageExists = page.status !== 404;
      console.log(
        JSON.stringify({ attempt: i, health: health.status, ecommerceBff: bff.status, ecommercePage: page.status }),
      );
...
async function runSmoke(token, workspaceId) {
  console.log("\n=== C4 Ecommerce — pages ===");
  await probePage("C4", "/ecommerce hub", "/ecommerce", token, workspaceId, {
    contains: ["Ecommerce", "checkout"],
  });
  await probePage("C4", "/ecommerce/editor", "/ecommerce/editor", token, workspaceId, {
    contains: ["Editor", "tienda"],
  });
  await probePage("C4", "/analytics/ecommerce", "/analytics/ecommerce", token, workspaceId, {
    contains: ["Ecommerce", "carrito"],
  });
  await probePage("C4", "redirect /dashboard/stores", "/dashboard/stores", token, workspaceId, {
  ...
  const unified = await probeApi(
    ...
    "/api/platform/ecommerce/reporting/unified",
    token,
    workspaceId,
    [200],
  );
  await probeApi("C4", "API list projects", "/api/platform/ecommerce/projects", token, workspaceId, [200]);

  if (unified && typeof unified.unified !== "object") {
    fail("C4", "unified shape", "missing unified block");
  } else if (unified) {
    pass("C4", "unified shape", `stores=${unified.unified?.total_stores ?? 0}`);
  }
}

async function main() {
  console.log(`Staging smoke C4 Ecommerce → ${BASE}\n`);
  await waitForDeploy();
