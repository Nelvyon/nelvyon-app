...
| Área | Cambio | Descripción |
|------|--------|-------------|
| Web pública | **Deploy prod: biblioteca visual + capturas SaaS + DeviceMockup** | Gates: tsc/eslint/build/static/crawl/assets **PASS**. Publica `library/` (F-01/F-02 + I-01…I-05), `saas-shots`, DeviceMockup, hub `/producto`. Evidencia post-deploy en `docs/evidence/public-web-prod-postdeploy_latest.json`. |
| Web pública | **Biblioteca Envato organizada + integración (pre-deploy)** | Pipeline `organize-envato-library.mjs`: dedupe, classify F/M/I, extract SVG, WebP/AVIF photos → `public/brand/public/library/`. Integrado Home/Producto/CRM/IA/Agencia/Enterprise/Servicios/Sectores. `DeviceMockup` + `saas-shots`. Estado: `docs/ops/ENVATO_LIBRARY_STATUS.md`. |
| Marca / Web | **Envato P0 definitivo 42 (OK CEO + mods)** | Ampliado: IA business/collab/ML humano, cloud infra, SEO/Ads, BPA/workflows, analytics, mockups dashboard/CRM. Vídeos **máx. 3**. Eliminados paper/fondos artísticos y UI kits ajenos. Doc `NELVYON_VISUAL_LIBRARY_ENVATO.md`. **Descarga = CEO.** |
| Web pública | **Capturas reales SaaS + lista Envato (sin deploy)** | Pipeline Playwright autenticado (cookie + fixtures demo Aether Labs) → 18 WebP en `public/brand/public/saas-shots/`. Integrados en Home/Producto/módulos/Precios/Enterprise/Agencia vía `SaasProductCapture`. Wishlist Envato: `docs/ops/ENVATO_PUBLIC_WEB_WISHLIST.md`. Aceternity no ampliado. **NO deploy**. |
| Web pública | **Base visual Zubaz → NELVYON (sin deploy)** | Extraído Envato Zubaz a `.reference/zubaz`. Mapa `docs/ops/ZUBAZ_NELVYON_VISUAL_MAP.md`. Port composición `index-03` (hero centrado + thumb + float cards + iconboxes + splits + counters + pricing SaaS + FAQ). Assets `public/brand/public/zubaz/`. Aceternity selectivo (spotlight/beams). Sin Bootstrap. tsc PASS. **NO deploy** hasta aprobación. |
| Web pública | **Certificación final + deploy producción** | Gates locales: tsc/eslint/build/static/crawl **PASS** · LH a11y/BP **100** · perf **87–89** · SEO **91–92**. Defectos corregidos (href partners, nested main, contraste, CSP/`/api/auth/me` en marketing, WebP, UTF-8). Deploy Railway `@nelvyon/web` `aa9ec67a` **SUCCESS**. Post-deploy `https://app.nelvyon.com` **26/26** HTTP 200 + `/api/contact` 400 alive. Evidencia: `docs/evidence/public-web-cert-final.json`, `docs/evidence/public-web-prod-postdeploy_latest.json`. Residuales: perf shell JS · LH meta-description falso negativo · bf-cache. |
| Web pública | **Sitio multipágina NELVYON (identidad propia)** | Extracción Sofax/Nivia a `.reference/` (solo referencia). Nuevo DS `features/public-web` (Next/React/TS/Tailwind v4, sin Bootstrap). Chrome unificado, home + rutas IA (plataforma, agencia, automatizaciones, soluciones, sectores, enterprise, integraciones, precios, casos, recursos, FAQ, seguridad, aviso legal, subprocessors). Assets premium curados en `public/brand/public/`. Blog con 4 artículos reales. Legal ES canónico. tsc/eslint/build **PASS** · vitest marketing pricing **8/8**. |
...
| UI / SaaS | **W3CRM Fase 2 — módulo 18: webhooks / settings / marketplace / herramientas / PWA / lead-scoring** | **Causas raíz:** webhooks toggle/logs sin HTTP; settings General read-only; marketplace sin estados; links 404 en herramientas; emojis corruptos; PWA unsubscribe falso. Fix PATCH/DELETE/logs + updateTenantProfile + UI. tsc/build **PASS** · smoke 307/401. Ver §30. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 17b P1: objetos / documentos / helpdesk / encuestas / prospecting** | **Causas raíz:** objetos sin UI de registros + create silencioso + `recordCount` vs `recordsCount`; documentos con KPI/columnas inventadas; helpdesk/encuestas/prospecting sin error handling. Fix panel registros, datos reales, errores visibles, delete/deactivate/share revoke. tsc/build **PASS** · vitest **2480** · smoke 307/401. Ver §29. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 17 P0: countdown / snippets / A/B / QR** | **Causas raíz:** countdown payload/display desalineado con API; snippets `type` vs `channels`; A/B botones muertos + KPI falso `+23%` + `declare_winner` sin UI; QR canvas no escaneable. Fix alineación API + declare winner + QR real. tsc/build **PASS** · vitest **2480**. Ver §28. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 16: ERP** | Auditadas 5 pantallas ERP. **Causas raíz:** (1) `submitPR`/`approvePR` del core no expuestos en HTTP + UI sin crear PR → draft eterno; (2) `reserve` inventario sin UI; (3) hex/literales fuera del DS. Fix: actions HTTP + UI ciclo PR + reserve + NelvyonDs/KpiTile. tsc/build **PASS** · vitest **2480** · smoke 307/401. Ver §27. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 15: Membresías** | Auditado `/saas/memberships`. **Causas raíz:** (1) pestaña afiliados parseaba `{commissions}` pero API devuelve array → siempre vacío; (2) `subscribe` sin UI; (3) `is_active` sin write en `updatePlan`; (4) PATCH/DELETE con `contacts.read` → `contacts.write`. KpiTile + errores visibles. tsc/build **PASS** · vitest **2480**. Ver §26. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 14: Afiliados / Fidelización** | Auditados `/saas/affiliates` + `/saas/loyalty`. **Causas raíz:** (1) `mark-paid`/CEO gate sin UI; (2) `active` de enlaces sin write path → `setLinkActive`; (3) loyalty `redeem`/`adjust`/`transactions` APIs sin superficie; (4) settings incompletos (`active`, tiers) + validación servicio. KpiTile, errores inline, filtros comisiones. tsc/build **PASS** · vitest **2479** · smoke 307/401. Ver §25. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 13: LMS / Cursos** | Auditado `/saas/lms`. **Causas raíz:** `listEnrollments` omitía progreso y cert URL; DELETE curso sin UI; publish no refrescaba estado del editor. Fix: JOIN progress+certificates, delete, publish local status, KpiTile/tokens. tsc/build/vitest **2472** PASS. Ver §24. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 12: Ecommerce / Tienda Online** | Auditado `/saas/store`. **Causas raíz:** (1) pedidos `paid` sin acciones UI (flujo saltaba paid); (2) `GET orders/[id]` con items no consumido → modal detalle; (3) errores settings/productos/pedidos silenciosos; (4) PATCH producto ignoraba `null` al limpiar campos. Tokens + `KpiTile`. tsc/ESLint/build **PASS** · vitest **2471** · store 29/29 · smoke 307/401 **PASS**. Ver §23. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 11: Administración, usuarios, roles, permisos y auditoría** | Auditados `/saas/{team,auditoria,security,subcuentas}`. **Causas raíz:** (1) Suspender/Activar usaba `POST` (solo invite) y no existía `reactivate` → `SaasTeamService.reactivate` + `PATCH`; (2) shape API↔UI (`lastActiveAt`/`avatar`) desalineado → mapper; (3) Editar rol no-op → modal `PATCH` role; (4) MFA `provisioningUri` descartado → mostrado/copiable; (5) filtros auditoría con offset viejo → reset página 1. Tokens semánticos en team/security/subcuentas. tsc/ESLint/build **PASS** · vitest **2471 passed / 4 skipped** · smoke 307/401 **PASS** · staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §22. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 10: Analítica, informes y exportaciones** | Auditado `/saas/reportes` (+ nav `attribution`). **Causas raíz:** (1) `POST /api/saas/reports/generate` generaba ZIP real pero nunca insertaba en `saas_reports` → historial siempre vacío — persistencia añadida en `SaasDashboardReportService.generateAndPublish` (fail-soft) + `type` para nombre/título del informe; (2) `?tab=attribution` del nav ignorado → scroll a `#attribution` + `activeId`; (3) literales de color y KPIs sin `KpiTile` → tokens semánticos. PDF del historial enlazado. tsc/ESLint/build **PASS** · vitest core **2468 passed / 4 skipped** · saas-reports **5/5** · smoke 307/401 **PASS** · staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §21. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 9: Facturación, pagos y suscripciones** | Auditados `/saas/{billing,facturas}`. **3 hallazgos funcionales de causa raíz corregidos**, todos encadenados en `/saas/billing`: (1) `GET /api/saas/invoices` (facturas de plataforma NELVYON→tenant, `SaasInvoiceService`, generadas por `OsCronMaintenance`) ya existía y no se consumía en ninguna UI del repo — se añadió sección "Historial de facturas"; (2) `POST /api/saas/billing/cancel` ya escribía `saas_tenants.billing_status` (columna real desde la migración 482) pero `buildSaasBillingSummary` nunca lo devolvía y no había botón — se propagó `billingStatus` por toda la cadena de proyección de tenant (`saasTenantMapper`, `SaasOnboardingService`, `SaasBillingService`, `SaasTenantBridgeService`, `SaasDashboardService`) + botones Pausar/Cancelar/Reactivar con confirmación inline; (3) el endpoint no soportaba una acción `resume` — añadida. Adicionalmente, el botón "↓ PDF" de `/saas/facturas` no tenía `onClick` pese a que `GET /api/saas/facturas/[id]/pdf` ya generaba el HTML imprimible — conectado, y disponible también para facturas pagadas. `/saas/billing` (patrón `DarkCard`/hex) reescrita íntegramente a `NelvyonDs*`+tokens semánticos; `/saas/facturas` con literales de color puntuales migrados. tsc/ESLint/build **PASS** · `vitest backend/saas backend/email src/features/saas-crm` **195 archivos / 2467 passed / 4 skipped** (sin regresión por el campo `billingStatus` añadido a `SaasTenant`) · smoke sin sesión (307/401, sin 500) **PASS** · verificación visual autenticada en staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §20. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 8: Funnels, formularios y landing pages** | Auditados `/saas/{formularios,funnels,web-builder,web-builder/[pageId]}`. **3 hallazgos funcionales de causa raíz corregidos**: (1) `/saas/formularios` no exponía el toggle de `is_active` ni el botón de eliminar (ambos ya soportados por `PATCH`/`DELETE /api/saas/formularios/[formId]`), y `saas_form_submissions` (envíos ya persistidos por el endpoint público `/api/forms/[formId]/submit`) no tenía **ningún** endpoint de lectura autenticado ni UI de visor — se creó `GET /api/saas/formularios/[formId]/submissions` (con verificación de ownership de tenant) + componente `SubmissionsModal`; (2) `/saas/funnels` no tenía botón de eliminar pese a que `DELETE /api/saas/funnels/[funnelId]` ya existía — añadido con confirmación; (3) `/saas/web-builder` no enlazaba al editor de secciones `/saas/web-builder/[pageId]` (ya completamente implementado) y `SaasWebBuilderService.delete()` no tenía ruta HTTP expuesta — se creó `DELETE /api/saas/web-builder/[pageId]` + botones "Editar"/eliminar en el listado. Adicionalmente, `/saas/funnels` (patrón `DarkCard`, 24 usos) y el editor `/saas/web-builder/[pageId]` (hex literales `#020817`/`#0084ff`, opacidades `white/[]`) migrados íntegramente a `NelvyonDs*` + tokens semánticos + `KpiTile`; el generador de preview HTML del editor (`buildPreviewHtml`, mirror de `renderHtml()` del servidor) se dejó intacto por representar el diseño de la página publicada del cliente, no el shell de NELVYON. tsc/ESLint/build **PASS** · `vitest backend/saas backend/email src/features/saas-crm` **195 archivos / 2467 passed / 4 skipped** · smoke sin sesión incluyendo el nuevo endpoint de submissions (307/401, sin 500) **PASS** · verificación visual autenticada en staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §19. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 7: Marketing y redes sociales** | Auditados `/saas/{social,publicidad,seo,reputacion}`: las cuatro pantallas ya eran implementaciones reales sin mocks (CRUD, OAuth, degraded states) — sin hallazgo funcional de causa raíz, a diferencia de los módulos 4/5/6. Único hallazgo: uso sistemático de colores Tailwind literales (`text-red-400`, `bg-red-500/10`, `text-green-400`, `text-yellow-400`, `border-amber-500/30`, `#0084ff`, `text-white`) en vez de los tokens semánticos (`destructive/success/warning/primary/primary-foreground`) ya establecidos en los módulos 1–6 — violación de "un solo sistema visual" (renderizaba bien, no es el bug del módulo 2). Corregido sustituyendo cada literal por su token en errores, banners, badges, botones y lógica de color (ROAS, dificultad de keyword, rating de estrellas); KPIs de las 4 pantallas migrados a `KpiTile`. Colores por plataforma/marca (Instagram/Facebook/LinkedIn/TikTok en `social`) y por tipo de dato se mantienen intencionalmente (mismo patrón ya documentado para el calendario en §17.3 — codificación de categoría, no theming). tsc/ESLint/build **PASS** · `vitest backend/saas backend/email src/features/saas-crm` **195 archivos / 2467 passed / 4 skipped** (sin tests nuevos — sin cambios de lógica de backend) · smoke sin sesión (307/401, sin 500) **PASS** · verificación visual autenticada en staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §18. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 6: Calendario y citas** | `/api/saas/citas` solo exponía `GET`/`POST` — el esquema `saas_appointments` ya modelaba 5 estados (`scheduled/confirmed/completed/cancelled/no_show`) pero ninguna cita podía transicionar nunca desde `scheduled`, dejando el KPI "Completadas" de `/saas/citas` matemáticamente fijo en `0` y sin forma de eliminar una cita creada por error. Añadido `PATCH`/`DELETE /api/saas/citas/[id]` (permiso `workflows.write`, mismo usado ya por `POST`; scoping por `tenant_id` preservado) + botones de acción por fila (Confirmar/Completar/Cancelar/Eliminar) en la UI, con manejo de error visible. KPIs migrados a `KpiTile`. `/saas/calendar` recibe fixes de estados ausentes: loading sin skeleton (estado ya existía pero nunca se renderizaba), vista lista sin empty state, fallos de red en la carga de eventos ignorados en silencio. `@fullcalendar/react@7.0.2` evaluado formalmente y descartado para este módulo (grid CSS propio ya cubre mes/lista/filtros sin dependencia adicional). tsc/ESLint/build **PASS** · `vitest backend/saas backend/email src/features/saas-crm` **195 archivos / 2467 passed / 4 skipped** · smoke sin sesión incluyendo PATCH/DELETE (307/401, sin 500) **PASS** · verificación visual autenticada en staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §17. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 5: Automatizaciones y workflows** | `/saas/workflows` (builder clásico) recibe fixes de consistencia visual (`KpiTile`, tokens `warning/success/destructive`, `NelvyonDsStatusDot`, limpieza de código muerto). **Fix de causa raíz grave en `/saas/workflows/editor`**: el editor visual (`@xyflow/react` sobre `DragDropWorkflowService`) nunca leía `GET /api/saas/workflows/visual` (ya implementado) — siempre arrancaba con 2 nodos hardcodeados sin forma de recuperar flujos guardados, ni paleta para añadir nodos nuevos, por lo que solo podía publicar siempre la misma combinación fija `contact_created → send_email`. Añadido `GET`/`DELETE /api/saas/workflows/visual/[id]` (expone `getWorkflow`/`deleteWorkflow`, ya testeados) + editor reescrito con panel "Mis flujos" (listar/cargar/eliminar) y paleta real de nodos trigger/acción (acciones limitadas a los 4 tipos que `publishAsSaasWorkflow` soporta de verdad, para no introducir una nueva degradación silenciosa a `notify`). tsc/ESLint/build **PASS** · `vitest dragDropWorkflow.test.ts+saasWorkflow*` (92 passed) + core (2467 passed/4 skipped) **PASS** · smoke sin sesión (307/401, sin 500) **PASS** · verificación visual autenticada en staging `BLOCKED_ENVIRONMENT`. Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §16. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — módulo 4: Comunicación** | Auditados `/saas/{inbox,campanias,secuencias,sms,whatsapp,dialer,deliverability}`. `inbox`/`whatsapp`/`dialer` ya cumplían el estándar visual (no se tocan, salvo KPI de SLA de `inbox` migrado a `KpiTile` por coherencia). `campanias`/`secuencias`/`deliverability` reescritos de clases hardcodeadas/`DarkCard` a `NelvyonDsCard/Badge/Button/SectionHeader`+`KpiTile`. **3 bugs funcionales de causa raíz corregidos**: (1) open rate de `campanias` siempre mostraba 0% por un numerador `0` hardcodeado pese a que el backend ya devolvía `openedCount` real; (2) `/saas/sms` mostraba "campañas SMS" con datos descartados (`campaigns: []` hardcodeado) y un botón "Nueva campaña" que llamaba a una acción no implementada en el backend, además el envío único usaba el campo `body` en vez de `message` — se añadió `SaasSmsService.listRecent()` (lee `saas_sms_log`, ya poblada pero nunca leída) + `GET /api/saas/sms` expone el historial real + página reescrita sin el concepto de "campaña" inexistente; (3) `deliverability` sin manejo de error ni feedback de éxito — añadidos. tsc/ESLint/build **PASS** · `vitest backend/saas backend/email src/features/saas-crm` **195 archivos / 2467 tests passed, 4 skipped** · smoke sin sesión (307/401, sin 500) **PASS** · verificación visual autenticada en staging `BLOCKED_ENVIRONMENT` (sin `DATABASE_URL` local, igual que módulos 2 y 3). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §15. `claimReady: false` |
...
| Área | Cambio | Descripción |
|------|--------|-------------|
| UI / SaaS | **W3CRM Fase 2 — CRM/Pipeline + fix causa raíz contraste oscuro** | Auditoría detectó bug sistémico: `SaasShellLayout` nunca activaba el scope `.dark` de Tailwind → `NelvyonDsCard`/`Badge`/`Button`/`SectionHeader` renderizaban con colores de tema claro sobre el fondo `#020817` en **todas** las páginas `/saas/*`; además `--color-destructive/success/warning` no estaban registrados en `@theme inline`, dejando sin efecto `text-destructive`/`bg-warning`/`text-success` en ~150 archivos del producto. Corregido en `globals.css` (tokens + bloque `.dark`) y `SaasShellLayout.tsx` (clase `dark` en el contenedor raíz) — cambio aditivo, verificado por compilación CSS real + mockup estático con captura de pantalla. Reactivado `DealsKanban`/`DealFormModal`/`DealDetailPanel` (código completo y probado, sin usar hasta ahora) en la pestaña "Deals" de `/saas/pipeline`, sin instalar `@hello-pangea/dnd`. `DealsKpiRow`/`CommercialKpiRow` migrados a `KpiTile` (mismo patrón que Dashboard). tsc/ESLint/build/Vitest (2449 tests) **PASS**. `claimReady: false` |
| UI / SaaS | **W3CRM Fase 2 — Dashboard ejecutivo** | Usuario confirmó Opción B (reconstrucción nativa + libs puntuales evaluadas). Nuevos primitivos `SaasDashboardWidgets.tsx` (`SaasWidgetHeader`, `KpiTile`, `SaasAvatarBubble`) inspirados en composición W3CRM, ubicados junto al shell SaaS existente (no se crea carpeta `nelvyon-ui` — evita un 3er sistema visual). `/saas/dashboard` reconectado a las mismas APIs reales, cero mock data, cero funcionalidad eliminada. `@hello-pangea/dnd@18` y `@fullcalendar/react@7` verificados compatibles React 19 (reservados para `pipeline`/`citas`). tsc/ESLint/build (312 páginas)/Vitest (2464 tests) **PASS**. `claimReady: false` |
