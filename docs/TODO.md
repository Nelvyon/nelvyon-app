...
- [ ] **P2 — validar visualmente en staging** el módulo Funnels/formularios/landing pages con sesión autenticada real (mismo bloqueo de `DATABASE_URL` local que módulos 2–7)
- [ ] **W3CRM Fase 2 — siguiente módulo (automático):** Facturación, pagos y suscripciones → Analítica/informes/exportaciones → … (orden completo en `docs/ops/W3CRM_MIGRATION_PLAN.md` §7)
