...
| **Evidencia** | mig **520** · `ErpDomainSnapshotStore` · `ErpPersistentRuntime` · API routes wired · vitest `ErpPersistenceRoundtrip` / `ErpDomainSnapshotStore` · smoke script `staging-smoke-erp-persistence.mjs` · OsCatalogV1 nextAction Postgres SSOT · tip **uncommitted** · staging live tip **`bd165985`** aún **sin** 519/520 · restart smoke **pending** |
| **Consecuencias** | Código: Postgres SSOT when DB set · **not claimed live** until deploy + restart smoke · relational dual-write (519 companions) optional later · **CONDITIONAL_READY** · **NOT READY** · `claimReady: false` |
