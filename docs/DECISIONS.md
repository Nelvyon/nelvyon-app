...
## ADR-070 — RAG small-corpus minScore floor (raise only · full PASS)
...
## ADR-072 — Sequence enrollment open/click tracking (mig 521)

| Campo | Valor |
|-------|-------|
| **Fecha** | 2026-07-28 |
| **Decisión** | Añadir columnas booleanas `email_opened` / `email_clicked` a `saas_sequence_enrollments` (ADD IF NOT EXISTS). Staging applied; prod only via ADR-064 CEO gate. |
| **Por qué** | Branch steps de secuencias necesitan estado real sin mocks. |
| **Rollback** | `ALTER TABLE ... DROP COLUMN IF EXISTS email_opened, email_clicked` (documentado; no ejecutar en prod sin plan). |
| **Consecuencias** | Tracking routes + Vitest V2 · Playwright secuencias · claimReady false |

---

## ADR-073 — Workflows trigger CHECK sync + comunidades replies deferred

| Campo | Valor |
|-------|-------|
| **Fecha** | 2026-07-28 |
| **Decisión** | (1) Mig **522** alinea CHECK `trigger_type` con API (añade `score_threshold`). (2) Respuestas anidadas de comunidades **permanecen deshabilitadas honestamente** hasta migración `parent_post_id` + UX + tests (no simular). |
...
| **Plan** | `docs/ops/W3CRM_MIGRATION_PLAN.md` — incluía pregunta abierta bloqueante (§9), **resuelta el mismo día**: el usuario confirmó Opción B (reconstrucción nativa en Tailwind v4/TS + evaluación caso a caso de librerías puntuales compatibles con React 19, sin tocar el sistema de estilos base) |
...
| **Actualización 2026-07-30 (Fase 2, módulo 1)** | Dashboard ejecutivo migrado: nuevos primitivos (`SaasWidgetHeader`, `KpiTile`, `SaasAvatarBubble`) ubicados en `saas-shell/components/` (no en una carpeta `nelvyon-ui` aparte, para no crear un tercer sistema visual junto a `design-system/` y el shell SaaS). `@hello-pangea/dnd@18` y `@fullcalendar/react@7` verificados compatibles con React 19 — reservados para `pipeline`/`citas`, no instalados todavía. tsc/lint/build/vitest PASS. |
| **Actualización 2026-07-30 (Fase 2, módulo 2 — CRM/Pipeline)** | Hallazgo de causa raíz durante la auditoría: `SaasShellLayout` nunca activaba el scope `.dark` de Tailwind, por lo que **todo** componente basado en tokens semánticos (`NelvyonDsCard`, `NelvyonDsBadge`, `NelvyonDsButton`, `NelvyonDsSectionHeader`) renderizaba con colores de tema claro (tarjeta blanca, texto oscuro) sobre el fondo `#020817` — bug de contraste roto en decenas de pantallas `/saas/*`, no solo CRM/Pipeline. Verificación adicional (compilación aislada con `@tailwindcss/postcss`) confirmó que `--color-destructive/success/warning` nunca se registraron en `@theme inline`, dejando sin efecto las clases `text-destructive`/`bg-warning`/`text-success` (y variantes `-foreground`) en ~150 archivos de todo el producto. **Decisión:** corregir la causa raíz en `globals.css` (registrar los tokens que faltaban + bloque `.dark { ... }` con la paleta oscura NELVYON) y activar `className="dark"` en el contenedor raíz de `SaasShellLayout`, en vez de reescribir cada componente afectado uno a uno. Cambio puramente aditivo en CSS (ningún valor/clase existente se elimina). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §13.1 para el detalle y la evidencia empírica (compilación + captura de mockup estático). Además, se reactivó `DealsKanban`/`DealFormModal`/`DealDetailPanel` (código completo y probado pero sin usar en ninguna pantalla real) en la pestaña "Deals" de `/saas/pipeline`, evitando instalar `@hello-pangea/dnd` al existir ya un kanban HTML5 nativo funcional. tsc/lint/build/vitest PASS. |
| **Actualización 2026-07-31 (fix previo al módulo 3)** | Bug documentado en §13.4 (`tone={... ?? "default"}` inválido en badge de Contratos de `/saas/pipeline`) corregido a `"neutral"`. Commit separado `8974e873` antes de iniciar el módulo 3. |
| **Actualización 2026-07-31 (Fase 2, módulo 3 — IA NELVYON)** | Alcance real auditado: `/saas/{ai,autopilot,agentes,chat,copywriter,knowledge-base}` (el resto del grupo `ia` de `saasNav.ts` — pack-store/data-playbooks/brief-to-launch/compliance/benchmark — es un motor de packs/growth distinto, queda para módulo posterior). `/saas/ai` y `/saas/autopilot` migrados de divs con clases hardcodeadas (`border-white/10`, `#0084ff` literal) a `NelvyonDsCard/Badge/SectionHeader/StatusDot` + `KpiTile`, mismo patrón que Dashboard/CRM — sin tocar ninguna API. `/saas/agentes` ganó una sección real de "Historial de ejecuciones" con datos ya obtenidos de `/api/saas/agentes/runs` pero nunca renderizados. Hallazgo funcional de causa raíz en `/saas/chat`: el `POST` de `/api/saas/chat` nunca persistía en `saas_chat_messages` pese a que el `GET` sí leía de esa tabla — el historial de conversación se perdía siempre y esas conversaciones quedaban fuera del export/delete GDPR (`SaasGdprService` ya opera sobre esa tabla). Corregido con `SaasChatService.saveExchange(...)` (persiste sin reinvocar el LLM) + `DELETE /api/saas/chat` (expone `clearHistory`, ya existente sin uso) + frontend que carga historial real al montar y permite "Nueva conversación". Ruta huérfana `/saas/knowledge-base` (funcional, con API real y RBAC, pero ausente de `saasNav.ts` y con `activeId` erróneo) añadida al nav (grupo `gestion`, junto a `helpdesk`, por ser un centro de ayuda de clientes real — no un corpus de entrenamiento IA) con traducciones en los 6 locales. **Decisión explícita de no crear pantallas nuevas**: "Prompts library" y "editor de configuración de modelos" pedidos por el usuario no tienen API real detrás en el inventario auditado — la configuración de modelos/router y el consumo IA reales ya se exponen dentro de Panel IA (`router-health`, `private-ai/metrics`) y se integraron ahí en vez de inventar endpoints. tsc/lint/build/vitest (2446 passed/4 skipped) PASS; smoke de rutas sin sesión (307/401, sin 500) PASS; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (sin `DATABASE_URL` local, igual que módulo 2). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §14. |
| **Actualización 2026-07-31 (Fase 2, módulo 4 — Comunicación)** | Alcance auditado: `/saas/{inbox,campanias,secuencias,sms,whatsapp,dialer,deliverability}` (grupo `comunicacion` de `saasNav.ts`; `social` queda para el módulo 5 "Marketing y redes sociales" por orden explícito del usuario). Auditoría encontró que `/saas/{inbox,whatsapp,dialer}` ya cumplían el estándar visual (`NelvyonDs*`+tokens semánticos) de módulos anteriores — no se tocan ("si lo existente ya cumple, no lo toques"), salvo un ajuste de coherencia menor en el KPI de SLA de `/saas/inbox` (de `NelvyonDsCard` manual a `KpiTile`, mismo patrón que el resto). `/saas/{campanias,secuencias,deliverability}` seguían con clases hardcodeadas (`text-white`, `bg-[#0084ff]`, `DarkCard`) heredadas de antes del fix de contraste del módulo 2 — reescritas a `NelvyonDsCard/Badge/Button/SectionHeader`+`KpiTile`. **Tres hallazgos funcionales de causa raíz corregidos, no solo estéticos:** (1) `/saas/campanias` calculaba el "Open rate" de cada fila con `(0 / c.sentCount) * 100` — numerador literal `0`, siempre mostraba "0%" pese a que el backend (`SaasCampaniasService.getCampanias`) ya devolvía `openedCount`/`clickedCount` reales por campaña, simplemente ausentes del tipo `Campania` del frontend; corregido usando el dato real. (2) La página `/saas/sms` mostraba una lista de "campañas SMS" que la propia función de carga sobrescribía con un array vacío hardcodeado (`{ campaigns: [] }`) sin leer nunca la respuesta real de la API, y el modal "Nueva campaña" llamaba a `POST /api/saas/sms` con `{action:"create_campaign"}`, acción que el backend (`SaasSmsService`) nunca implementó — siempre fallaba con un 400 genérico; además el modal "Enviar SMS único" enviaba el campo `body` cuando la API espera `message`, por lo que ese envío también fallaba siempre. Se auditó el backend y se confirmó que `saas_sms_log` (migración 419) ya persiste cada envío por tenant pero **nunca se leía de vuelta** — se añadió `SaasSmsService.listRecent()` + `messages` en `GET /api/saas/sms`, y se reescribió la página para mostrar el historial real (mismo patrón que WhatsApp/Dialer) en vez de un concepto de "campaña" sin respaldo real; se corrigió el campo `message` en el envío único. (3) `/saas/deliverability` no tenía manejo de errores (fallo silencioso deja "Cargando métricas…" indefinidamente) ni feedback de éxito tras guardar la IP dedicada o avanzar el warm-up — añadidos ambos. Ningún cambio de RBAC, multi-tenancy, `claimReady` (false) ni canary IA (KILL). tsc/ESLint/build **PASS**; `vitest backend/saas/__tests__/SaasSmsService.test.ts` (nuevos tests de `listRecent`) + suite core **PASS**; smoke de rutas sin sesión (307 páginas / 401 APIs, sin 500) **PASS**; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (sin `DATABASE_URL` local, mismo patrón que módulos 2 y 3). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §15. |
| **Actualización 2026-07-31 (Fase 2, módulo 5 — Automatizaciones y workflows)** | Alcance auditado: `/saas/workflows` (builder clásico, ya real y extenso: 16 triggers/17 acciones/24+ plantillas, runs/versiones, sin mocks) + `/saas/workflows/editor` (editor visual `@xyflow/react` sobre `DragDropWorkflowService`/tabla `dragdrop_workflows`). **Hallazgo funcional de causa raíz grave:** el editor visual nunca llamaba a `GET /api/saas/workflows/visual` (ya implementado y funcional) — siempre arrancaba con los mismos 2 nodos hardcodeados (`contact_created → send_email`) sin forma de recuperar un flujo guardado; no existía ninguna paleta ni botón para insertar nodos nuevos, por lo que el "editor visual" solo podía publicar siempre esa misma combinación fija independientemente de lo que el usuario intentara construir; y `deleteWorkflow` (ya implementado y testeado en el servicio) no estaba expuesto por ninguna API. **Decisión:** corregir exponiendo exclusivamente capacidad de backend ya real y probada (mismo patrón que `SaasSmsService.listRecent` en el módulo 4) — nuevo `GET`/`DELETE /api/saas/workflows/visual/[id]` (permisos `workflows.read`/`workflows.delete`, ya existentes en RBAC) + editor reescrito con panel "Mis flujos" (listar/cargar/eliminar flujos reales) y botones para añadir nodos trigger/acción con selector de tipo. El catálogo de acciones del selector se limita deliberadamente a los 4 tipos que `DragDropWorkflowService.publishAsSaasWorkflow` mapea de forma explícita (`send_email`, `notify`, `add_tag`, `webhook_out`) — el resto degradaría en silencio a `notify` al publicar, y ofrecerlos habría sido introducir una mentira funcional nueva en vez de corregir la existente; los 16 triggers se ofrecen completos porque se pasan sin remapear. Cero cambios en `DragDropWorkflowService`/`SaasWorkflowService`/modelo de datos. Fixes de consistencia visual adicionales en el builder clásico (KPIs a `KpiTile`, banners y botones de acción de `yellow/green/red-500` literales a tokens `warning/success/destructive`, puntos de estado de runs a `NelvyonDsStatusDot`) + eliminación de una constante `_panels` muerta detectada en la auditoría. tsc/ESLint/build **PASS**; `vitest dragDropWorkflow.test.ts + saasWorkflow*` (92 passed) + suite core (2467 passed/4 skipped) **PASS**; smoke de rutas sin sesión (307/401, sin 500, incluye `DELETE` en la nueva ruta) **PASS**; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (mismo patrón que módulos 2, 3 y 4). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §16. |
| **Actualización 2026-07-31 (Fase 2, módulo 6 — Calendario y citas)** | Alcance auditado: `/saas/citas` (agenda 1:1 sobre `saas_appointments`, con confirmación por email SES, ya sin mocks) + `/saas/calendar` (vista unificada de eventos sobre `SaasCalendarService`, ya sin mocks). `@fullcalendar/react@7.0.2` — reservado en HANDOVER desde el módulo 1 como referencia visual candidata — se evaluó formalmente y **se descartó**: el grid CSS nativo de `/saas/calendar` ya cubre mes/lista/filtros por tipo sin necesidad de una dependencia de terceros, y adoptarla solo por alineación visual sin una ganancia funcional real (drag-and-drop, vista semana/día) habría sido complejidad injustificada. **Hallazgo funcional de causa raíz:** `saas_appointments` modela 5 estados (`scheduled/confirmed/completed/cancelled/no_show`) y `/saas/citas` ya calculaba un KPI "Completadas" a partir de ese campo, pero `/api/saas/citas/route.ts` solo exponía `GET`/`POST` — ninguna cita podía transicionar nunca de `scheduled`, por lo que ese KPI estaba matemáticamente condenado a mostrar siempre `0`, y una cita creada por error no podía eliminarse. **Decisión:** corregir exponiendo el ciclo de vida ya modelado en el esquema (mismo patrón que los módulos 4 y 5) — nuevo `PATCH`/`DELETE /api/saas/citas/[id]`, reutilizando el permiso `workflows.write` que ya usa `POST /api/saas/citas` (no existe un permiso RBAC dedicado `citas.*`; introducir uno nuevo a mitad de esta migración visual habría sido una decisión de RBAC fuera de alcance, documentada como mejora futura en `docs/ops/W3CRM_MIGRATION_PLAN.md` §17.5) + botones de acción por fila en la UI. `/saas/calendar` recibe fixes de estados detectados en la auditoría: `loading` se capturaba pero nunca se renderizaba ningún skeleton, la vista lista no mostraba empty state con el mes filtrado sin eventos, y los fallos de red al cargar eventos se ignoraban en silencio (`if (res.ok)` sin rama de error) — los tres corregidos. El uso de color por tipo de evento (`style={{ backgroundColor: e.color }}`) se mantiene intencionalmente: es codificación de color por categoría de dato, un patrón legítimo de calendarios, no un hardcodeo de tema. Cero cambios en el esquema de `saas_appointments` ni en `SaasCalendarService`. tsc/ESLint/build **PASS**; `vitest backend/saas backend/email src/features/saas-crm` (2467 passed/4 skipped) **PASS**; smoke de rutas sin sesión incluyendo `PATCH`/`DELETE` (307/401, sin 500) **PASS**; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (mismo patrón que módulos 2, 3, 4 y 5). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §17. |
| **Actualización 2026-07-31 (Fase 2, módulo 7 — Marketing y redes sociales)** | Alcance auditado: `/saas/{social,publicidad,seo,reputacion}` (grupo de `saasNav.ts` correspondiente al módulo 7 confirmado por el usuario). A diferencia de los módulos 4/5/6, la auditoría **no encontró ningún hallazgo funcional de causa raíz** — las cuatro pantallas ya tenían CRUD real, integraciones OAuth (Meta/Google/LinkedIn/TikTok/Snapchat), atribución multi-touch, sync de Google Business Profile y estados degradados (`SaasDegradedBanner`) funcionales antes de esta sesión, sin mocks ni capacidades de backend sin exponer. **Único hallazgo:** uso sistemático de utilidades Tailwind literales (`text-red-400`, `bg-red-500/10`, `text-green-400`, `text-yellow-400`, `border-amber-500/30`, `#0084ff`, `text-white`) en vez de los tokens semánticos del design system (`destructive/success/warning/primary/primary-foreground`) ya establecidos en los módulos 1–6 — a diferencia del bug del módulo 2 (variables CSS ausentes que rompían el render), estos literales renderizaban correctamente; es una violación de la regla "no mantengas dos sistemas visuales diferentes" por consistencia, no un defecto de renderizado. **Decisión:** sustitución sistemática 1:1 de cada literal por su token semántico en errores, banners, badges, botones y lógica de color condicional (ROAS en `publicidad`, dificultad/posición/delta de keyword en `seo`, rating de estrellas en `reputacion`) + migración de KPIs a `KpiTile` en las 4 pantallas (incluyendo importar `KpiTile`, ausente en `reputacion` pese a que el resto del módulo ya usaba `NelvyonDs*`). Los colores por plataforma/marca (`PLATFORM_CONFIG` en `social`: Instagram rosa, Facebook/Meta azul, LinkedIn azul oscuro, TikTok morado) se mantienen intencionalmente — codificación de marca por categoría, mismo patrón ya documentado para el calendario en §17 (color por tipo de evento), no theming hardcodeado. Cero cambios de lógica de negocio, RBAC o esquema. tsc/ESLint/build **PASS**; `vitest backend/saas backend/email src/features/saas-crm` (2467 passed/4 skipped, sin tests nuevos) **PASS**; smoke de rutas sin sesión (307/401, sin 500) **PASS**; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (mismo patrón que módulos 2–6). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §18. |
| **Actualización 2026-07-31 (Fase 2, módulo 8 — Funnels, formularios y landing pages)** | Alcance auditado: `/saas/formularios` (form builder + plantillas + embed), `/saas/funnels` (funnel builder + A/B testing + analytics), `/saas/web-builder` (listado) y `/saas/web-builder/[pageId]` (editor visual de secciones con preview, SEO, dominio custom, historial de versiones). Volviendo al patrón de los módulos 4/5/6 (a diferencia del módulo 7), la auditoría **sí encontró 3 hallazgos funcionales de causa raíz** — capacidades ya implementadas en el backend y nunca expuestas en la UI: (1) `saas_forms.is_active` (soportado por `PATCH`) sin toggle en la UI, `DELETE` sin botón, y sobre todo `saas_form_submissions` — los envíos del endpoint público `/api/forms/[formId]/submit` se guardaban correctamente (incluso con `contact_id` vinculado) pero **no existía ningún endpoint autenticado de lectura ni UI de visor**, dejando esos datos completamente inaccesibles para el tenant; (2) `DELETE /api/saas/funnels/[funnelId]` ya funcional pero sin botón en el listado; (3) el editor `/saas/web-builder/[pageId]` (completamente implementado: secciones, preview en iframe, SEO, dominio custom, versiones) **no estaba enlazado desde el listado** `/saas/web-builder` — no había forma de llegar a él sin escribir la URL manualmente — y `SaasWebBuilderService.delete()` existía en el servicio pero sin ruta HTTP. **Decisión:** mismo patrón que módulos 4/5/6 — exponer únicamente capacidad de backend ya real y probada, sin tocar lógica de negocio ni esquema. Nuevo `GET /api/saas/formularios/[formId]/submissions` (permiso `workflows.read`, verifica ownership de tenant con `WHERE tenant_id = $2` antes de exponer cualquier dato, `LEFT JOIN saas_contacts` para mostrar el contacto vinculado) + `SubmissionsModal` + toggle `isActive` + botón eliminar en `/saas/formularios`; botón eliminar en `/saas/funnels`; nuevo `DELETE /api/saas/web-builder/[pageId]` (permiso `contacts.write`, llama a `SaasWebBuilderService.delete()` ya existente) + botón "Editar" (enlace real al editor) y botón eliminar en `/saas/web-builder`. Adicionalmente, `/saas/funnels` (patrón `DarkCard`, 24 usos) y el editor `/saas/web-builder/[pageId]` (hex literales `#020817`/`#0084ff`, opacidades `white/[]`) — ambos heredados de antes del fix de contraste del módulo 2 y nunca migrados — se reescribieron íntegramente a `NelvyonDsCard/Badge/Button` + tokens semánticos + `KpiTile`. El generador `buildPreviewHtml()` del editor (mini-HTML renderizado dentro del iframe de preview, mirror deliberado de `SaasWebBuilderService.renderHtml()`) se dejó intacto: sus colores hardcodeados representan el diseño de la página publicada del cliente final, no el shell de NELVYON, y deben coincidir exactamente con lo que el servidor genera en producción — tocarlos habría sido una regresión, no una mejora. tsc/ESLint/build **PASS**; `vitest backend/saas backend/email src/features/saas-crm` (2467 passed/4 skipped, sin tests nuevos — sin cambios de lógica de backend, solo un endpoint de lectura nuevo) **PASS**; smoke de rutas sin sesión incluyendo el nuevo endpoint de submissions y el nuevo `DELETE` (307/401, sin 500) **PASS**; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (mismo patrón que módulos 2–7). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §19. |
| **Actualización 2026-07-31 (Fase 2, módulo 9 — Facturación, pagos y suscripciones)** | Alcance auditado: `/saas/billing` (plan, uso, checkout Stripe, portal) y `/saas/facturas` (facturación a clientes: crear/enviar/cobrar, dunning, PDF). Mismo patrón que módulos 4–6/8: la auditoría encontró **3 hallazgos funcionales de causa raíz**, todos encadenados en `/saas/billing`. (1) `SaasInvoiceService` (invocado por `OsCronMaintenance`) genera y persiste facturas de plataforma reales en `saas_invoices`, y `GET /api/saas/invoices`/`GET /api/saas/invoices/[id]` (permiso `invoices.read`) ya las exponían — pero ningún componente en `/saas/*` ni `/portal/*` los consumía; el tenant no tenía forma de ver su propio historial de facturación de plataforma. (2) `POST /api/saas/billing/cancel` ya escribía `saas_tenants.billing_status` (columna real desde la migración 482) pero `buildSaasBillingSummary` nunca lo leía ni lo devolvía, y `/saas/billing` no tenía ningún botón que invocara el endpoint — una capacidad de auto-servicio completa en el backend, invisible e inalcanzable desde la UI. (3) el endpoint solo soportaba `pause`/`cancel_at_period_end`, sin acción `resume` — no había vuelta atrás sin editar la base de datos a mano. **Decisión:** mismo patrón que módulos 4–6/8 — exponer capacidad de backend ya real, propagando el campo que faltaba por toda la cadena de proyección de tenant en vez de leerlo de forma ad-hoc solo en el endpoint de billing: `billing_status` añadido a `SAAS_TENANT_SELECT`/`saasTenantFromRow` (`saasTenantMapper.ts`, con tipo `SaasBillingStatus` y normalización fail-safe a `"active"`), a la interfaz `SaasTenant` (`SaasOnboardingService.ts`), y a los dos `SELECT`/`RETURNING` manuales que aún no usaban la constante compartida (`SaasTenantBridgeService.ts` unificado en `SAAS_TENANT_SELECT`; `SaasDashboardService.ts` con columna añadida a su `JOIN`) — cambio puramente aditivo, sin romper ningún consumidor existente (tsc/vitest confirmaron 0 regresiones). `SaasBillingSummary.tenant.billingStatus` poblado en `buildSaasBillingSummary`; nueva acción `resume` en el endpoint de cancelación; `/saas/billing` gana banner de estado + botones "Pausar"/"Cancelar" (confirmación inline, sin `window.confirm`)/"Reactivar" + sección "Historial de facturas". Adicionalmente, el botón "↓ PDF" de `/saas/facturas` (ya en el markup) no tenía `onClick` pese a que `GET /api/saas/facturas/[id]/pdf` ya generaba el HTML imprimible — conectado, y disponible también para facturas pagadas (antes se ocultaba sin razón funcional). `/saas/billing` (patrón `DarkCard`/hex, heredado de antes del fix del módulo 2) reescrita íntegramente a `NelvyonDsCard/Badge/Button` + tokens semánticos; `/saas/facturas` con literales de color puntuales migrados. Cero cambios en `SaasInvoiceService`, `SaasFacturasService` ni en el cron. tsc/ESLint/build **PASS**; `vitest backend/saas backend/email src/features/saas-crm` (2467 passed/4 skipped, sin regresión por el campo `billingStatus` añadido a `SaasTenant`) **PASS**; smoke de rutas sin sesión (307/401, sin 500) **PASS**; verificación visual autenticada en staging queda `BLOCKED_ENVIRONMENT` (mismo patrón que módulos 2–8). Ver `docs/ops/W3CRM_MIGRATION_PLAN.md` §20. |
| **Actualización 2026-07-31 (Fase 2, módulo 10 — Analítica, informes y exportaciones)** | Alcance: /saas/reportes (+ nav attribution). Causas raíz: generate ZIP nunca insertaba en saas_reports (historial vacío); ?tab=attribution ignorado; tokens literales. Fix: persistencia fail-soft en SaasDashboardReportService, type→nombre/título, scroll attribution, KpiTile/tokens, PDF historial. tsc/build/vitest PASS. Ver W3CRM_MIGRATION_PLAN §21. |
...
## ADR-077 — Rate-limit crítico: staging memory vs prod fail-closed (2026-08-01)

| Campo | Valor |
|-------|-------|
| **Fecha** | 2026-08-01 |
| **Decisión** | Sin Upstash, reglas críticas (uth-login, etc.) fail-closed solo en entorno estricto (Railway production / NELVYON_DEPLOY_ENV=production). Staging/preview/NELVYON_DEPLOY_ENV=staging|test usan fallback in-memory con los mismos límites (no bypass, no fail-open global). Producción exige Upstash antes del go-live auth. |
