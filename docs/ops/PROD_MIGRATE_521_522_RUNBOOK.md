...
```powershell
git fetch origin
git rev-parse HEAD origin/main   # tip autorizado (hoy 7806641b — ver HANDOVER)
...
# NELVYON_PROD_MIGRATE_COMMIT_SHA=7806641b   # opcional pero recomendado (tip HANDOVER)
```
...
railway run -s Postgres -e production -- pwsh -Command '
  ...
  $env:NELVYON_PROD_MIGRATE_COMMIT_SHA = "7806641b"
