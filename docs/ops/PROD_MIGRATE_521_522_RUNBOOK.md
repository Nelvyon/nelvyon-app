...
```powershell
git fetch origin
git rev-parse HEAD origin/main   # tip autorizado (hoy 9bbd5808 — ver HANDOVER)
...
# NELVYON_PROD_MIGRATE_COMMIT_SHA=9bbd5808   # opcional pero recomendado (tip HANDOVER)
```
...
railway run -s Postgres -e production -- pwsh -Command '
  ...
  $env:NELVYON_PROD_MIGRATE_COMMIT_SHA = "9bbd5808"
