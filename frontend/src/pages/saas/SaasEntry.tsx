...
import {
  Lock, Loader2, Fingerprint, AlertTriangle, KeyRound, Shield, Eye, EyeOff
} from "lucide-react";
...
export default function SaasEntry() {
  ...
  const { loading: workspaceLoading, needsWorkspaceSelection, workspaces } = useWorkspace();
  ...
  /** Con sesión: esperar workspaces; sin ninguno → onboarding (crear/seleccionar tenant); si hay → dashboard. */
  useEffect(() => {
    if (loading || !user) return;
    if (workspaceLoading) return;
    const needsOnboarding =
      workspaces.length === 0 || needsWorkspaceSelection;
    navigate(needsOnboarding ? "/saas/onboarding" : "/saas/home", { replace: true });
  }, [user, loading, workspaceLoading, needsWorkspaceSelection, workspaces.length, navigate]);

  const handleAdminLogin = () => {
    if (!isDemoAuthAllowed()) return;
    enterDemo();
    client.entities.activities.create({
      data: { activity_type: "login", description: "Admin demo login", source: "saas_entry", status: "completed" }
    }).catch(() => {});
  };
  ...
  /** Con sesión activa esta pantalla solo redirige: no mostrar el formulario de login detrás. */
  if (loading || user) {
    return (
      <div className="min-h-screen bg-[#050507] flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-8 h-8 text-blue-500/40 animate-spin mx-auto mb-3" />
  ...
  return (
    <div className="min-h-screen bg-[#050507] relative overflow-hidden flex items-center justify-center">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-blue-900/[0.03] rounded-full blur-[120px]" />
      </div>
      ...
      <div className={cn(
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-600/80 to-cyan-600/80 shadow-xl shadow-blue-500/10 mb-4">
          ...
          {mode === "main" ? (
            <>
              ...
              {isDemoAuthAllowed() && (
                <Button
                  ...
                  className="w-full h-11 bg-blue-600/90 hover:bg-blue-500/90 text-white font-medium rounded-xl shadow-lg shadow-blue-500/10 transition-all duration-300 hover:shadow-blue-500/20 mb-3 text-sm"
                >
            ...
            <>
              <div className="text-center mb-5">
                <KeyRound className="w-4 h-4 text-blue-400/60 mx-auto mb-2" />
              ...
              <Button
                ...
                className="w-full h-10 bg-blue-600/80 hover:bg-blue-500/80 text-white font-medium rounded-xl text-sm mb-3"
              >
                Verificar y Acceder
              </Button>
