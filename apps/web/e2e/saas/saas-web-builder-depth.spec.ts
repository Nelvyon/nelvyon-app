...
test.describe("SaaS Web Builder Depth (S37)", () => {
  test.beforeEach(async ({ page, context }) => {
    ...
    await page.route("**/api/saas/web-builder**", async (route) => {
      const url = route.request().url();
      const path = new URL(url).pathname;
      if (path.endsWith("/p-e2e-1") || path.includes("/web-builder/p-e2e-1")) {
  ...
  test("listado de páginas carga sin error y muestra KPIs", async ({ page }) => {
    await gotoAwaitingApi(page, "/saas/web-builder", "/api/saas/web-builder");
  ...
  test("editor WYSIWYG abre con sections y permite guardar", async ({ page }) => {
    ...
    await gotoAwaitingApi(page, "/saas/web-builder/p-e2e-1", "/api/saas/web-builder");
    const body = await page.locator("body").textContent();
    expect(body).not.toContain("Something went wrong");
