...
export async function GET(req: Request) {
  try {
    ...
    const campanias = await getSaasCampaniasService().getCampanias(tenant.id);
...
export async function POST(req: Request) {
  try {
    ...
    if (typeof b.body !== "string" || b.body.trim().length === 0) return NextResponse.json({ error: "body is required" }, { status: 400 });
    if (typeof b.channel !== "string") return NextResponse.json({ error: "channel is required" }, { status: 400 });
    const campania = await getSaasCampaniasService().createCampania(tenant.id, {
      name: b.name,
      ...
      audienceFilter: typeof b.audienceFilter === "object" && b.audienceFilter !== null ? (b.audienceFilter as Record<string, unknown>) : {},
    });
