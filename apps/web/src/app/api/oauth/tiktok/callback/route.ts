...
export async function GET(req: Request) {
  ...
  if (error || !code) {
    return redirectToIntegrations(origin, "error=tiktok_denied");
  }

  const stateRaw = url.searchParams.get("state");
  const parsed = parseState(stateRaw);
  if (!parsed) {
    return redirectToIntegrations(origin, "error=tiktok_failed");
  }
  ...
  try {
    ...
    await OAuthService.instance().saveConnection(parsed.userId, "tiktok", {
      accessToken,
      expiresAt,
      accountId,
      accountName,
    ...
    return redirectToIntegrations(origin, "success=tiktok");
  } catch {
    return redirectToIntegrations(origin, "error=tiktok_failed");
  }
}
