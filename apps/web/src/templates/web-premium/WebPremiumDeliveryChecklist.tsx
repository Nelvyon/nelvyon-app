...
export function WebPremiumDeliveryChecklist() {
  return (
    <section aria-labelledby="delivery-qa-heading" className="border-t border-dashed border-border bg-card px-4 py-10 sm:px-6 lg:py-14">
      <div className="mx-auto max-w-5xl space-y-4">
        <h2 className="text-xl font-semibold tracking-tight text-foreground" id="delivery-qa-heading">
          OS delivery QA (Web Premium checklist)
        </h2>
        <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground">
          Mirrors backend/ops/runbooks/web_premium_nelvyon_v1.md expectations plus responsive, console, link, SEO, CWV, and Design
          System v2 parity. Check items manually — this panel does not run tests from the browser.
        </p>
        <ol className="list-decimal space-y-4 pl-5 text-sm leading-relaxed text-foreground">
          {WEB_PREMIUM_DELIVERY_ITEMS.map((item) => (
