...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!customerId || !accessToken || !refreshToken || !expiryRaw) {
      return res.status(400).json({ error: "customerId, accessToken, refreshToken y tokenExpiry son requeridos" });
    }
    ...
    await getGoogleAdsService().saveCredentials(user.userId, customerId, accessToken, refreshToken, tokenExpiry);
