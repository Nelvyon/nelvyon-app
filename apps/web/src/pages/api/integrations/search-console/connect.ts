...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!siteUrl || !accessToken || !refreshToken || !expiryRaw) {
      return res.status(400).json({ error: "siteUrl, accessToken, refreshToken y tokenExpiry son requeridos" });
    }
    ...
    await getSearchConsoleService().saveCredentials(user.userId, siteUrl, accessToken, refreshToken, tokenExpiry);
    return res.status(200).json({ ok: true });
  } catch (e: unknown) {
    if (e instanceof OsAgentError && e.message === "Unauthorized") return res.status(401).json({ error: "Token inválido" });
    if (e instanceof OsAgentError && e.code === "search_console_validation") return res.status(400).json({ error: e.message });
