...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const twimlUrl = typeof req.body?.twimlUrl === "string" ? req.body.twimlUrl.trim() : "";
    if (!to || !twimlUrl) {
      return res.status(400).json({ error: "to y twimlUrl son requeridos" });
    }

    const out = await getTwilioService().makeCall(user.userId, to, twimlUrl);
