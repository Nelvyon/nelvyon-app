...
import { BlockSection, fade, str } from "./shared";
import { useInViewportRef } from "./useInViewport";

const Hero3dBlock = dynamic(() => import("./blocks3d/Hero3dBlock").then((m) => ({ default: m.Hero3dBlock })), { ssr: false });
const Product3dBlock = dynamic(() => import("./blocks3d/Product3dBlock").then((m) => ({ default: m.Product3dBlock })), { ssr: false });
const Stats3dBlock = dynamic(() => import("./blocks3d/Stats3dBlock").then((m) => ({ default: m.Stats3dBlock })), { ssr: false });

function LazyBlock({ children, eager }: { children: ReactNode; eager?: boolean }) {
  const { ref, visible } = useInViewportRef("120px");
  if (eager) return <>{children}</>;
  return (
    <div ref={ref}>
      {visible ? children : <div aria-hidden className="min-h-[200px]" />}
    </div>
  );
}

export function BlockRenderer({ blocks }: { blocks: LandingBlock[] }) {
  const firstHeroIdx = blocks.findIndex((b) => b.type === "hero" || b.type === "hero_3d");

  function renderBlock(block: LandingBlock, index: number) {
    const isAboveFold = index === firstHeroIdx;
    const p = block.props;
    switch (block.type) {
          case "hero_3d":
            return <Hero3dBlock block={block} key={block.id} />;
          case "product_3d":
            return <Product3dBlock block={block} key={block.id} />;
          case "stats_3d":
            return <Stats3dBlock block={block} key={block.id} />;
          case "hero":
            return (
              <BlockSection block={block} key={block.id}>
                <div className="relative overflow-hidden py-16 text-center md:py-28">
                  ...
                  <motion.h1
                    className="relative bg-gradient-to-b from-white to-white/80 bg-clip-text text-4xl font-bold tracking-tight text-transparent md:text-6xl lg:text-7xl"
                  ...
                  <motion.p
                    className="relative mx-auto mt-6 max-w-2xl text-lg leading-relaxed opacity-90 md:text-xl"
                    style={{ color: str(p.textColor, "#fff") }}
                    {...fade}
                  >
                    {str(p.subheadline)}
                  ...
                  {p.ctaText ? (
                    <motion.a
                      className="relative mt-10 inline-flex min-h-[48px] items-center rounded-full bg-white px-10 py-3 text-sm font-semibold text-slate-900 shadow-elevated transition hover:scale-[1.03] hover:shadow-lg active:scale-[0.98]"
                      href={str(p.ctaUrl, "#")}
                      whileHover={{ y: -2 }}
                      whileTap={{ scale: 0.98 }}
                    >
                  ...
                  {items.map((t, i) => (
                    ...
                    >
                      ...
                      <footer className="mt-6 flex items-center gap-3 border-t border-border pt-4">
                        {t.avatar ? (
                          <Image alt="" className="rounded-full object-cover" height={40} loading="lazy" src={t.avatar} width={40} />
                        ) : (
              ...
              <BlockSection block={block} key={block.id}>
                {p.imageUrl ? (
                  <div className="relative mx-auto aspect-video max-w-4xl overflow-hidden rounded-2xl shadow-elevated">
                    <Image alt={str(p.alt, "")} className="object-cover" fill loading="lazy" sizes="(max-width:768px) 100vw, 896px" src={str(p.imageUrl)} />
                  </div>
                ) : null}
              </BlockSection>
            );
  ...
  return (
    <motion.div className="font-[family-name:var(--font-inter,Inter,var(--font-dm-sans,DM_Sans),system-ui,sans-serif)]">
      {blocks.map((block, index) => (
        <LazyBlock eager={index === firstHeroIdx} key={block.id}>
        ...
        <motion.div className="overflow-hidden rounded-xl border border-border bg-card shadow-card" key={i} {...fade}>
          ...
          {open === i ? (
            <motion.div
              ...
              className="border-t border-border px-6 pb-4 pt-2 text-sm leading-relaxed text-muted-foreground"
