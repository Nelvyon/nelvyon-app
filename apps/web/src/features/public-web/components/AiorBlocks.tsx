...
export function AiorFeatureGrid({ items }: { items: readonly Feature[] }) {
  return (
...
export function AiorCheckList({ items }: { items: readonly string[] }) {
  return (
    <ul className="hero-list" style={{ textAlign: "left" }}>
      {items.map((item) => (
        <li key={item}>
          <i className="fa-sharp fa-solid fa-circle-check" aria-hidden /> {item}
        </li>
      ))}
    </ul>
  );
...
export function AiorProcess({ steps }: { steps: readonly Feature[] }) {
  return (
    <div className="row gy-4">
      {steps.map((step, i) => (
...
export function AiorRelated({ title, items }: { title: string; items: readonly LinkItem[] }) {
  if (!items.length) return null;
  return (
    <AiorSection soft>
      <AiorTitle eyebrow="Relacionado" title={title} center />
      <div className="row gy-4">
        {items.map((item) => (
