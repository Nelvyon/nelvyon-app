...
import type { OsLegacyTask } from "./types";

/** Legacy fallback UI (entities/os_tasks). Enabled via NEXT_PUBLIC_OS_TASKS_CANONICAL_UI=false */
export function OsTasksListLegacyView() {
  const perms = useOsPermissions();
  const [tasks, setTasks] = useState<OsLegacyTask[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [tRes, cRes, pRes] = await Promise.all([
        osTasksLegacyApi.list({ limit: 500, query, sort: "-id" }),
  ...
  const markComplete = async (task: OsLegacyTask) => {
    if (!perms.canEdit) return;
    try {
      await osTasksLegacyApi.update(task.id, { status: "completada" });
      setTasks((prev) =>
  ...
  return (
    <OsShellLayout onRefresh={() => void load()} refreshing={loading}>
      <OsPageHeader
        ...
        description="Fuente: entities/os_tasks (legacy)."
        actions={
          perms.canCreate ? (
            ...
            {tasks.map((t) => {
              ...
              return (
                <tr key={t.id} className="border-b border-white/5">
                  ...
                  <td className="px-4 py-2">
                    <OsStatusBadge label={t.priority ?? "media"} tone={taskPriorityTone(t.priority ?? "media")} />
                  </td>
