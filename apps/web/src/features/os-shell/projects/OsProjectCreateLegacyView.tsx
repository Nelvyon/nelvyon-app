...
/** Legacy fallback create (nelvyon_projects). */
export function OsProjectCreateLegacyView() {
  ...
  const [form, setForm] = useState<OsLegacyProjectWriteInput>(emptyOsProjectForm(presetClient));
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const created = await osProjectsLegacyApi.create({
        ...form,
        name: form.name.trim(),
      });
