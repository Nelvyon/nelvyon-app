...
import type { OsLegacyProject } from "./types";

/** Legacy fallback UI (nelvyon_projects). Enabled via NEXT_PUBLIC_OS_PROJECTS_CANONICAL_UI=false */
export function OsProjectsListLegacyView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [pRes, cRes] = await Promise.all([
        osProjectsLegacyApi.list({ limit: 500, query, sort: "-id" }),
